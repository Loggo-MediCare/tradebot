...
def get_trading_signal():
    ...
    # ========== 標題區 ==========
    fmt.print_header("🤖 台股 2308.TW (南亞) AI " \
    "交易信号生成器 V2")
    ...
    try:
        ...
        df = yf.download('2308.TW', period='90d', progress=False, auto_adjust=True)
        ...
        try:
            ticker_info = yf.Ticker('2308.TW').info
    ...
    fmt.print_metric("股票代碼", "2308.TW (2308.TWle)")
    fmt.print_metric("當前價格", f"${current_price:.2f}")
    fmt.print_metric("股票代碼", "2308.TW (南亞)")
    ...
    sentiment_result = calculate_sentiment_score('2308.TW', verbose=False)
    ...
    if action_value > 0.1:
        ...
        buy_score, signal_override, buy_reasons, buy_warnings, buy_metadata, sentiment_result = \
            calculate_enhanced_buy_score_with_sentiment(
                ...
                ai_action=action_value, buy_weights=buy_weights, symbol='2308.TW'
            )
    ...
    print("║        📱 快速摘要                   ║")
    print("╠" + "═" * 40 + "╣")
    print(f"║  股票: 2308.TW (2308.TWle)                 ║")
    print(f"║  股票: 2308.TW (南亞)                ║")
...
if __name__ == "__main__":
    ...
    if result:
        ...
        try:
            import yfinance as yf
            chart_df = yf.Ticker("2308.TW").history(period="6mo")
            chart_df.columns = [c.lower() for c in chart_df.columns]
            chart_path = "2308_chart.png"
            plot_candlestick(chart_df, "2308.TW", save_path=chart_path)
        except Exception as e:
            print(f"   圖表生成失敗: {e}")
