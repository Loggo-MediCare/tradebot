...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('3587')


    print(f"🤖 3587 (閎康) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    model_filename = "ppo_3587_two_improved"
    ...
    try:
        ...
        df = yf.download('3587.TWO', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ...
            ticker_info = yf.Ticker('3587.TWO').info
    ...
    ai_muted = should_mute_ai_signal('3587.TWO', threshold=52)
    ...
    try:
        ...
        profit_positive = evaluate_fundamentals_for_sell(yf, '3587.TWO')['good']
    ...
    weight_calc = DynamicWeightCalculator('3587.TWO')
    ...
    sentiment_result = calculate_sentiment_score('3587.TWO', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            ...
            fundamentals = evaluate_fundamentals_for_sell(yf, '3587.TWO')
        ...
        _growth = calculate_growth_score_adjustment(yf, '3587.TWO')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"   {get_model_accuracy_display('3587')}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 3587 閎康 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('3587', '閎康', max_results=5)