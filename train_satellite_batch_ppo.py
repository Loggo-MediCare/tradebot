...
def train_stock(ticker, symbol, name):
    ...
    try:
        ...
        env = DummyVecEnv([lambda df_=df_clean: ImprovedTradingEnv(df_)])
...
if __name__ == '__main__':
    print("=" * 70)
    print("批量訓練 衛星/網路/國防 相關股票 PPO 模型")
    ...
    print("PPO 訓練完成摘要")
    print('='*70)
    print(f"{'代號':<14} {'名稱':<20} {'狀態':<4} {'準確度':>8} {'回報率':>10}")
    print("-" * 60)
    for ticker, name, success, acc, ret in results:
        ...
        print(f"{ticker:<14} {name:<20} {status:<4} {acc_str:>8} {ret_str:>10}")
