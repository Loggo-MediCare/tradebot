"""台股 2881 (富邦金) — PPO only | 回測+37.73% vs 買入持有+36.30%"""
...
import numpy as np,pandas as pd,yfinance as yf,gymnasium as gym
...
CODE='2881';TICKER='2881.TW';NAME='富邦金'
PPO_MODEL='ppo_2881_tw_improved'
# PPO評估基準: P&L回測+37.73% vs 買入持有+36.30% (分類準確度為誤導性指標)
PPO_BACKTEST_RETURN=37.73;BAH_RETURN=36.30
...
def get_trading_signal():
    ...
    print("\n"+"="*80+f"\n🤖 PPO 交易訊號  [回測+{PPO_BACKTEST_RETURN:.2f}% vs 買入持有+{BAH_RETURN:.2f}%]\n"+"="*80)
    ...
    except Exception as e:print(f"⚠️  PPO:{e}")

    # ── Scoring (技術指標 + PPO) ──────────────────────────────────────────────
    score=50.;reasons=[];warns=[]
    if ppo_action>0.1:score+=ppo_action*30;reasons.append(f"PPO看多{ppo_action:+.2f}")
    elif ppo_action<-0.1:score+=ppo_action*30;warns.append(f"PPO看空{ppo_action:+.2f}")
    if macd>msig:score+=7;reasons.append("MACD金叉")
    else:score-=7;warns.append("MACD死叉")
    if s10>s30:score+=7;reasons.append("均線多頭")
    else:score-=7;warns.append("均線空頭")
    if rsi<35:score+=10;reasons.append(f"RSI超賣{rsi:.1f}")
    elif rsi>75:score-=10;warns.append(f"RSI超買{rsi:.1f}")
    if slope>0.05:score+=5;reasons.append("MA50上升")
    elif slope<-0.05:score-=5;warns.append("MA50下降")
    if obv>obvm:score+=4;reasons.append("OBV多頭")
    else:score-=4
    if target_price:
        up=(target_price-cp)/cp*100
        if up>10:score+=7;reasons.append(f"目標價空間{up:.1f}%")
        elif up<-5:score-=7;warns.append(f"超越目標價{up:.1f}%")
    ...
    return{'date':latest_date,'symbol':TICKER,'name':NAME,'current_price':cp,'price_change_pct':chg,
           'signal':signal,'score':score,'ppo_action':ppo_action,'rsi':rsi,'macd':macd}
