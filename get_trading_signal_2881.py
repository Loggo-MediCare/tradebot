...
def get_trading_signal():
    ...
    # 1. 加载模型
    model_path = r"C:\Users\Silvi\Projects\trading-bot\ppo_2881_tw_improved"
    print(f"\n📦 加载 AI 模型: {model_path}")
    ...
    try:
        ...
        df = yf.download('2881.TW', period='90d', progress=False, auto_adjust=True)
        ...
        try:
            ticker_info = yf.Ticker('2881.TW').info
            # ===== 基本面資料（PEG / 市值）=====
    ...
    # 7. 初始化动态权重计算器
    weight_calc = DynamicWeightCalculator('2881.TW')
    ...
    sentiment_result = calculate_sentiment_score('2881.TW', verbose=False)
...
if __name__ == "__main__":
    ...
    if result:
        ...
        try:
            import yfinance as yf
            chart_df = yf.Ticker("2881.TW").history(period="6mo")
            chart_df.columns = [c.lower() for c in chart_df.columns]
            chart_path = "2881_chart.png"
            plot_candlestick(chart_df, "2881.TW", save_path=chart_path)
        except Exception as e:
            print(f"   圖表生成失敗: {e}")

        print(f"\n📱 快速摘要:")
        print(f"   股票: {result['symbol']} (富邦金)")
        ...
        # 顯示AI模型準確度摘要
        print(f"   {get_model_accuracy_display('2881.TW')}")
    else:
        print("\n❌ 信号生成失败")