...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('7717')


    print(f"🤖 7717 (萊德光電) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    model_path = r"ppo_7717_two_improved"
    ...
    try:
        ...
        df = yf.download('7717.TWO', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
    ...
    ai_muted = should_mute_ai_signal('7717.TWO', threshold=52)
    ...
    try:
        ...
        profit_positive = evaluate_fundamentals_for_sell(yf, '7717.TWO')['good']
    ...
    sentiment_result = calculate_sentiment_score('7717.TWO', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            ...
            fundamentals = evaluate_fundamentals_for_sell(yf, '7717.TWO')
        ...
        _growth = calculate_growth_score_adjustment(yf, '7717.TWO')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"   {get_model_accuracy_display('7717')}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 7717 萊德光電 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('7717', '萊德光電', max_results=5)