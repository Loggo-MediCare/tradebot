...
def analyze_gex(calls_df, puts_df, spot, expiry_str):
    ...
    # 計算 flip 上方的 net_gex 符號，以正確描述「越過後是正還是負 Gamma」
    # 用 net_gex_hypo(flip+1) 直接判斷，不假設「當前的反面」
    above_flip_positive = None
    if gamma_flip_found and gamma_flip is not None and flip_subset:
        try:
            val_above = net_gex_hypo(gamma_flip + 1.0)
            above_flip_positive = (val_above > 0)
        except Exception:
            above_flip_positive = None
    ...
    return {
        ...
        'above_flip_positive':  above_flip_positive,
...
def calc_squeeze_score(gex_data, spot):
    ...
    # Step B：越過 Flip 後的狀態由 net_gex_hypo(flip+1) 實際計算決定
    # 不假設「當前的反面」，直接用掃描結果
    above_flip_positive = gex_data.get('above_flip_positive')
    if above_flip_positive is not None:
        after_flip_above = "正 Gamma（穩定）"     if above_flip_positive else "負 Gamma（追漲追跌）"
        after_flip_below = "負 Gamma（追漲追跌）" if above_flip_positive else "正 Gamma（穩定）"
    else:
        after_flip_above = "正 Gamma（穩定）"     if currently_negative_gamma else "負 Gamma（追漲追跌）"
        after_flip_below = "負 Gamma（追漲追跌）" if currently_negative_gamma else "正 Gamma（穩定）"
        ...
        if gamma_flip_found and gamma_flip is not None:
            if gamma_flip > spot:
                ...
                if dist < 5:
                    ...
                    reasons.append(
                        f"Gamma Flip ${gamma_flip:.0f} 在上方 {dist:.1f}%——"
                        f"突破後轉{after_flip_above}（追漲力道出現）"
                    )
                else:
                    warnings.append(
                        f"Gamma Flip ${gamma_flip:.0f} 在上方 {dist:.1f}%——"
                        f"目前仍在正 Gamma 保護範圍，突破後轉{after_flip_above}"
                    )
            else:
                dist = (spot - gamma_flip) / spot * 100
                warnings.append(
...
def interpret_signal(score, gex_data, spot):
    ...
    # ── Gamma Flip 描述（由 net_gex_hypo(flip+1) 決定上方狀態）───────────
    above_flip_positive = gex_data.get('above_flip_positive')
    ...
    if gf_found and gf is not None:
        if gf > spot:
            ...
            # 突破後進入 flip 上方區域
            if above_flip_positive is not None:
                after_flip_state = "正 Gamma（穩定）" if above_flip_positive else "負 Gamma（追漲追跌）"
            else:
                after_flip_state = "負 Gamma（追漲追跌）" if not is_neg_gamma else "正 Gamma（穩定）"
        else:
            dist_pct  = (spot - gf) / spot * 100
            direction = "跌破"
            location  = f"下方 {dist_pct:.1f}%"
            # 跌破後進入 flip 下方區域（下方狀態 = 上方的反面）
            if above_flip_positive is not None:
                after_flip_state = "負 Gamma（追漲追跌）" if above_flip_positive else "正 Gamma（穩定）"
            else:
                after_flip_state = "負 Gamma（追漲追跌）" if not is_neg_gamma else "正 Gamma（穩定）"
        flip_note = (
...
def print_gex_report(symbol, expiry_choice=None):
    ...
    if gf_found and gf is not None:
        ...
        _afp  = gex.get('above_flip_positive')
        if _afp is not None:
            flip_regime = "上方=正Gamma / 下方=負Gamma" if _afp else "上方=負Gamma / 下方=正Gamma"
        else:
            flip_regime = "上方=正Gamma / 下方=負Gamma" if gex['total_net_gex'] < 0 else "上方=負Gamma / 下方=正Gamma"
        print(f"  {'⚡ Gamma Flip':<22} ${gf:>9.0f}  {side} {dist:.1f}%  {flip_regime}")
    else:
        print(f"  {'⚡ Gamma Flip':<22} {'N/A':>10}  掃描範圍內未找到翻轉點")
    print(f"  {'🟡 Peak |GEX|（磁吸）':<22} ${pgk:>9.2f}  |GEX| 最大 Strike（磁吸點）")
