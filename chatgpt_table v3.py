...
def upside_score(upside_pct):
    ...
    if u >= 50:
        return 1.0
    return clamp(np.log1p(u/10.0) / np.log1p(50/10.0))
...
def compute_final_score(row):
    ...
    base = model_acc * vol_score * pat_score
    ...
    # 缺資料 → 用平均時忽略 NaN
    analyst_parts = [x for x in [u_score, r_score, c_score] if not pd.isna(x)]
...
def main():
    ...
    # 統一 price 欄位名
    if "price" not in sig.columns and "current_price" in sig.columns:
        sig["price"] = sig["current_price"]
    elif "price" not in sig.columns and "Price" in sig.columns:
        sig["price"] = sig["Price"]
    ...
    keep_ana = [
        ...
        "current_price",
        "target_mean_price",
        "upside_to_target_mean_%", # This is the target column name
        "num_analyst_opinions",
        "recommendation_key",
        "recommendation_mean"
    ]
    
    # Find the actual upside column name in ana, allowing for variations
    actual_upside_col = None
    for col in ana.columns:
        if col.lower().strip() == "upside_to_target_mean_%":
            actual_upside_col = col
            break
    
    if actual_upside_col is None:
        raise ValueError(f"ANALYST_CSV 找不到欄位：'upside_to_target_mean_%' 或類似名稱")

    # Rename the actual upside column to the standardized name if it's different
    if actual_upside_col != "upside_to_target_mean_%":
        ana.rename(columns={actual_upside_col: "upside_to_target_mean_%"}, inplace=True)

    # Filter ana to only include relevant columns (now that upside is standardized)
    ana = ana[[c for c in keep_ana if c in ana.columns]].copy()


    # -------- merge --------
    # Merge signals with analyst data. If there's a 'price' column in sig, it will be kept.
    # If 'current_price' exists in both, the analyst one will be suffixed '_ana'.
    df = sig.merge(ana, on="ticker", how="left", suffixes=("", "_ana"))

    # Ensure 'price' column is available for stop/take calculations
    if "price" not in df.columns:
        if "current_price" in df.columns: # Use analyst's current_price if signal's price is missing
            df["price"] = df["current_price"]
        elif "current_price_ana" in df.columns: # Use analyst's current_price_ana if signal's price is missing
            df["price"] = df["current_price_ana"]
        else:
            df["price"] = np.nan # Fallback if no price column is found
    ...
    def _stop_take(row):
        price = row.get("price", np.nan) # Use the standardized 'price' column
    ...
    show_cols = [
        ...
        "final_score",
        "risk_level",
        "price",  # signals 的現價欄 (standardized)
        "target_mean_price",
