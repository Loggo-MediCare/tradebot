"""Train XGBoost + PPO for US semiconductor/equipment stocks"""
...
STOCKS = [
    ('AMAT', 'Applied Materials',   '2015-01-01'),
    ('ASML', 'ASML Holding',        '2015-01-01'),
    ('KLAC', 'KLA Corporation',     '2015-01-01'),
    ('LRCX', 'Lam Research',        '2015-01-01'),
    ('MU',   'Micron Technology',   '2015-01-01'),
    ('PLTR', 'Palantir',            '2020-09-30'),  # IPO Sep 2020
    ('SIMO', 'Silicon Motion',      '2015-01-01'),
    ('SNDK', 'SanDisk',             '2024-02-01'),  # re-listed as SNDK Feb 2024
    ('TER',  'Teradyne',            '2015-01-01'),
]
...
def train_stock(ticker, company, start_date, idx):
    ...
    if need_xgb:
        ...
        print("  [XGBoost] already exists — skipped")
        try:
            d = json.load(open(f'model_accuracy_{ticker}.json', encoding='utf-8'))
    ...
    if need_ppo:
        ...
        print("  [PPO] already exists — skipped")
        try:
            d = json.load(open(f'model_accuracy_{ticker}_ppo.json', encoding='utf-8'))
...
if __name__ == '__main__':
    print("=" * 60)
    print(f"  US Semiconductor Batch — {len(STOCKS)} stocks")
    print(f"  Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 60)

    results, failed = [], []
    for idx, (ticker, company, start) in enumerate(STOCKS, 1):
        try:
            r = train_stock(ticker, company, start, idx)
    ...
    print("=" * 68)
    print(f"  {'Ticker':<6} {'Company':<24} {'XGBoost':>9} {'PPO':>7} {'Winner'}")
    print("  " + "-"*62)
    for r in sorted(results, key=lambda x: max(x['xgb_acc'], x['ppo_acc']), reverse=True):
        ...
        print(f"  {r['ticker']:<6} {r['company']:<24} "
