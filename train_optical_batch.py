"""Train missing PPO + full models for optical/fiber stocks"""
...
# (code, ticker, name, need_xgb, need_ppo)
STOCKS = [
    ('4966', '4966.TW',  '譜瑞-KY',      False, True),   # add PPO
    ('4979', '4979.TW',  '華星光通',      False, True),   # add PPO
    ('CIEN', 'CIEN',     'Ciena',         False, True),   # add PPO
    ('COHR', 'COHR',     'Coherent Corp', True,  True),   # full train
    ('GLW',  'GLW',      'Corning',       True,  True),   # full train
    ('INFN', 'INFN',     'Infinera',      True,  True),   # full train
    ('VIAV', 'VIAV',     'Viavi Solutions',True, True),   # full train
]

END_DATE = '2026-05-01'; STEPS = 100_000
...
class TradingEnv(gym.Env):
    ...
    def reset(self,seed=None,options=None):
        super().reset(seed=seed); self.i=0; self.bal=10000.0; self.sh=0; self.profit=0.0; return self._obs(),{}
    def _obs(self):
    ...
    def step(self,action):
        ...
        return self._obs(),self.profit/10000.0+(0.01 if abs(a)>0.1 else 0),done,False,{}

results=[]
for code, ticker, name, need_xgb, need_ppo in STOCKS:
    sl=code.lower()
    is_tw = '.' in ticker
    sfx = ticker.split('.')[-1] if is_tw else 'US'
    xgb_file = f'xgb_{code}_{sfx.lower()}_model.pkl' if is_tw else f'xgb_{sl}_model.pkl'
    ppo_file  = f'ppo_{code}_{sfx.lower()}_improved'  if is_tw else f'ppo_{sl}_improved'

    print(f"\n{'='*55}\n  [{STOCKS.index((code,ticker,name,need_xgb,need_ppo))+1}/{len(STOCKS)}] {code} {name}")
    print(f"  Need: {'XGB ' if need_xgb else ''}{'PPO' if need_ppo else ''}\n{'='*55}")
    ...
    print(f"  B&Hold: {bh:+.1f}%")

    df=build_features(df); dc=df.dropna(subset=FEAT+['target'])
    ...
    if need_xgb:
        ...
        xm.fit(Xt,yt); xgb_acc=accuracy_score(ye,xm.predict(Xe))
        joblib.dump(xm, xgb_file)
        acc_key = f'model_accuracy_{code}_{sfx}.json' if is_tw else f'model_accuracy_{code}.json'
        json.dump({'symbol':code,'ticker':ticker,'model_type':'XGBoost',
                  ...
                  open(acc_key,'w',encoding='utf-8'),indent=2)
        print(f"  [XGBoost] {xgb_acc*100:.2f}%")
    else:
        try:
            acc_key = f'model_accuracy_{code}.json'
            d=json.load(open(acc_key,encoding='utf-8')); xgb_acc=d.get('backtest_accuracy',0)/100
            print(f"  [XGBoost] existing {xgb_acc*100:.2f}%")
        except: pass

    ppo_acc=0.0
    if need_ppo:
        print(f"  [PPO] Training ({STEPS//1000}k steps)...")
        env=DummyVecEnv([lambda: TradingEnv(tr)])
        pm=PPO('MlpPolicy',env,verbose=0,learning_rate=0.0003,n_steps=2048,batch_size=64,n_epochs=10,gamma=0.99,ent_coef=0.01)
        pm.learn(total_timesteps=STEPS); pm.save(ppo_file)
        env2=TradingEnv(te.reset_index(drop=True)); obs,_=env2.reset()
        preds,labels=[],[]
        for i in range(len(te)-6):
            act,_=pm.predict(obs,deterministic=True); preds.append(1 if float(act[0])>0.1 else 0)
            labels.append(int(te.iloc[i]['target'])); obs,_,done,_,_=env2.step(act)
            if done: break
        ppo_acc=accuracy_score(labels,preds) if preds else 0.0
        ppo_key = f'model_accuracy_{code}_{sfx}_ppo.json' if is_tw else f'model_accuracy_{code}_ppo.json'
        json.dump({'symbol':code,'ticker':ticker,'model_type':'PPO',
                   ...
                   'last_updated':datetime.now().strftime('%Y-%m-%d %H:%M:%S')},
                  open(ppo_key,'w',encoding='utf-8'),indent=2)
        print(f"  [PPO]     {ppo_acc*100:.2f}%  (buys:{sum(preds)}/{len(preds)})")

    winner='XGBoost' if xgb_acc>=ppo_acc else 'PPO'
    results.append({'code':code,'name':name,'xgb':round(xgb_acc*100,2),'ppo':round(ppo_acc*100,2),'bh':round(bh,1),'winner':winner})

print(f"\n{'='*62}\n  RESULTS\n{'='*62}")
print(f"  {'Code':<6} {'Name':<18} {'XGB':>7} {'PPO':>7} {'B&Hold':>8} W")
print("  "+"-"*56)
for r in sorted(results,key=lambda x:max(x['xgb'],x['ppo']),reverse=True):
    ...
    print(f"  {r['code']:<6} {r['name']:<18} {r['xgb']:>6.1f}% {r['ppo']:>6.1f}% {r['bh']:>7.1f}% {r['winner']} {star}")
