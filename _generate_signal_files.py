"""Generate get_trading_signal_XXXX.py for all 16 stocks. Run once, then delete."""
import os
os.chdir(os.path.dirname(os.path.abspath(__file__)))

STOCKS = [
    ('1326', '1326.TW', '台化',    44.32, 27.14),
    ('2303', '2303.TW', '聯電',    67.03, 32.24),
    ('2308', '2308.TW', '台達電',  54.41, 53.01),
    ('2337', '2337.TW', '旺宏',    59.28, 63.57),  # existing signal file — will skip
    ('2344', '2344.TW', '華邦電',  56.40, 37.89),
    ('2368', '2368.TW', '金像電',  50.81, 46.27),
    ('2449', '2449.TW', '京元電子',60.00, 58.29),
    ('2609', '2609.TW', '陽明',    70.99, 70.67),
    ('2882', '2882.TW', '國泰金',  70.09, 69.95),  # just created — will skip
    ('2890', '2890.TW', '永豐金',  75.68, 25.32),
    ('2892', '2892.TW', '第一金',  85.05, 84.88),
    ('3034', '3034.TW', '聯詠',    52.79, 28.05),
    ('3036', '3036.TW', '文曄',    50.09, 62.11),
    ('3189', '3189.TW', '景碩',    57.48, 39.71),
    ('3231', '3231.TW', '緯創',    64.68, 34.06),
    ('3443', '3443.TW', '創意',    49.37, 43.72),
]
...
XGB_MODEL = 'xgb_{code}_tw_model.pkl'
PPO_MODEL = 'ppo_{code}_tw_improved'
...
def get_trading_signal():
    ...
    if target_price:
        ...
        print(f"分析師目標價:    NT${{target_price:.2f}}  (距現價 {{upside:+.1f}}%)")

    print("\\n" + "=" * 80)
    print(f"🧠 XGBoost 模型  (準確度 {{XGB_ACC:.2f}}%)")
    print("=" * 80)
    ...
    try:
        ...
        if should_mute_ai_signal(TICKER, threshold=52):
            ...
            direction = '買入 📈' if ppo_action > 0.1 else ('賣出 📉' if ppo_action < -0.1 else '持有 ➡️')
            print(f"動作值: {{ppo_action:+.4f}}  →  {{direction}}")
    except Exception as e:
        print(f"⚠️  PPO 失敗: {{e}}")
    ...
    if xgb_prob is not None:
        ...
        if xgb_prob > 0.6:
            reasons.append(f"XGBoost 買入概率 {{xgb_prob*100:.1f}}%")
        elif xgb_prob < 0.4:
            warnings_list.append(f"XGBoost 看空 ({{xgb_prob*100:.1f}}%)")

    if ppo_action > 0.1:
        score += ppo_action * 15; reasons.append(f"PPO 建議買入 ({{ppo_action:+.2f}})")
    elif ppo_action < -0.1:
        score += ppo_action * 15; warnings_list.append(f"PPO 建議賣出 ({{ppo_action:+.2f}})")
    ...
    if ma50_slope > 0.05:
        score += 4; reasons.append(f"MA50 上升 (+{{ma50_slope:.2f}}%)")
    elif ma50_slope < -0.05:
        score -= 4; warnings_list.append(f"MA50 下降 ({{ma50_slope:.2f}}%)")

    if obv_now > obv_ma:
        score += 3; reasons.append("OBV 多頭")
    else:
        score -= 3
    ...
    if bb_position < 20:
        score += 5; reasons.append(f"接近布林帶下軌 ({{bb_position:.0f}}%)")
    elif bb_position > 85:
    ...
    if score >= 65:
        ...
        if target_price:
            print(f"      • 目標價: NT${{target_price:.2f}}")
    elif score <= 35:
    ...
    return {{'date': latest_date, 'symbol': TICKER, 'name': NAME,
            'current_price': current_price, 'price_change_pct': price_change_pct,
            'signal': signal, 'score': score,
            'xgb_prob': xgb_prob, 'ppo_action': ppo_action, 'rsi': rsi, 'macd': macd}}
...
created = []
skipped = []
for code, ticker, name, xgb_acc, ppo_acc in STOCKS:
    out = f'get_trading_signal_{code}.py'
    if os.path.exists(out):
        skipped.append(f'{code} {name}')
        continue
    content = TEMPLATE.format(
        code=code, ticker=ticker, name=name,
        xgb_acc=xgb_acc, ppo_acc=ppo_acc
    )
