...
def get_trading_signal():
    """生成今日交易信号"""
    print("=" * 80)
    print("🤖 美股 AMD AI AI HYBRID PPO + XGBoost 交易信号生成器")
    print("=" * 80)
    ...
    try:
        model = joblib.load("xgb_amd_model.pkl")
    ...
    ppo_model = None
    try:
        ppo_model = PPO.load("ppo_amd_improved")
        print("✅ PPO 模型加载成功!")
    except Exception as e:
    ...
    try:
        ...
        df = yf.download('AMD', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
    ...
    try:
        profit_positive = evaluate_fundamentals_for_sell(yf, 'AMD')['good']
    except Exception:
    ...
    sentiment_result = calculate_sentiment_score('AMD', verbose=True)
    ...
    if action_value > 0.1:
        ...
        # ==========================================
        # 🎯 AMD 专属策略增强 (基于相关性与机构资金)
        # ==========================================
        print("\n" + "=" * 80)
        print("🎯 AMD AMD 专属策略分析")
        print("=" * 80)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            # 获取公司基本面（超跌时才检查）
            fundamentals = evaluate_fundamentals_for_sell(yf, 'AMD')
        ...
        _growth = calculate_growth_score_adjustment(yf, 'AMD')
