...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('3707')


    print(f"🤖 3707 (漢磊) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    model_path = r"ppo_3707_tw_improved"
    ...
    try:
        ...
        df = yf.download('3707.TWO', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ...
            ticker_info = yf.Ticker('3707.TWO').info
    ...
    ai_muted = should_mute_ai_signal('3707.TWO', threshold=52)
    ...
    try:
        ...
        profit_positive = evaluate_fundamentals_for_sell(yf, '3707.TWO')['good']
    ...
    weight_calc = DynamicWeightCalculator('3707.TWO')
    ...
    sentiment_result = calculate_sentiment_score('3707.TWO', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            ...
            fundamentals = evaluate_fundamentals_for_sell(yf, '3707.TWO')
        ...
        _growth = calculate_growth_score_adjustment(yf, '3707.TWO')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"   {get_model_accuracy_display('3707')}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 3707 漢磊 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('3707', '漢磊', max_results=5)