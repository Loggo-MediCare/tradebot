...
def get_trading_signal():
    ...
    # ========== 標題區 ==========
    fmt.print_header("🤖 美股 GOOG (Google) AI 交易信号生成器 V2")
    print(f"   📅 生成時間: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"   📊 模型準確度: {get_model_accuracy_display('GOOG')}")

    # ========== 載入模型 ==========
    model_path = r"C:\Users\Silvi\Projects\trading-bot\ppo_goog_improved"
    ...
    try:
        ...
        df = yf.download('GOOG', period='90d', progress=False, auto_adjust=True)
    ...
    fmt.print_metric("股票代碼", "GOOG (Google)")
    ...
    sentiment_result = calculate_sentiment_score('GOOG', verbose=False)
    ...
    if action_value > 0.1:
        ...
        buy_score, signal_override, buy_reasons, buy_warnings, buy_metadata, sentiment_result = \
            calculate_enhanced_buy_score_with_sentiment(
                ...
                ai_action=action_value, buy_weights=buy_weights, symbol='GOOG'
            )
