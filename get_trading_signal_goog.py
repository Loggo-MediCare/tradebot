...
def get_trading_signal():
    ...
    print(f"🤖 GOOG (Google) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print("=" * 80)

    # 1. 加载模型
    import os
    model_filename = "ppo_goog_improved"
    ...
    try:
        ...
        df = yf.download('GOOG', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ticker_info = yf.Ticker('GOOG').info
    ...
    ai_muted = should_mute_ai_signal('GOOG', threshold=52)
    if ai_muted:
    ...
    try:
        profit_positive = evaluate_fundamentals_for_sell(yf, 'GOOG')['good']
    except Exception:
    ...
    sentiment_result = calculate_sentiment_score('GOOG', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            # 获取公司基本面（超跌时才检查）
            fundamentals = evaluate_fundamentals_for_sell(yf, 'GOOG')
        ...
        _growth = calculate_growth_score_adjustment(yf, 'GOOG')
...
if __name__ == "__main__":
    ...
    if result:
        print(f"\n📱 快速摘要:")
        print(f"   股票: {result['symbol']} (Google)")
        ...
        # 顯示AI模型準確度摘要
        print(f"   {get_model_accuracy_display('GOOG')}")
    else:
        print("\n❌ 信号生成失败")
