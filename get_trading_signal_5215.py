...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('5215')


    print(f"🤖 5215 (科嘉-KY) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    model_filename = "ppo_5215_tw_improved"
    ...
    try:
        ...
        df = yf.download('5215.TW', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ...
            ticker_info = yf.Ticker('5215.TW').info
    ...
    ai_muted = should_mute_ai_signal('5215.TW', threshold=52)
    ...
    try:
        ...
        profit_positive = evaluate_fundamentals_for_sell(yf, '5215.TW')['good']
    ...
    weight_calc = DynamicWeightCalculator('5215.TW')
    ...
    sentiment_result = calculate_sentiment_score('5215.TW', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            ...
            fundamentals = evaluate_fundamentals_for_sell(yf, '5215.TW')
        ...
        _growth = calculate_growth_score_adjustment(yf, '5215.TW')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"   股票: {result['symbol']} (科嘉-KY)")
        ...
        print(f"   {get_model_accuracy_display('5215')}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 5215 科嘉-KY 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('5215', '科嘉-KY', max_results=5)