"""台股 2892 (第一金) — XGBoost 85.05% / PPO 84.88%  ⭐ 雙高準確度"""
...
CODE='2892';TICKER='2892.TW';NAME='第一金';XGB_ACC=85.05;PPO_ACC=84.88
...
def get_trading_signal():
    print(f"🤖 {CODE}({NAME}) ⭐雙高準確度 | {get_model_accuracy_display(CODE)} | {datetime.now().strftime('%Y-%m-%d %H:%M')}");print("="*80)
    ...
    print("\n"+"="*80+f"\n🧠XGBoost({XGB_ACC:.2f}%) + 🤖PPO({PPO_ACC:.2f}%)  ⭐雙模型高度一致\n"+"="*80)
    xgb_prob=None
    try:
