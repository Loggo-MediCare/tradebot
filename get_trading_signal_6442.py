...
MODEL_FILE = 'ppo_6442_tw_improved'
...
def get_trading_signal():
    ...
    print(f"🤖 {TICKER} (光聖) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    print(f"當前價格: NT${current_price:.2f}  ({latest_date})")


    print(f"RSI: {rsi:.2f}  {'[超買]' if rsi > 70 else '[超賣]' if rsi < 30 else '[中性]'}")


    print(f"MACD: {macd:.4f}  {'[金叉]' if macd > macd_signal else '[死叉]'}")


    print(f"均線: SMA10={sma_10:.2f}, SMA30={sma_30:.2f}  {'[多頭]' if sma_10 > sma_30 else '[空頭]'}")


    print(f"布林帶位置: {bb_pos:.1f}%")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 6442 光聖 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('6442', '光聖', max_results=5)