...
class SignalFormatter:
    ...
    def print_signal_box(signal_type, strength, score, recommendations):
        emoji_map = {"BUY": "🟢", "SELL": "🔴", "HOLD": "🟡", "ADD": "🔥", "WAIT": "⏸️"}
...
class AddPositionChecker:
    ...
    def check_volume_healthy(self, volume_ratio):
    ...
    def print_summary(self):
        ...
        detail_keys = {'ma50_rising': 'ma50', 'structure_complete': 'structure', 'volume_healthy': 'volume'}
        for key, label in condition_labels.items():
            detail = self.details.get(detail_keys[key], {})
...
def get_trading_signal():
    ...
    fmt.print_header("🤖 台股 2417 AI 交易信号生成器 V2")
    print(f"   📅 生成時間: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"   📊 模型準確度: {get_model_accuracy_display('2417.TW')}")

    # ========== 載入多模型 (Ensemble) ==========
    base = r"C:\Users\Silvi\Projects\trading-bot"
    model_configs = [
        ('PPO', PPO, f"{base}\\ppo_2417_tw_improved"),
        ('A2C', A2C, f"{base}\\a2c_2417_tw_improved"),
        ('SAC', SAC, f"{base}\\sac_2417_tw_improved"),
    ]
    loaded_models = []
    ...
    try:
        ...
        df = yf.download('2417.TW', period='90d', progress=False, auto_adjust=True)
        if df.empty:
            print("   ❌ 無法獲取數據")
            return None
        ...
        num_analysts = 0
        try:
            ticker_info = yf.Ticker('2417.TW').info
    ...
    sentiment_result = calculate_sentiment_score('2417.TW', verbose=False)
    ...
    # ── Tavily 即時新聞 ─────────────────────────────────────────────────────
    print("\n" + "=" * 80)
    print("🌐 2417.TW (2417.TW) 即時新聞  (Tavily REST API)")
    print("=" * 80)
    print_tavily_news('2417.TW', '2417.TW', max_results=5)
    ...
    # ========== 集成預測 (Ensemble) ==========
    action_values = []
    fmt.print_section("多模型集成預測 (Ensemble)")
    for name, m in loaded_models:
        try:
            a, _ = m.predict(obs, deterministic=True)
            av = float(a[0]) if isinstance(a, np.ndarray) else float(a)
            action_values.append(av)
            direction = "買入 🟢" if av > 0.1 else "賣出 🔴" if av < -0.1 else "持有 🟡"
            print(f"   {name:<4}  動作值: {av:+.4f}  →  {direction}")
        except Exception as e:
            print(f"   {name} 預測失敗: {e}")
    ...
    action_value = float(np.mean(action_values))
    _ppo_roi, _bh_roi = calculate_ppo_backtest_roi(
        next((m for m in models if hasattr(m, "policy")), None), df)
    print(f"   {'─'*40}")
    print(f"   集成平均值: {action_value:+.4f}  ({len(action_values)} 個模型)")

    weight_calc = DynamicWeightCalculator('2417.TW')
    ...
    if action_value > 0.1:
        ...
        buy_score, signal_override, buy_reasons, buy_warnings, buy_metadata, sentiment_result = \
            calculate_enhanced_buy_score_with_sentiment(
                ...
                ai_action=action_value, buy_weights=buy_weights, symbol='2417.TW'
            )
        ...
        if ma50_adj < 0:
            ...
            reasons.append("MA50趨勢向下")
        elif ma50_adj > 0:
            sell_score = max(0, sell_score - abs(ma50_adj) * 0.5)
...
if __name__ == "__main__":
    ...
    if result:
        ...
        try:
            import yfinance as yf
            chart_df = yf.Ticker("2417.TW").history(period="6mo")
            chart_df.columns = [c.lower() for c in chart_df.columns]
            chart_path = "2417_chart.png"
            plot_candlestick(chart_df, "2417.TW", save_path=chart_path)
        except Exception as e:
            print(f"   圖表生成失敗: {e}")
