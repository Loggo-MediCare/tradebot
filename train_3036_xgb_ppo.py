...
CODE = '3036'; TICKER = '3036.TW'; NAME = '文曄'
...
if df.empty or len(df) < 50: print(f"[SKIP] {len(df)} rows"); sys.exit(1)
...
df=build(df); dc=df.dropna(subset=FEAT+['target'])
...
joblib.dump(xm,'xgb_3036_tw_model.pkl')
json.dump({'symbol':CODE,'ticker':TICKER,'model_type':'XGBoost',
          ...
          open('model_accuracy_3036_TW.json','w',encoding='utf-8'),indent=2)
json.dump({'symbol':CODE,'ticker':TICKER,'model_type':'XGBoost',
           'validation_accuracy':round(xgb_acc*100,2),'backtest_accuracy':round(xgb_acc*100,2),
           'last_updated':datetime.now().strftime('%Y-%m-%d %H:%M:%S')},
          open('model_accuracy_3036.json','w',encoding='utf-8'),indent=2)
...
pm.learn(total_timesteps=100000); pm.save('ppo_3036_tw_improved')
...
json.dump({'symbol':CODE,'ticker':TICKER,'model_type':'PPO',
           ...
           'last_updated':datetime.now().strftime('%Y-%m-%d %H:%M:%S')},
          open('model_accuracy_3036_TW_ppo.json','w',encoding='utf-8'),indent=2)
