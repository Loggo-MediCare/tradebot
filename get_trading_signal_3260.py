...
def get_trading_signal():
    print("=" * 80)
    print("🤖 台股 3260 (威剛) AI 交易信号生成器")
    print("=" * 80)
    print(f"生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    accuracy_display = get_model_accuracy_display('3260.TWO')
    if accuracy_display:
        print(f"模型準確度: {accuracy_display}")
    print("=" * 80)

    model_path = r"C:\Users\Silvi\Projects\trading-bot\ppo_3260_two_improved"
    print(f"\n📦 加载 AI 模型: {model_path}")
    try:
    ...
    try:
        ...
        df = yf.download('3260.TWO', period='2y', progress=False, auto_adjust=True)
        if df.empty:
            print("❌ 无法获取数据")
            return None
        ...
        peg_ratio = None
        try:
            ticker_info = yf.Ticker('3260.TWO').info
    ...
    weight_calc = DynamicWeightCalculator('3260.TWO')
    ...
    sentiment_result = calculate_sentiment_score('3260.TWO', verbose=False)
    ...
    # ── Tavily 即時新聞 ─────────────────────────────────────────────────────
    print("\n" + "=" * 80)
    print("🌐 威剛 (3260.TWO) 即時新聞  (Tavily REST API)")
    print("=" * 80)
    print_tavily_news('3260.TWO', '威剛', max_results=5)
    ...
    if action_value > 0.1:
        ...
        buy_score, signal_override, buy_reasons, buy_warnings, buy_metadata, sentiment_result = \
            calculate_enhanced_buy_score_with_sentiment(
                ...
                ai_action=action_value, buy_weights=buy_weights, symbol='3260.TWO'
            )
...
if __name__ == "__main__":
    ...
    if result:
        ...
        try:
            import yfinance as yf
            chart_df = yf.Ticker("3260.TWO").history(period="6mo")
            chart_df.columns = [c.lower() for c in chart_df.columns]
            chart_path = "3260_chart.png"
            plot_candlestick(chart_df, "3260.TWO", save_path=chart_path)
        except Exception as e:
            print(f"   圖表生成失敗: {e}")

        print(f"\n📱 快速摘要:")
        print(f"   股票: {result['symbol']} (威剛)")
        print(f"   日期: {result['date']}")
        print(f"   价格: NT${result['current_price']:.2f}")
        print(f"   信号: {result['signal']}")
        if result['strength'] > 0:
            print(f"   强度: {result['strength']:.2f}")
        print(f"   {get_model_accuracy_display('3260.TWO')}")
    else:
        print("\n❌ 信号生成失败")
