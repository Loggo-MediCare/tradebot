...
class ImprovedTradingEnv(gym.Env):
    ...
    def reset(self,seed=None,options=None):
        ...
        self.shares_held=0; self.total_profit=0; self.total_trades=0; self.last_total_value=self.initial_balance
    ...
    def _get_observation(self):
        ...
        return np.array([float(self.shares_held),float(self.balance),cp,
            ...
            float(self.total_profit),float(sr),float(cr)],dtype=np.float32)
    def step(self,action):
        if isinstance(action,np.ndarray): action=float(action[0])
        action=np.clip(action,-1.0,1.0); cp=float(self.df.iloc[self.current_step]['close']); traded=False
        if action>0.15:
        ...
        self.current_step+=1; done=self.current_step>=len(self.df)-1; tv=self.balance+self.shares_held*cp
        self.total_profit=tv-self.initial_balance; vc=tv-self.last_total_value; reward=vc/self.initial_balance
...
def train_stock(ticker, symbol, name):
    ...
    try:
        ...
        if len(df_clean) < 50: print("  ❌ 數據不足"); return False
        ...
        test_env=ImprovedTradingEnv(df_clean); obs,_=test_env.reset(); correct=0; total=0
        for _ in range(len(df_clean)-1):
            action,_=model.predict(obs,deterministic=True); obs,reward,done,_,_=test_env.step(action)
        ...
        with open(f'model_accuracy_{symbol}.json','w',encoding='utf-8') as f:
            json.dump({'symbol':symbol,'model_type':'PPO','training_accuracy':float(acc),'validation_accuracy':float(acc),'backtest_accuracy':float(acc),'backtest_return':float(ret),'win_rate':float(acc),'sharpe_ratio':None,'total_signals':int(total),'correct_signals':int(correct),'live_accuracy':None,'last_updated':datetime.now().strftime('%Y-%m-%d %H:%M:%S'),'history':[]},f,ensure_ascii=False,indent=2)
...
if __name__ == "__main__":
    print("="*70)
    print("訓練 6510精測 / 8299群聯(更正) / 2308台達電")
