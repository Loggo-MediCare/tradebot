...
def money_flow_strength(df):
    ...
    strong_money = ((capital_inflow or (up_down_ratio > 1.3 and volume_ratio > 1.0)) and obv_bullish)


    details = {


        'up_volume_30d': int(up_vol), 'down_volume_30d': int(down_vol),


        'up_down_ratio': round(up_down_ratio, 2),


        'net_money_flow_30d': net_money_flow_30d, 'net_money_flow_5d': net_money_flow_5d,


        'capital_inflow': capital_inflow, 'obv_bullish': obv_bullish,
...
def detect_memory_cycle_phase(df):
    ...
    early_upcycle = (price > ma50.iloc[-1] and ma50.iloc[-1] > ma200.iloc[-1] and ma50.diff().iloc[-1] > 0)


    late_cycle = (price > ma50.iloc[-1] * 1.25 and ma50.diff().iloc[-1] < ma50.diff().iloc[-5] if len(df) >= 5 else False)


    if early_upcycle:


        return "EARLY_UPCYCLE"
...
def trend_acceleration(df):
    ...
    slope10 = df['sma_10'].diff().iloc[-1]


    slope30 = df['sma_30'].diff().iloc[-1]


    return slope10 > slope30 and slope10 > 0 and df['close'].iloc[-1] > df['sma_10'].iloc[-1]
...
def explosive_trend_filter(df):
    ...
    explosive = strong_money and accelerating and cycle_phase == "EARLY_UPCYCLE"


    return {


        "explosive": explosive, "volume_ratio": vol_ratio, "cycle_phase": cycle_phase,


        "money_inflow": strong_money, "trend_accelerating": accelerating,


        "flow_details": flow_details, 'explosion_detected': explosive
...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('6510')


    print(f"🤖 6510 (精測電子) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")


    print("=" * 80)


    model_path = r"ppo_6510_improved"
    ...
    try:
        ...
        df = yf.download('6510.TWO', period='300d', progress=False, auto_adjust=True)
        ...
        try:
            ...
            ticker_info = yf.Ticker('6510.TWO').info
    ...
    ai_muted = should_mute_ai_signal('6510.TWO', threshold=52)
    ...
    macd_signal_val = float(latest_data['macd_signal'])
    ...
    print(f"MACD Signal:     {macd_signal_val:.4f}  {'[金叉]' if macd > macd_signal_val else '[死叉]'}")
    ...
    try:
        ...
        profit_positive = evaluate_fundamentals_for_sell(yf, '6510.TWO')['good']
    ...
    weight_calc = DynamicWeightCalculator('6510.TWO')
    ...
    sentiment_result = calculate_sentiment_score('6510.TWO', verbose=True)


    if not (sentiment_result and sentiment_result['news_count'] > 0):
    ...
    if triangle_detected:
        ...
        triangle_status_text = {"BREAK_UP": "向上突破", "BREAK_DOWN": "向下跌破", "CONSOLIDATING": "收斂中，等待方向"}.get(triangle_status, "型態成立")
    ...
    if action_value > 0.1:
        ...
        buy_score, signal_override, buy_reasons, buy_warnings, buy_metadata, sentiment_result = calculate_enhanced_buy_score_with_sentiment(
            ...
            rsi=rsi, macd=macd, macd_signal=macd_signal_val, sma_10=sma_10, sma_30=sma_30,


            current_price=current_price, bb_upper=bb_upper, bb_lower=bb_lower,


            volume_ratio=volume_ratio, ai_action=action_value, buy_weights=buy_weights,


            symbol='6510.TWO', price_change_pct=price_change_pct, rsi_prev=rsi_prev,
        ...
        if explosion["explosive"]:
            ...
            buy_reasons.append("🚀 爆发行情确认: 主升段初期")


            buy_reasons.append("资金强势流入 (OBV > MA20)")


            buy_reasons.append("趋势加速 (10日斜率 > 30日斜率)")
        ...
        if triangle_detected:
            ...
            if triangle_status == "BREAK_UP":
                ...
                buy_score += 10; buy_reasons.append("三角收斂向上突破")


            elif triangle_status == "CONSOLIDATING":


                buy_score += 4; buy_reasons.append("三角收斂整理中，關注突破")
        ...
        if breakout_signal['detected']:
            ...
            if breakout_signal['type'] == 'TRUE_BREAKOUT':
                ...
                buy_score += 15; buy_reasons.append(breakout_signal['signal_text'])


            elif breakout_signal['type'] == 'FALSE_BREAKOUT':


                buy_score -= 10; buy_warnings.append(breakout_signal['signal_text'])
        ...
        if pattern_signal['patterns']:
            ...
            if pattern_signal['score_adjustment'] > 0:
                ...
                buy_score += pattern_signal['score_adjustment']; buy_reasons.append(f"型態: {pattern_signal['signal_text']}")
        ...
        if volume_signal['surge'] and volume_signal['surge']['detected']:
            ...
            if volume_signal['surge']['type'] == 'SURGE_UP':
                ...
                buy_score += 15; buy_reasons.append(volume_signal['surge']['signal_text'])
        ...
        if explosion["explosive"]:
            ...
            signal = "强势持有 (HOLD - TREND EXPLOSION)"; signal_emoji = "🚀"


            strength = abs(action_value)


            suggested_price_low = suggested_price_high = current_price
        ...
        is_macd_bearish = macd < macd_signal_val
        ...
        if ma50_slope_adjustment < 0:
            ...
            sell_score += abs(ma50_slope_adjustment); reasons.append(f"MA50趨勢向下 ({ma50_slope_info['slope_pct']:.2f}%)")


        elif ma50_slope_adjustment > 0:


            sell_score = max(0, sell_score - int(abs(ma50_slope_adjustment) * 0.5))
        ...
        if target_price is not None and current_price > 0:
            ...
            if current_price > target_price and recommendation_mean and recommendation_mean <= 2.5:
                ...
                sell_score += sell_weights['target_below']; reasons.append(f"⚠️ 股價過高! 超越目標價且評級差")


            elif target_upside_mean < -10:


                sell_score += sell_weights['target_below']; reasons.append(f"📉 目標價低於現價 {target_upside_mean:.1f}%")


            elif target_upside_mean < 5:


                sell_score += sell_weights['target_near']; reasons.append(f"⚠️ 上漲空間有限 (僅{target_upside_mean:.1f}%)")


        if is_macd_bearish: sell_score += sell_weights['macd_bearish']; reasons.append("MACD 死叉,趋势转弱")


        if is_trending_down: sell_score += sell_weights['ma_bearish']; reasons.append("短期均线下穿长期均线")


        if bb_position > 90: sell_score += sell_weights['bb_upper']; reasons.append(f"价格接近布林带上轨 ({bb_position:.1f}%)")


        elif bb_position > 80: sell_score += sell_weights['bb_high']; reasons.append("价格偏高,接近布林带上轨")
        ...
        if triangle_detected:
            ...
            if triangle_status == "BREAK_DOWN": sell_score += 15; reasons.append("三角收斂向下跌破")


            elif triangle_status == "BREAK_UP": reasons.append("三角收斂向上突破，賣壓訊號需保守解讀")


            elif triangle_status == "CONSOLIDATING": reasons.append("三角收斂整理中，等待方向確認")


        if volume_ratio > 2.5 and rsi < 80 and is_strong_trend:


            sell_score = 0; reasons.clear()


            reasons += [f"🚀 超强势突破信号!", f"极度放量({volume_ratio:.1f}x) + MACD金叉 + 均线多头", f"💡 建议: 继续持有100%"]


        elif is_strong_trend and rsi > 70:


            sell_score = int(sell_score * 0.2); reasons.clear()


            reasons += [f"⚠️  RSI超买但符合强势股特征", f"MACD金叉 + 均线多头 + 放量({volume_ratio:.1f}x)", f"💡 建议: 持有或小幅减仓10-20%"]


        elif volume_ratio > 3.0 and rsi > 80:


            sell_score += 30; reasons.append(f"⚠️  超级放量({volume_ratio:.1f}x) + RSI严重超买({rsi:.1f})")


        elif volume_ratio > 2.0 and rsi > 70 and (not is_strong_trend):


            sell_score += 20; reasons.append(f"高位放量({volume_ratio:.1f}x)但趋势转弱")


        elif volume_ratio < 0.5 and current_price > sma_10:


            sell_score += 15; reasons.append(f"价涨量缩(量比{volume_ratio:.1f}x)，上涨乏力")


        if bb_position < 35 and sell_score > 0:


            fundamentals = evaluate_fundamentals_for_sell(yf, '6510.TWO')


            if fundamentals['good']:


                sell_score = 0; reasons.clear()


                reasons += [f"🎯 优质公司超跌! 布林带{bb_position:.1f}%", "💎 建议: 不要追杀,这是价值投资机会!"]


            elif fundamentals['bad']:


                reasons.clear(); reasons += ["⚠️ 公司亏损,超跌合理", "💡 建议: 可以卖出止损"]


            else:


                sell_score = int(sell_score * 0.3); reasons.clear()


                reasons += [f"⚠️  股价接近支撑位(布林带{bb_position:.1f}%)", "💡 建议: 暂不追杀,等待反弹或进一步确认"]


        # 🌱 基本面成長保護: Revenue Growth > 33% 或 EPS Growth > 100% 各降低賣出強度 8 分


        _growth = calculate_growth_score_adjustment(yf, '6510.TWO')
        ...
        if reasons:
            ...
            print(f"\n   📌 {'持有理由:' if sell_score == 0 else '卖出理由:'}")
    ...
    return {
        ...
        'date': latest_date, 'symbol': '6510.TWO', 'current_price': current_price,


        'signal': signal, 'action_value': action_value,
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"   {get_model_accuracy_display('6510')}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 6510 精測電子 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('6510', '精測電子', max_results=5)