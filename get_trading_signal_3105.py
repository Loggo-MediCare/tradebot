...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('3105.TWO')
    ...
    try:
        ...
        with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'model_accuracy_3105.json'), 'r') as _f:


            _acc_data = _json.load(_f)


        _ret = _acc_data.get('backtest_return')


        _ret_str = f" | 回測回報: +{_ret:.1f}%" if _ret is not None else ""
    ...
    print(f"模型準確度: {accuracy_display}{_ret_str}")
    ...
    model_filename = "ppo_3105_two_improved"
    ...
    try:
        ...
        df = yf.download('3105.TWO', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ...
            ticker_info = yf.Ticker('3105.TWO').info
    ...
    ai_muted = should_mute_ai_signal('3105.TWO', threshold=45)


    if ai_muted:


        print("⚠️  AI模型準確度低於45%，已靜音AI交易動作（action=0）")
    ...
    try:
        ...
        profit_positive = evaluate_fundamentals_for_sell(yf, '3105.TWO')['good']
    ...
    weight_calc = DynamicWeightCalculator('3105.TWO')
    ...
    sentiment_result = calculate_sentiment_score('3105.TWO', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            ...
            fundamentals = evaluate_fundamentals_for_sell(yf, '3105.TWO')
        ...
        _growth = calculate_growth_score_adjustment(yf, '3105.TWO')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"   {get_model_accuracy_display('3105.TWO')}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 3105 3105 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('3105', '3105', max_results=5)