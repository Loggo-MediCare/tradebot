...
def calculate_bullish_combo_score(
    rsi, macd, macd_signal, sma_10, sma_30, volume_ratio, bb_position, current_price=None,
    price_change_pct=None, candle_direction=None
):
    """
    计算多头组合分数
    ==================
    ...
    # 3. 成交量评分 (最高 30分) + 方向校正
    # 核心修正: 放量必须结合价格方向，避免“放量下跌”被误判为利多
    volume_score = 0
    if volume_ratio > 3.0:
        volume_score = 30
        volume_text = f"爆量({volume_ratio:.1f}x)"
    elif volume_ratio > 2.0:
        volume_score = 28
        volume_text = f"大幅放量({volume_ratio:.1f}x)"
    elif volume_ratio > 1.5:
        volume_score = 25
        volume_text = f"放量突破({volume_ratio:.1f}x)"
    elif volume_ratio > 1.2:
        volume_score = 18
        volume_text = f"温和放量({volume_ratio:.1f}x)"
    elif volume_ratio > 0.8:
        volume_score = 5
        volume_text = "成交量正常"
    elif volume_ratio > 0.5:
        volume_score = -20
        volume_text = f"⚠️  成交量不足({volume_ratio:.1f}x)"
    else:
        volume_score = -35
        volume_text = f"⚠️  成交量严重不足({volume_ratio:.1f}x)"
    ...
    combo_score += volume_score
    reasons.append(volume_text)
...
def calculate_enhanced_buy_score(rsi, macd, macd_signal, sma_10, sma_30, current_price,
                                  ...
                                  sentiment_score=0.0, price_change_pct=None,
                                  rsi_prev=None, candle_direction=None,
    ...
    combo_score, combo_reasons = calculate_bullish_combo_score(
        ...
        price_change_pct=price_change_pct, candle_direction=candle_direction
    )
    ...
    if macd < macd_signal:
        ...
        # RSI 评分 (改进: 強勢股與題材股不扣分，並增加 RSI 趨勢二次確認)
        rsi_change = (rsi - rsi_prev) if rsi_prev is not None else None
        if rsi_change is not None:
            if rsi > 70 and rsi_change < -2.0 and (price_change_pct is not None and price_change_pct < 0):
                base_score -= 12
                warnings.append(f"⚠️  RSI高檔回落({rsi_change:+.1f})且價格下跌，短線轉弱")
            elif rsi < 35 and rsi_change > 1.5:
                base_score += 8
                base_reasons.append(f"✅ RSI低檔回升({rsi_change:+.1f})，跌深反彈機率上升")
        ...
        if (is_strong or is_theme_stock) and rsi > 65:
            ...
            base_reasons.append(
                f"✅ RSI超卖 ({rsi:.1f} < {rsi_oversold_threshold}, +{buy_weights.get('rsi_oversold', 30)}分)"
            )
        elif rsi < rsi_low_threshold and rsi_low_threshold > rsi_oversold_threshold:
            base_score += buy_weights.get('rsi_low', 15)
            base_reasons.append(
                f"✅ RSI偏低 ({rsi:.1f} < {rsi_low_threshold}, +{buy_weights.get('rsi_low', 15)}分)"
            )
        elif rsi > 92 and not is_strong and not is_theme_stock:
            # 弱势股 + 极高 RSI = 真的危险
            base_score -= 30
            warnings.append(f"⚠️  RSI极度超买({rsi:.1f})且无强势特征,回调风险极高")
        elif 70 < rsi <= 92 and not is_strong and not is_theme_stock:
            # 普通股票 RSI 70-92 = 仅提示，不扣分
            warnings.append(f"ℹ️  RSI偏高({rsi:.1f})，目前不作扣分处理")
        elif 65 < rsi <= 70 and not is_strong and not is_theme_stock:
            # 普通股票 RSI 65-70 = 仅提示，不扣分
            warnings.append(f"ℹ️  RSI略高({rsi:.1f})，目前不作扣分处理")
    ...
    # 合并理由/警告
    # 规则: 带风险提示前缀的文本不进入「买入理由」，统一放入 warnings。
    all_reasons = []
    if is_strong:
    ...
    for reason in combo_reasons:
        ...
        if reason_text.startswith("⚠️") or reason_text.startswith("ℹ️"):
