"""
PLTR (Palantir) XGBoost 優化訓練 - 與 PPO (63.5%) 比較
"""
import os
...
TICKER = 'PLTR'
print(f"優化訓練 {TICKER} (Palantir) XGBoost 模型")
print(f"目標: 超越 PPO 模型準確度 63.5%")
print("=" * 80)

try:
    # 下載更多數據
    df = yf.download(TICKER, start='2020-01-01', end='2026-12-31', progress=False)
    ...
    print("模型比較")
    print("=" * 80)
    print(f"PPO 模型準確度:     63.5%")
    print(f"XGBoost 模型準確度: {test_acc*100:.2f}%")

    if test_acc > 0.635:
        improvement = (test_acc - 0.635) * 100
        print(f"\n✅ XGBoost 勝出! 提升 {improvement:.2f} 百分點")
        winner = "XGBoost"
    elif test_acc < 0.635:
        decline = (0.635 - test_acc) * 100
        print(f"\n❌ PPO 更好, XGBoost 低 {decline:.2f} 百分點")
        winner = "PPO"
    else:
        print(f"\n🤝 兩者準確度相同")
        winner = "TIE"
    ...
    # 總是保存模型以供比較
    joblib.dump(model, 'xgb_pltr_model.pkl')
    print(f"\n✅ XGBoost 模型已保存: xgb_pltr_model.pkl")
    ...
    accuracy_data = {
        ...
        'notes': f'Optimized with {best_threshold*100:.1f}% gain threshold. Compared with PPO (63.5%). Winner: {winner}'
    }

    with open('model_accuracy_PLTR_xgboost.json', 'w', encoding='utf-8') as f:
