"""
美股 AVAV (AeroVironment) AI 交易信号生成器
======================================
 信号: 卖出 (SELL) 20251222
   强度: 1.00 / 1.00
   建议卖出比例: 100%
...
def get_trading_signal():
    """生成今日交易信号"""
    print("=" * 80)
    print("🤖 美股 AVAV (AeroVironment) AI 交易信号生成器")
    print("=" * 80)
   # print(f("生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    # 顯示AI模型準確度
    accuracy_display = get_model_accuracy_display('AVAV')
    if accuracy_display:
        print(f"模型準確度: {accuracy_display}")
    print("=" * 80)

    # 1. 加载模型
    model_path = r"C:\Users\Silvi\Projects\trading-bot\ppo_avav_improved"
    print(f"\n📦 加载 AI 模型: {model_path}")
    ...
    try:
        ...
        df = yf.download('AVAV', period='90d', progress=False, auto_adjust=True)
        ...
        try:
            ticker_info = yf.Ticker('AVAV').info
    ...
    # 7. 初始化动态权重计算器
    weight_calc = DynamicWeightCalculator('AVAV')
    ...
    sentiment_result = calculate_sentiment_score('AVAV', verbose=False)
    ...
    # ── Tavily 即時新聞 ─────────────────────────────────────────────────────
    print("\n" + "=" * 80)
    print("🌐 AeroVironment (AVAV) 即時新聞  (Tavily REST API)")
    print("=" * 80)
    print_tavily_news('AVAV', 'AeroVironment', max_results=5)
...
if __name__ == "__main__":
    ...
    if result:
        ...
        try:
            import yfinance as yf
            chart_df = yf.Ticker("AVAV").history(period="6mo")
            chart_df.columns = [c.lower() for c in chart_df.columns]
            chart_path = "AVAV_chart.png"
            plot_candlestick(chart_df, "AVAV", save_path=chart_path)
        except Exception as e:
            print(f"   圖表生成失敗: {e}")

        print(f"\n📱 快速摘要:")
        print(f"   股票: {result['symbol']} (AeroVironment)")
        ...
        # 顯示AI模型準確度摘要
        print(f"   {get_model_accuracy_display('AVAV')}")
    else:
        print("\n❌ 信号生成失败")
