...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('8110')


    print(f"🤖 8110 (華東) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    model_filename = "ppo_8110_tw_improved"
    ...
    try:
        ...
        df = yf.download('8110.TW', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ...
            ticker_info = yf.Ticker('8110.TW').info
    ...
    ai_muted = should_mute_ai_signal('8110.TW', threshold=52)
    ...
    try:
        ...
        profit_positive = evaluate_fundamentals_for_sell(yf, '8110.TW')['good']
    ...
    weight_calc = DynamicWeightCalculator('8110.TW')
    ...
    sentiment_result = calculate_sentiment_score('8110.TW', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if not skip_sell_scoring:
            ...
            # (Copy the sell logic from 6443, but change ticker to 8110.TW)
            ...
            _growth = calculate_growth_score_adjustment(yf, '8110.TW')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"   {get_model_accuracy_display('8110')}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 8110 華東 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('8110', '華東', max_results=5)