"""Train XGBoost + PPO for AAPL NVDA AMZN MSFT ORCL BA COST SPY"""
...
STOCKS = [
    ('AAPL', '蘋果',   '2015-01-01'),   # add XGBoost only (PPO exists)
    ('NVDA', '輝達',   '2015-01-01'),   # retrain both
    ('AMZN', '亞馬遜', '2015-01-01'),   # retrain both
    ('MSFT', '微軟',   '2015-01-01'),   # retrain both
    ('ORCL', '甲骨文', '2015-01-01'),   # retrain both
    ('BA',   '波音',   '2015-01-01'),   # full train
    ('COST', '好市多', '2015-01-01'),   # full train
    ('SPY',  '標普500','2015-01-01'),   # full train
]
...
def train_stock(ticker, name, start, idx):
    sl = ticker.lower()
    need_xgb = not os.path.exists(f'xgb_{sl}_model.pkl')
    need_ppo = not os.path.exists(f'ppo_{sl}_improved.zip')
    # Always retrain for fresh data
    need_xgb = need_ppo = True

    print(f"\n{'='*60}")
    print(f"  [{idx}/{len(STOCKS)}] {ticker} {name}")
    print(f"{'='*60}")
    ...
    # Buy & Hold baseline
    bh_roi = (te['close'].iloc[-1] - te['close'].iloc[0]) / te['close'].iloc[0] * 100
    ...
    print(f"  [B&Hold]  {bh_roi:+.1f}% (test period baseline)")

    winner = 'XGBoost' if xgb_acc >= ppo_acc else 'PPO'
    return {'ticker': ticker, 'name': name,
            'xgb_acc': round(xgb_acc*100,2), 'ppo_acc': round(ppo_acc*100,2),
            'bh_roi': round(bh_roi,1), 'winner': winner}


if __name__ == '__main__':
    print("=" * 60)
    print(f"  Training: AAPL NVDA AMZN MSFT ORCL BA COST SPY")
    print(f"  Started:  {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 60)

    results, failed = [], []
    for idx, (ticker, name, start) in enumerate(STOCKS, 1):
        try:
            r = train_stock(ticker, name, start, idx)
            if r: results.append(r)
            else: failed.append(ticker)
        except Exception as e:
            print(f"  [ERROR] {ticker}: {e}"); failed.append(ticker)

    print("\n" + "=" * 68)
    print("  ACCURACY COMPARISON TABLE")
    print("=" * 68)
    print(f"  {'Ticker':<6} {'Name':<8} {'XGBoost':>9} {'PPO':>7} {'B&Hold':>8} {'Winner'}")
    print("  " + "-"*62)
    for r in sorted(results, key=lambda x: max(x['xgb_acc'],x['ppo_acc']), reverse=True):
        ...
        print(f"  {r['ticker']:<6} {r['name']:<8} "
              f"{r['xgb_acc']:>8.2f}% {r['ppo_acc']:>6.2f}% "
              f"{r['bh_roi']:>7.1f}% {r['winner']} {star}")
    print("  " + "-"*62)
