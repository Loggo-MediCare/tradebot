"""
美股 PLTR (Palantir Technologies) AI 交易信号生成器
======================================
...
def get_trading_signal():
    """生成今日交易信号"""
    # 壓縮標題區塊 - 從6行減少到2行
    accuracy_display = get_model_accuracy_display('PLTR')
    print(f"🤖 PLTR (Palantir Technologies) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print("=" * 80)

    # 1. 加载模型
    model_path = r"C:\Users\Silvi\Projects\trading-bot\ppo_pltr_improved"
    print(f"\n📦 加载 AI 模型: {model_path}")
    ...
    try:
        ...
        df = yf.download('PLTR', period='90d', progress=False, auto_adjust=True)
        ...
        try:
            ticker_info = yf.Ticker('PLTR').info
    ...
    ai_muted = should_mute_ai_signal('PLTR', threshold=52)
    if ai_muted:
    ...
    try:
        profit_positive = evaluate_fundamentals_for_sell(yf, 'PLTR')['good']
    except Exception:
    ...
    # 7. 初始化动态权重计算器
    weight_calc = DynamicWeightCalculator('PLTR')
    ...
    sentiment_result = calculate_sentiment_score('PLTR', verbose=False)
    ...
    if action_value > 0.1:
        ...
        _growth = calculate_growth_score_adjustment(yf, 'PLTR')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        try:
            import yfinance as yf
            chart_df = yf.Ticker("PLTR").history(period="6mo")
            chart_df.columns = [c.lower() for c in chart_df.columns]
            chart_path = "PLTR_chart.png"
            plot_candlestick(chart_df, "PLTR", save_path=chart_path)
        except Exception as e:
            print(f"   圖表生成失敗: {e}")

        print(f"\n📱 快速摘要:")
        print(f"   股票: {result['symbol']} (Palantir Technologies)")
        ...
        # 顯示AI模型準確度摘要
        print(f"   {get_model_accuracy_display('PLTR')}")
    else:
        print("\n❌ 信号生成失败")
