...
class ImprovedTradingEnv(gym.Env):
    ...
    def step(self, action):
        ...
        trade_incentive = 0.0
        if abs(action) > 0.1:
            trade_incentive = 0.01
        cash_penalty = 0.0
        if self.balance > old_total_value * 0.9:
            cash_penalty = -0.005
...
# 2. 下载和处理数据
# ==========================================
def download_and_prepare_data(ticker='OUST', start_date='2015-01-01', end_date='2025-01-01'):
    print("=" * 70)
    print(f"下载 {ticker} 股票数据 (Ouster Inc)...")
...
if __name__ == "__main__":
    print("🚀 改进版 OUST (Ouster Inc) 交易 AI 训练系统")
    print("=" * 70)
    TICKER = 'OUST'
    ...
    print(f"目标股票: {TICKER} (Ouster Inc)")
