...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('2376')


    print(f"🤖 2376 (技嘉) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    model_filename = "ppo_2376_tw_improved"
    ...
    try:
        ...
        df = yf.download('2376.TW', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ...
            ticker_info = yf.Ticker('2376.TW').info
    ...
    ai_muted = should_mute_ai_signal('2376.TW', threshold=52)
    ...
    try:
        ...
        profit_positive = evaluate_fundamentals_for_sell(yf, '2376.TW')['good']
    ...
    weight_calc = DynamicWeightCalculator('2376.TW')
    ...
    sentiment_result = calculate_sentiment_score('2376.TW', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            ...
            fundamentals = evaluate_fundamentals_for_sell(yf, '2376.TW')
        ...
        _growth = calculate_growth_score_adjustment(yf, '2376.TW')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"   {get_model_accuracy_display('2376')}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 2376 技嘉 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('2376', '技嘉', max_results=5)