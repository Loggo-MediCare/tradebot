"""
改进版 台股 8996 高力 交易 AI 训练
=====================================
基于最佳模型 (AAPL Improved PPO) 客制化
改进点:
1. ✅ 充足训练数据 (2015-2026, 10年数据)
2. ✅ 改进的奖励函数 (鼓励交易)
3. ✅ 更长训练时间 (100,000 步)
4. ✅ 连续动作空间 (更灵活的买卖比例)
"""
...
# 2. 下载和处理数据
# ==========================================
def download_and_prepare_data(ticker='7717.TWO', start_date='2015-01-01', end_date='2025-01-01'):
    """
    🔥 改进 1: 下载更多年份的数据
    台股代码格式: 7717.TWO
    """
    ...
    try:
        ...
        # 避免 yfinance 在受限环境写缓存失败
        yf.set_tz_cache_location(r"C:\Users\Silvi\Projects\trading-bot\TMP")

        df = yf.download(ticker, start=start_date, end=end_date, progress=False)
        if df.empty:
            # 自动检测可用最早日期
            print("⚠️ 指定区间无数据，改用全量数据自动检测上市日期...")
            df = yf.download(ticker, period="max", progress=False)
            if df.empty:
                raise ValueError(f"无法下载 {ticker} 的数据")

            actual_start = pd.to_datetime(df.index.min()).date()
            actual_end = pd.to_datetime(df.index.max()).date()
            print(f"✅ 可用数据区间: {actual_start} 至 {actual_end}")
            ...
            if df.empty:
                raise ValueError(f"在可用区间内仍无数据，请检查起止日期")
        # 处理MultiIndex列
...
if __name__ == "__main__":
    print("🚀 改进版台股 8996 高力 交易 AI 训练系统")
    print("=" * 70)
    # 配置
    TICKER = '8996.TW'  # 台股 8996 高力
    START_DATE = '2015-01-01'  # 10年数据
    END_DATE = '2026-12-31'
    TRAIN_TEST_SPLIT = 0.8
    TOTAL_TIMESTEPS = 100000   # 数据充足,训练100k步
    print(f"目标股票: {TICKER}")
