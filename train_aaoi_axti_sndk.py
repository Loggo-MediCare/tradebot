"""Train AAOI + AXTI (new) + SNDK — all massive trend stocks"""
...
STOCKS = [
    ('AAOI', 'Applied Optoelectronics', '2015-01-01'),
    ('AXTI', 'AXT Inc',                 '2015-01-01'),
    ('SNDK', 'SanDisk',                 '2024-02-01'),  # re-listed Feb 2024
]
...
results=[]
for ticker, name, start in STOCKS:
    sl = ticker.lower()
    print(f"\n{'='*55}\n  {ticker} — {name}\n{'='*55}")
    df=yf.download(ticker,start=start,end=END_DATE,progress=False)
    if isinstance(df.columns,pd.MultiIndex): df.columns=df.columns.droplevel(1)
    if df.empty or len(df)<50: print(f"  [SKIP]"); continue
    ...
    joblib.dump(xm,f'xgb_{sl}_model.pkl')
    json.dump({'symbol':ticker,'model_type':'XGBoost','training_accuracy':round(accuracy_score(yt,xm.predict(Xt))*100,2),
               'validation_accuracy':round(xgb_acc*100,2),'backtest_accuracy':round(xgb_acc*100,2),
               'last_updated':datetime.now().strftime('%Y-%m-%d %H:%M:%S')},
              open(f'model_accuracy_{ticker}.json','w',encoding='utf-8'),indent=2)
    ...
    pm.learn(total_timesteps=STEPS); pm.save(f'ppo_{sl}_improved')
    ...
    json.dump({'symbol':ticker,'model_type':'PPO','validation_accuracy':round(ppo_acc*100,2),
               'backtest_accuracy':round(ppo_acc*100,2),'last_updated':datetime.now().strftime('%Y-%m-%d %H:%M:%S')},
              open(f'model_accuracy_{ticker}_ppo.json','w',encoding='utf-8'),indent=2)
    print(f"  [PPO]     {ppo_acc*100:.2f}%  |  Test B&Hold: {bh_test:+.1f}%")
    results.append({'t':ticker,'xgb':round(xgb_acc*100,2),'ppo':round(ppo_acc*100,2),'bh':round(bh,1),'bh_test':round(bh_test,1)})

print(f"\n{'='*62}\n  RESULTS (傻瓜 vs AI)\n{'='*62}")
print(f"  {'Ticker':<6} {'XGBoost':>9} {'PPO':>7} {'B&Hold全期':>11} {'B&Hold測試':>11}")
for r in results:
    print(f"  {r['t']:<6} {r['xgb']:>8.2f}% {r['ppo']:>6.2f}% {r['bh']:>10.1f}% {r['bh_test']:>10.1f}%")
