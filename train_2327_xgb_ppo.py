"""Retrain XGBoost + PPO for 2327 國巨 (2327.TW)"""
...
if df.empty or len(df) < 50:
    print(f"[SKIP] {len(df)} rows"); sys.exit(1)
...
joblib.dump(xm, 'xgb_2327_tw_model.pkl')
...
json.dump(acc_data, open('model_accuracy_2327_TW.json','w',encoding='utf-8'), indent=2)
json.dump(acc_data, open('model_accuracy_2327.json','w',encoding='utf-8'), indent=2)
...
json.dump({'symbol': CODE, 'ticker': TICKER, 'model_type': 'PPO',
           ...
           'last_updated': datetime.now().strftime('%Y-%m-%d %H:%M:%S')},
          open('model_accuracy_2327_TW_ppo.json','w',encoding='utf-8'), indent=2)
