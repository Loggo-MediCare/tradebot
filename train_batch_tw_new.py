"""Batch train XGBoost + PPO for new TW stocks"""
...
def train_stock(code, name, start_date):
    ...
    xgb_model = xgb.XGBClassifier(max_depth=5, learning_rate=0.05, n_estimators=200,
                                    min_child_weight=3, subsample=0.8, colsample_bytree=0.8,
                                    objective='binary:logistic', random_state=42, eval_metric='logloss')
    xgb_model.fit(Xt, yt)
    xgb_acc = accuracy_score(ye, xgb_model.predict(Xe))
    joblib.dump(xgb_model, f'xgb_{code}_tw_model.pkl')
    with open(f'model_accuracy_{code}_TW.json', 'w', encoding='utf-8') as f:
        json.dump({'symbol': code, 'ticker': ticker, 'model_type': 'XGBoost',
                   'training_accuracy': round(accuracy_score(yt, xgb_model.predict(Xt)) * 100, 2),
    ...
    ppo_model = PPO('MlpPolicy', env, verbose=0, learning_rate=0.0003,
                    n_steps=2048, batch_size=64, n_epochs=10, gamma=0.99, ent_coef=0.01)
    ppo_model.learn(total_timesteps=TOTAL_TIMESTEPS)
    ppo_model.save(f'ppo_{code}_tw_improved')
    ...
    with open(f'model_accuracy_{code}_TW_ppo.json', 'w', encoding='utf-8') as f:
        json.dump({'symbol': code, 'ticker': ticker, 'model_type': 'PPO',
                   ...
                   'last_updated': datetime.now().strftime('%Y-%m-%d %H:%M:%S')}, f, indent=2)
    print(f"  [PPO]     Accuracy: {ppo_acc*100:.2f}%")
...
if __name__ == '__main__':
    print("=" * 60)
    print("  BATCH TRAINING — TW Stocks (XGBoost + PPO)")
    ...
    print("=" * 62)
    print(f"  {'Code':<6} {'Name':<8} {'Ticker':<12} {'XGBoost':>9} {'PPO':>7} {'Winner'}")
    print("  " + "-" * 56)
    for r in sorted(results, key=lambda x: max(x['xgb_acc'], x['ppo_acc']), reverse=True):
        ...
        print(f"  {r['code']:<6} {r['name']:<8} {r['ticker']:<12} {r['xgb_acc']:>8.2f}% {r['ppo_acc']:>6.2f}% {r['winner']} {star}")
    print("  " + "-" * 56)
