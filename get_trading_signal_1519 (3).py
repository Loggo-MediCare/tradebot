...
def get_trading_signal():
    """生成今日交易信号"""
    # 壓縮標題區塊 - 從6行減少到2行
    accuracy_display = get_model_accuracy_display('1519')
    print(f"🤖 1519 (華城) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print("=" * 80)

    # 1. 加载模型
    model_path = r"C:\Users\Silvi\Projects\trading-bot\ppo_1519_tw_improved"
    print(f"\n📦 加载 AI 模型: {model_path}")
    ...
    try:
        ...
        df = yf.download('1519.TW', period='90d', progress=False, auto_adjust=True)
        ...
        try:
            ticker_info = yf.Ticker('1519.TW').info
            # ===== 基本面資料（PEG / 市值）=====
    ...
    ai_muted = should_mute_ai_signal('1519.TW', threshold=52)
    if ai_muted:
    ...
    try:
        profit_positive = evaluate_fundamentals_for_sell(yf, '1519.TW')['good']
    except Exception:
    ...
    # 7. 初始化动态权重计算器
    weight_calc = DynamicWeightCalculator('1519.TW')
    ...
    sentiment_result = calculate_sentiment_score('1519.TW', verbose=False)
    ...
    if action_value > 0.1:
        ...
        if bb_position < 35 and sell_score > 0:
            # 获取公司基本面（超跌时才检查）
            fundamentals = evaluate_fundamentals_for_sell(yf, '1519.TW')
        ...
        _growth = calculate_growth_score_adjustment(yf, '1519.TW')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        try:
            import yfinance as yf
            chart_df = yf.Ticker("1519.TW").history(period="6mo")
            chart_df.columns = [c.lower() for c in chart_df.columns]
            chart_path = "1519_chart.png"
            plot_candlestick(chart_df, "1519.TW", save_path=chart_path)
        except Exception as e:
            print(f"   圖表生成失敗: {e}")

        print(f"\n📱 快速摘要:")
        print(f"   股票: {result['symbol']} (華城)")
        ...
        # 顯示AI模型準確度摘要
        print(f"   {get_model_accuracy_display('1519')}")
    else:
        print("\n❌ 信号生成失败")
