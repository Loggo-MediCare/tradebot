...
print(f"準備訓練 {len(stocks_to_train)} 支股票...")
print("=" * 80)
...
for ticker, name in stocks_to_train:
    code = ticker.replace('.TW', '_tw').replace('.TWO', '_two').lower()
...
if results:
    ...
    for ticker, name, accuracy, status in results:
        if status == 'SUCCESS':
            print(f"{ticker:12} {name:10} {accuracy:6.2f}%  {status}")
        else:
            print(f"{ticker:12} {name:10}   N/A    {status}")
