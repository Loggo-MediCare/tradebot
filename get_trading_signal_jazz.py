"""
美股 JAZZ (Jazz Pharmaceuticals) AI 交易信号生成器
======================================
...
def get_trading_signal():
    """生成今日交易信号"""
    # 壓縮標題區塊 - 從6行減少到2行
    accuracy_display = get_model_accuracy_display('JAZZ')
    print(f"🤖 JAZZ (Jazz Pharmaceuticals) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print("=" * 80)

    # 1. 加载模型 (XGBoost + PPO Hybrid)
    print(f"\n📦 加载 Hybrid 模型: XGBoost + PPO")
    try:
        model = joblib.load("xgb_jazz_model.pkl")
    ...
    ppo_model = None
    try:
        ppo_model = PPO.load("ppo_jazz_improved")
        print("✅ PPO 模型加载成功!")
    except Exception as e:
    ...
    try:
        ...
        df = yf.download('JAZZ', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ticker_info = yf.Ticker('JAZZ').info
    ...
    if ppo_model is not None:
        ...
        try:
            ...
            print(format_ppo_roi_line('JAZZ', 'JAZZ', 'ppo_jazz_improved', df, ppo_action))
        except Exception as e:
    ...
    ai_muted = should_mute_ai_signal('JAZZ', threshold=52)
    if ai_muted:
    ...
    try:
        profit_positive = evaluate_fundamentals_for_sell(yf, 'JAZZ')['good']
    except Exception:
    ...
    # 7. 初始化动态权重计算器
    weight_calc = DynamicWeightCalculator('JAZZ')
    ...
    sentiment_result = calculate_sentiment_score('JAZZ', verbose=True)
    ...
    if action_value > 0.1:
        ...
        # ==========================================
        # 🎯 JAZZ 专属策略增强 (基于相关性与机构资金)
        # ==========================================
        print("\n" + "=" * 80)
        print("🎯 JAZZ Jazz Pharmaceuticals 专属策略分析")
        print("=" * 80)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            # 获取公司基本面（超跌时才检查）
            fundamentals = evaluate_fundamentals_for_sell(yf, 'JAZZ')
        ...
        _growth = calculate_growth_score_adjustment(yf, 'JAZZ')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        # 顯示AI模型準確度摘要
        print(f"   {get_model_accuracy_display('JAZZ')}")
    else:
        print("\n❌ 信号生成失败")
