...
def get_trading_signal():
    """生成今日交易信号"""
    print("=" * 80)
    print("🤖 美股 tsla (tsla) AI 交易信号生成器")
    print("=" * 80)
   # print(f("生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    # 顯示AI模型準確度
    accuracy_display = get_model_accuracy_display('TSLA')
    print(f"模型準確度: {accuracy_display}")
    print("=" * 80)

    # 1. 加载模型
    import os
    model_filename = "ppo_tsla_improved"
    ...
    try:
        ...
        df = yf.download('tsla', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ticker_info = yf.Ticker('TSLA').info
    ...
    ai_muted = should_mute_ai_signal('TSLA', threshold=52)
    if ai_muted:
    ...
    try:
        profit_positive = evaluate_fundamentals_for_sell(yf, 'TSLA')['good']
    except Exception:
    ...
    # 7. 初始化动态权重计算器
    weight_calc = DynamicWeightCalculator('tsla')
    ...
    sentiment_result = calculate_sentiment_score('tsla', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            # 获取公司基本面（超跌时才检查）
            fundamentals = evaluate_fundamentals_for_sell(yf, 'tsla')
        ...
        _growth = calculate_growth_score_adjustment(yf, 'TSLA')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        # 顯示AI模型準確度摘要
        print(f"   {get_model_accuracy_display('TSLA')}")
    else:
        print("\n❌ 信号生成失败")
