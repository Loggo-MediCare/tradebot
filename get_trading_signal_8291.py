...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('8291.TWO')


    print(f"🤖 8291.TWO (尚茂) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    model_path = "ppo_8291_two_improved"
    ...
    try:
        ...
        df = yf.download('8291.TWO', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ...
            ticker_info = yf.Ticker('8291.TWO').info
    ...
    ai_muted = should_mute_ai_signal('8291.TWO', threshold=52)
    ...
    try:
        ...
        profit_positive = evaluate_fundamentals_for_sell(yf, '8291.TWO')['good']
    ...
    weight_calc = DynamicWeightCalculator('8291.TWO')
    ...
    sentiment_result = calculate_sentiment_score('8291.TWO', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            ...
            fundamentals = evaluate_fundamentals_for_sell(yf, '8291.TWO')
        ...
        _growth = calculate_growth_score_adjustment(yf, '8291.TWO')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"   {get_model_accuracy_display('8291.TWO')}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 8291 尚茂 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('8291', '尚茂', max_results=5)