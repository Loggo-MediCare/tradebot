...
def get_trading_signal():
    """生成今日交易信号"""
    # 壓縮標題區塊 - 從6行減少到2行
    accuracy_display = get_model_accuracy_display('FN')
    print(f"🤖 FN (FN) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print("=" * 80)

    # 1. 加载模型
    import os
    model_filename = "ppo_fn_improved"
    ...
    try:
        ...
        df = yf.download('FN', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
    ...
    ai_muted = should_mute_ai_signal('FN', threshold=52)
    if ai_muted:
    ...
    try:
        profit_positive = evaluate_fundamentals_for_sell(yf, 'FN')['good']
    except Exception:
    ...
    # 7. 初始化动态权重计算器
    weight_calc = DynamicWeightCalculator('FN')
    ...
    sentiment_result = calculate_sentiment_score('FN', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            # 获取公司基本面（超跌时才检查）
            fundamentals = evaluate_fundamentals_for_sell(yf, 'FN')
        ...
        _growth = calculate_growth_score_adjustment(yf, 'FN')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        # 顯示AI模型準確度摘要
        print(f"   {get_model_accuracy_display('FN')}")
    else:
        print("\n❌ 信号生成失败")
