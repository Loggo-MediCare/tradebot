"""Train XGBoost + PPO for 10 Western stocks: SNDK,TSLA,NVDA,MU,AMZN,GOOGL,WDC,AVGO,LITE,TSM"""
...
print(f"\n{'='*60}\n  WESTERN STOCKS COMPARISON SUMMARY\n{'='*60}")
print(f"  {'Ticker':<8} {'Name':<18} {'XGBoost':>9} {'PPO':>9} {'Winner':>9}")
print(f"  {'-'*57}")
for t,n,xa,pa in results:
    w='XGBoost' if xa>=pa else 'PPO'
    print(f"  {t:<8} {n:<18} {xa*100:>8.2f}% {pa*100:>8.2f}% {w:>9}")
print(f"{'='*60}")
