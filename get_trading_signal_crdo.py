...
def get_trading_signal():
    ...
    print(f"🤖 CRDO (Credo Technology) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print("=" * 80)

    # 1. 加载模型
    import os
    model_filename = "ppo_crdo_improved"
    ...
    try:
        ...
        df = yf.download('CRDO', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
    ...
    ai_muted = should_mute_ai_signal('CRDO', threshold=52)
    if ai_muted:
    ...
    try:
        profit_positive = evaluate_fundamentals_for_sell(yf, 'CRDO')['good']
    except Exception:
    ...
    sentiment_result = calculate_sentiment_score('CRDO', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            # 获取公司基本面（超跌时才检查）
            fundamentals = evaluate_fundamentals_for_sell(yf, 'CRDO')
        ...
        _growth = calculate_growth_score_adjustment(yf, 'CRDO')
