...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('3661')


    print(f"🤖 3661 (世芯-KY) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    model_filename = "ppo_3661_tw_improved"
    ...
    try:
        ...
        df = yf.download('3661.TW', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ...
            ticker_info = yf.Ticker('3661.TW').info
    ...
    ai_muted = should_mute_ai_signal('3661.TW', threshold=52)
    ...
    try:
        ...
        profit_positive = evaluate_fundamentals_for_sell(yf, '3661.TW')['good']
    ...
    weight_calc = DynamicWeightCalculator('3661.TW')
    ...
    sentiment_result = calculate_sentiment_score('3661.TW', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            ...
            fundamentals = evaluate_fundamentals_for_sell(yf, '3661.TW')
        ...
        _growth = calculate_growth_score_adjustment(yf, '3661.TW')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"   股票: {result['symbol']} (世芯-KY)")
        ...
        print(f"   {get_model_accuracy_display('3661')}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 3661 世芯-KY 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('3661', '世芯-KY', max_results=5)