"""
LITE (Lumentum Holdings) XGBoost 優化訓練
"""
import os
...
TICKER = 'LITE'
print(f"優化訓練 {TICKER} (Lumentum Holdings) XGBoost 模型")
print("=" * 80)

try:
    # 下載數據 - 使用更長的歷史數據
    df = yf.download(TICKER, start='2010-01-01', end='2026-12-31', progress=False)
    ...
    model = xgb.XGBClassifier(
        ...
        random_state=42,
        eval_metric='logloss',
        scale_pos_weight=len(y_train[y_train==0]) / len(y_train[y_train==1])  # 處理類別不平衡
    )
    ...
    # 如果準確度提升，保存模型
    if test_acc >= 0.494:  # 至少比之前好一點
        joblib.dump(model, 'xgb_lite_model.pkl')
        print(f"\n✅ 模型已保存: xgb_lite_model.pkl (準確度提升!)")
        ...
        with open('LITE_feature_importance.json', 'w', encoding='utf-8') as f:
        ...
        print(f"\n⚠️ 準確度未提升 ({test_acc*100:.2f}%)，未保存模型")
