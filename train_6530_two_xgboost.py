...
TICKER = '6530.TWO'
print(f"訓練 6530.TWO (創威) XGBoost 模型")
print("=" * 80)
...
try:
    ...
    joblib.dump(model, 'xgb_6530_two_model.pkl')
    print(f"\n模型已保存: xgb_6530_two_model.pkl")

    accuracy_data = {
        'symbol': '6530.TWO', 'model_type': 'XGBoost',
        'training_accuracy': float(train_acc * 100),
        'validation_accuracy': float(test_acc * 100),
        'backtest_accuracy': float(test_acc * 100),
        'last_updated': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    }
    with open('model_accuracy_6530_TWO.json', 'w', encoding='utf-8') as f:
    ...
    fi_data = {
        'ticker': '6530.TWO', 'model_type': 'XGBoost',
    ...
    with open('6530_TWO_feature_importance.json', 'w', encoding='utf-8') as f:
