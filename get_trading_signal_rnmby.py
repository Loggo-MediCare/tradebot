...
def get_trading_signal():
    ...
    print(f"🤖 RNMBY (Rheinmetall ADR) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print("=" * 80)

    # 1. 加载模型
    import os
    model_filename = "ppo_rnmby_improved"
    ...
    try:
        ...
        df = yf.download('RNMBY', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
    ...
    ai_muted = should_mute_ai_signal('RNMBY', threshold=52)
    if ai_muted:
    ...
    try:
        profit_positive = evaluate_fundamentals_for_sell(yf, 'RNMBY')['good']
    except Exception:
    ...
    sentiment_result = calculate_sentiment_score('RNMBY', verbose=True)
...
# 加入爆发行情评分调整
        ...
        _growth = calculate_growth_score_adjustment(yf, 'RNMBY')
