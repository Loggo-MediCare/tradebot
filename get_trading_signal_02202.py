...
港股 02202 (萬科企業 (Vanke)) AI 交易信号生成器
...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('2202.HK')


    print(f"🤖 2202.HK (萬科企業 Vanke) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    model_path = r"ppo_02202_improved"
    ...
    try:
        ...
        df = yf.download('2202.HK', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ...
            ticker_info = yf.Ticker('2202.HK').info
    ...
    ai_muted = should_mute_ai_signal('2202.HK', threshold=52)
    ...
    try:
        ...
        profit_positive = evaluate_fundamentals_for_sell(yf, '2202.HK')['good']
    ...
    weight_calc = DynamicWeightCalculator('2202.HK')
    ...
    sentiment_result = calculate_sentiment_score('2202.HK', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            ...
            fundamentals = evaluate_fundamentals_for_sell(yf, '2202.HK')
        ...
        _growth = calculate_growth_score_adjustment(yf, '2202.HK')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"   {get_model_accuracy_display('2202.HK')}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 02202 萬科企業 (Vanke 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('02202', '萬科企業 (Vanke', max_results=5)