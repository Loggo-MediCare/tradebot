"""
批量創建新訓練股票的 XGBoost 交易信號文件
"""
import os

# Successfully trained stocks from batch
stocks_to_create = [
    ('1711.TW', '永光'),
    ('2399.TW', '映泰'),
    ('2467.TW', '志聖'),
...
print(f"Creating signal files for {len(stocks_to_create)} stocks...")
print("=" * 80)

created_files = []

for ticker, name in stocks_to_create:
...
TICKER = '{ticker}'
MODEL_FILE = 'xgb_{code}_model.pkl'
...
'''
    ...
    filename = f'get_trading_signal_{stock_number}.py'
    ...
    try:
        ...
        created_files.append((filename, ticker, name))
        print(f"[OK] Created: {filename} for {ticker} {name}")
    except Exception as e:
        print(f"[ERROR] Failed to create {filename}: {e}")

print("=" * 80)
print(f"Created {len(created_files)} new signal files")
print(f"Skipped {len(stocks_to_create) - len(created_files)} existing files")
