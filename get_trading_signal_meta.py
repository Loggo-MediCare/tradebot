...
def get_trading_signal():
    """生成今日交易信号"""
    print("=" * 80)
    print("🤖 美股 META AI AI HYBRID PPO + XGBoost 交易信号生成器")
    print("=" * 80)
    print(f"生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    # 顯示AI模型準確度
    accuracy_display = get_model_accuracy_display('META')
    ...
    try:
        model = joblib.load("xgb_meta_model.pkl")
    ...
    ppo_model = None
    try:
        ppo_model = PPO.load("ppo_meta_improved")
        print("✅ PPO 模型加载成功!")
    except Exception as e:
    ...
    try:
        ...
        df = yf.download('META', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ticker_info = yf.Ticker('META').info
    ...
    if ppo_model is not None:
        ...
        try:
            ...
            print(format_ppo_roi_line('META', 'META', 'ppo_meta_improved', df, ppo_action))
        except Exception as e:
    ...
    ai_muted = should_mute_ai_signal('META', threshold=52)
    if ai_muted:
    ...
    try:
        profit_positive = evaluate_fundamentals_for_sell(yf, 'META')['good']
    except Exception:
    ...
    # 7. 初始化动态权重计算器
    weight_calc = DynamicWeightCalculator('META')
    ...
    sentiment_result = calculate_sentiment_score('META', verbose=True)
    ...
    if action_value > 0.1:
        ...
        # ==========================================
        # 🎯 META 专属策略增强 (基于相关性与机构资金)
        # ==========================================
        print("\n" + "=" * 80)
        print("🎯 META Meta 专属策略分析")
        print("=" * 80)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            # 获取公司基本面（超跌时才检查）
            fundamentals = evaluate_fundamentals_for_sell(yf, 'META')
        ...
        _growth = calculate_growth_score_adjustment(yf, 'META')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        # 顯示AI模型準確度摘要
        print(f"   {get_model_accuracy_display('META')}")
    else:
        print("\n❌ 信号生成失败")
