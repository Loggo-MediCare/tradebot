...
class ImprovedTradingEnv(gym.Env):
    ...
    def __init__(self, df, initial_balance=10000):
        ...
        self.current_step = self.history_window - 1


        self.action_space = spaces.Box(low=-1.0, high=1.0, shape=(1,), dtype=np.float32)


        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(10, 8), dtype=np.float32)
    ...
    def _get_features(self, row):
        ...
        return np.array([
            ...
            float(row.get('sma_10', 0.0)),


            float(row.get('sma_30', 0.0)),
    ...
    def _get_observation(self):
        ...
        start_idx = self.current_step - self.history_window + 1


        for idx in range(start_idx, self.current_step + 1):


            safe_idx = max(0, min(idx, len(self.df) - 1))


            row = self.df.iloc[safe_idx]


            obs_matrix.append(self._get_features(row))
...
def add_technical_indicators(df):
    ...
    # Features used by the saved PPO model.
    ...
    df['rsi'] = (df['rsi_raw'] / 100.0).fillna(0.5)
    ...
    feature_cols = [
        ...
        'log_ret', 'sma_10', 'sma_30', 'sma_50', 'rsi',


        'macd', 'macd_signal', 'bb_position', 'vol_change', 'volatility'
...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('2331.TW')
    ...
    model_path = r"C:\Users\Silvi\Projects\trading-bot\ppo_2331_tw_improved"
    ...
    try:
        ...
        try:
            ...
            ticker_info = yf.Ticker('2331.TW').info
    ...
    if len(df) < 10:
        ...
        print("❌ 数据不足，无法构建模型需要的 10x8 观察窗口")
    ...
    vec_env = DummyVecEnv([lambda: ImprovedTradingEnv(df)])


    vecnorm_path = r"C:\Users\Silvi\Projects\trading-bot\ppo_2331_advanced_vecnorm.pkl"
    ...
    if os.path.exists(vecnorm_path):
        ...
        try:
            ...
            print(f"⚠️  标准化统计加载失败，使用原始特征推理: {e}")
...
if __name__ == "__main__":
    ...
    if result:
        ...
        try:
            ...
            chart_df = yf.Ticker("2331.TW").history(period="6mo")


            chart_df.columns = [c.lower() for c in chart_df.columns]


            chart_path = "2331_chart.png"


            plot_candlestick(chart_df, "2331.TW", save_path=chart_path)
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 2331 精英 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('2331', '精英', max_results=5)