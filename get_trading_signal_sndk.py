...
def get_trading_signal():
    """生成今日交易信号"""
    print("=" * 80)
    print("🤖 美股 SNDK (SanDisk) AI 交易信号生成器")
    print("=" * 80)
   # print(f("生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    # 1. 加载模型
    model_path = r"C:\Users\Silvi\Projects\trading-bot\ppo_sndk_improved"
    print(f"\n📦 加载 AI 模型: {model_path}")
    ...
    try:
        ...
        df = yf.download('SNDK', period='90d', progress=False, auto_adjust=True)
        ...
        try:
            ticker_info = yf.Ticker('SNDK').info
    ...
    # 7. 初始化动态权重计算器
    weight_calc = DynamicWeightCalculator('SNDK')
    ...
    sentiment_result = calculate_sentiment_score('SNDK', verbose=False)
...
if __name__ == "__main__":
    ...
    if result:
        ...
        try:
            import yfinance as yf
            chart_df = yf.Ticker("SNDK").history(period="6mo")
            chart_df.columns = [c.lower() for c in chart_df.columns]
            chart_path = "SNDK_chart.png"
            plot_candlestick(chart_df, "SNDK", save_path=chart_path)
        except Exception as e:
            print(f"   圖表生成失敗: {e}")
