"""
重新訓練穩定型股票 PPO 模型
使用波動率標準化獎勵函數，解決低波動股票 reward 過小導致不交易的問題

Stocks: 2412 中華電信, 2382 廣達電腦, 2049 上銀
"""
import os
...
class VolNormTradingEnv(gym.Env):
    """
    Volatility-normalized reward:
    reward = (value_change / total_value) / avg_daily_vol
    This amplifies the signal for stable/low-volatility stocks so the model
    learns even from small price moves.
    """
    def __init__(self, df, avg_daily_vol, initial_balance=10000):
        super(VolNormTradingEnv, self).__init__()
        self.df = df.reset_index(drop=True)
        self.initial_balance = initial_balance
        self.avg_daily_vol = max(avg_daily_vol, 0.001)  # floor at 0.1%
        # Scale factor: normalize to 1% effective daily vol target
        self.vol_scale = 0.01 / self.avg_daily_vol
        self.current_step = 0
    ...
    def step(self, action):
        ...
        total_value_before = self.balance + self.shares_held * current_price

        traded = False

        if action > 0.1:  # lower threshold for stable stocks
        ...
        # Volatility-normalized reward: amplifies signal for stable stocks
        value_change = total_value - self.last_total_value
        pct_return = value_change / total_value_before if total_value_before > 0 else 0
        reward = pct_return * self.vol_scale
        ...
        # Penalize inactivity
        if self.total_trades < self.current_step / 50:
            reward -= 0.005
...
def train_stock(ticker, symbol, name):
    print(f"\n{'='*80}")
    print(f"訓練 {ticker} ({name}) - 波動率標準化版")
    print(f"{'='*80}")
    ...
    try:
        ...
        avg_daily_vol = float(daily_returns.std())
        print(f"  平均日波動率: {avg_daily_vol*100:.3f}% (波動率縮放因子: {0.01/max(avg_daily_vol,0.001):.2f}x)")
        ...
        # 訓練 (更多步數給穩定股票)
        env = DummyVecEnv([lambda: VolNormTradingEnv(df_clean, avg_daily_vol)])
        model = PPO(
            'MlpPolicy',
            env,
        ...
        # 評估
        test_env = VolNormTradingEnv(df_clean, avg_daily_vol)
...
if __name__ == "__main__":
    ...
    for ticker, name, success in results:
        print(f"  {'OK' if success else 'FAIL'}  {ticker} ({name})")
