...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('4973')


    print(f"🤖 4973 (廣宇) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    model_path = r"ppo_4973_tw_improved"
    ...
    try:
        ...
        df = yf.download('4973.TWO', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ...
            ticker_info = yf.Ticker('4973.TWO').info
    ...
    ai_muted = should_mute_ai_signal('4973.TWO', threshold=52)
    ...
    try:
        ...
        profit_positive = evaluate_fundamentals_for_sell(yf, '4973.TWO')['good']
    ...
    weight_calc = DynamicWeightCalculator('4973.TWO')
    ...
    sentiment_result = calculate_sentiment_score('4973.TWO', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            ...
            fundamentals = evaluate_fundamentals_for_sell(yf, '4973.TWO')
        ...
        _growth = calculate_growth_score_adjustment(yf, '4973.TWO')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"   {get_model_accuracy_display('4973')}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 4973 廣宇 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('4973', '廣宇', max_results=5)