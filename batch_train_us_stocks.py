"""
Batch Training Script for US Stocks
Trains multiple US stock trading models
"""
import subprocess
import sys
import io
...
# List of US stocks to train
US_STOCKS = [
    'MRNA', 'MU', 'AMD', 'GILD', 'ARM', 'STX', 'EXE', 'EQTT',
    'RKLB', 'ASTS', 'FLY', 'SATS', 'PL', 'AMZN', 'TMUS', 'QCOM',
    'FLYX', 'GOOGL', 'BAC', 'DXYZ', 'XOVR', 'VCX'
]

def create_training_script(ticker):
    """Create a training script for a specific ticker"""
    script_content = f'''"""
改进版 {ticker} 股票交易 AI 训练
"""
import os
...
def download_and_prepare_data(ticker='{ticker}', start_date='2015-01-01', end_date='2025-01-31'):
    print("=" * 70)
...
if __name__ == "__main__":
    ...
    print("\\n[OK] 训练完成!")
    print(f"模型文件: ppo_{{TICKER.lower()}}_improved.zip")
'''
    filename = f'train_{ticker.lower()}_improved.py'
    with open(filename, 'w', encoding='utf-8') as f:
        f.write(script_content)
    return filename

def train_stock(ticker):
    """Create and train a single stock"""
    print(f"\n{'='*70}")
    print(f"Processing {ticker}...")
    print(f"{'='*70}")

    # Create training script
    script_file = create_training_script(ticker)
...
if __name__ == "__main__":
    print("="*70)
    print("Batch Training for US Stocks")
    print("="*70)
    print(f"Stocks to train: {', '.join(US_STOCKS)}")
    print(f"Total: {len(US_STOCKS)} stocks")
    print("="*70)

    results = {}
    for ticker in US_STOCKS:
        success = train_stock(ticker)
    ...
    print(f"[OK] Successful: {len(successful)}/{len(US_STOCKS)}")
    if successful:
        for ticker in successful:
            print(f"   - {ticker}")

    if failed:
        print(f"\n[ERROR] Failed: {len(failed)}/{len(US_STOCKS)}")
