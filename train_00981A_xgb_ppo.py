"""Train XGBoost + PPO for 00981A 主動統一台股增長 (00981A.TW)"""
...
CODE = '00981A'; TICKER = '00981A.TW'; NAME = '主動統一台股增長'
...
if df.empty or len(df) < 50:
    print(f"[ERROR] Only {len(df)} rows — insufficient data for {TICKER}"); sys.exit(1)
...
joblib.dump(xm, 'xgb_00981A_tw_model.pkl')
...
json.dump(acc_data, open('model_accuracy_00981A_TW.json',  'w', encoding='utf-8'), indent=2)
json.dump(acc_data, open('model_accuracy_00981A.json', 'w', encoding='utf-8'), indent=2)
print(f"[XGBoost] Validation: {xgb_acc*100:.2f}%")
...
pm.save('ppo_00981A_tw_improved')
...
json.dump({'symbol': CODE, 'ticker': TICKER, 'model_type': 'PPO',
           ...
           'last_updated': datetime.now().strftime('%Y-%m-%d %H:%M:%S')},
          open('model_accuracy_00981A_TW_ppo.json', 'w', encoding='utf-8'), indent=2)
