...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('7610')


    print(f"🤖 7610 (聯友金屬) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    model_filename = "ppo_7610_tw_improved"
    ...
    try:
        ...
        df = yf.download('7610.TW', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ...
            ticker_info = yf.Ticker('7610.TW').info
    ...
    ai_muted = should_mute_ai_signal('7610.TW', threshold=52)
    ...
    try:
        ...
        profit_positive = evaluate_fundamentals_for_sell(yf, '7610.TW')['good']
    ...
    weight_calc = DynamicWeightCalculator('7610.TW')
    ...
    sentiment_result = calculate_sentiment_score('7610.TW', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            ...
            fundamentals = evaluate_fundamentals_for_sell(yf, '7610.TW')
        ...
        _growth = calculate_growth_score_adjustment(yf, '7610.TW')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"   {get_model_accuracy_display('7610')}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 7610 聯友金屬 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('7610', '聯友金屬', max_results=5)