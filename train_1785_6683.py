"""Train XGBoost + PPO for 1785.TWO and 6683.TWO"""
...
for code, ticker, name in STOCKS:
    ...
    joblib.dump(m, f'xgb_{code}_two_model.pkl')
    with open(f'model_accuracy_{code}_TWO.json', 'w', encoding='utf-8') as f:
    ...
    ppo.save(f'ppo_{code}_two_improved')
    ...
    with open(f'model_accuracy_{code}_TWO_ppo.json', 'w', encoding='utf-8') as f:
    ...
    results.append({'ticker': ticker, 'name': name, 'xgb': xgb_acc * 100, 'ppo': ppo_acc * 100})
...
print(f"  {'-'*56}")
for r in results:
    w = 'XGBoost' if r['xgb'] >= r['ppo'] else 'PPO'
    print(f"  {r['ticker']:<12} {r['name']:<12} {r['xgb']:>9.2f}% {r['ppo']:>9.2f}% {w:>10}")
print(f"{'='*60}")
