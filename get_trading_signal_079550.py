...
079550.KS (LIG Nex1 Co Ltd) XGBoost 交易信號生成器
...
MODEL_FILE = 'xgb_079550_ks_model.pkl'
...
def get_trading_signal():
    ...
    print(f"🤖 {TICKER} (LIG Nex1) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    print(f"均線: SMA10=₩{float(latest['sma_10']):,.0f}, SMA30=₩{float(latest['sma_30']):,.0f} {'[多頭]' if float(latest['sma_10']) > float(latest['sma_30']) else '[空頭]'}")
    ...
    try:
        ...
        sentiment_result = calculate_sentiment_score(TICKER, verbose=True)
    ...
    if prediction == 1 and buy_prob >= 60:
        ...
        print(f"   建議買入價格: ₩{current_price * 0.995:,.0f} - ₩{current_price * 1.005:,.0f}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 079550 079550 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('079550', '079550', max_results=5)