...
def get_trading_signal():
    """生成今日交易信号"""
    print("=" * 80)
    print("🤖 台股 6443 (元晶) AI 交易信号生成器")
    print("=" * 80)
   # print(f("生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    # 顯示AI模型準確度
    accuracy_display = get_model_accuracy_display('6443.TW')
    if accuracy_display:
        print(f"模型準確度: {accuracy_display}")
    print("=" * 80)

    # 1. 加载模型
    model_path = r"C:\Users\Silvi\Projects\trading-bot\ppo_6443_tw_improved"
    print(f"\n📦 加载 AI 模型: {model_path}")
    ...
    try:
        ...
        df = yf.download('6443.TW', period='90d', progress=False, auto_adjust=True)
        ...
        try:
            ticker_info = yf.Ticker('6443.TW').info
    ...
    # 7. 初始化动态权重计算器
    weight_calc = DynamicWeightCalculator('6443.TW')
    ...
    sentiment_result = calculate_sentiment_score('6443.TW', verbose=False)
    ...
    # ── Tavily 即時新聞 ─────────────────────────────────────────────────────
    print("\n" + "=" * 80)
    print("🌐 元晶 (6443.TW) 即時新聞  (Tavily REST API)")
    print("=" * 80)
    print_tavily_news('6443.TW', '元晶', max_results=5)
    ...
    if action_value > 0.1:
        ...
        try:
            ...
            manual_data = load_manual_sentiment('6443.TW')
            if manual_data and 'technical_override' in manual_data:
                override = manual_data['technical_override']
                if override.get('ignore_rsi_overbought') and rsi > 70:
                    manual_override_active = True
                    signal = "持有 (HOLD - 题材股续涨)"
                    signal_emoji = "🟢"
                    print(f"\n🏷️  检测到人工题材标注覆盖:")
                    print(f"   题材: {manual_data.get('theme', '')}")
                    print(f"   覆盖原因: {override.get('reason', '')}")
                    print(f"   ⚠️  {manual_data.get('risk_warning', '')}")
                    print(f"\n{signal_emoji} 信号: {signal}")
                    print(f"   AI原始建议: 卖出 (RSI {rsi:.1f})")
                    print(f"   人工覆盖: 持有 (题材股RSI钝化正常)")
                    print(f"\n   💡 菜篮族经济学操作建议:")
                    print(f"      • 忽略AI的卖出建议 (RSI超买但题材火爆)")
                    print(f"      • 沿5日均线持有 (SMA10: NT${sma_10:.2f})")
                    print(f"      • 止损价: NT${sma_10 * 0.95:.2f} (跌破5日线-5%)")
                    print(f"      • 不建议追高 (已涨47.5%)")
                    print(f"      • 目标价: NT$32 (前高压力)")
...
if __name__ == "__main__":
    ...
    if result:
        ...
        try:
            import yfinance as yf
            chart_df = yf.Ticker("6443.TW").history(period="6mo")
            chart_df.columns = [c.lower() for c in chart_df.columns]
            chart_path = "6443_chart.png"
            plot_candlestick(chart_df, "6443.TW", save_path=chart_path)
        except Exception as e:
            print(f"   圖表生成失敗: {e}")

        print(f"\n📱 快速摘要:")
        print(f"   股票: {result['symbol']} (元晶)")
        ...
        # 顯示AI模型準確度摘要
        print(f"   {get_model_accuracy_display('6443.TW')}")
    else:
        print("\n❌ 信号生成失败")
