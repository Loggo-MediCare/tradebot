...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('3037.TW')


    print(f"🤖 3037 (欣興 Unimicron) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    model_filename = "xgb_3037_tw_model.pkl"
    ...
    try:
        ...
        df = yf.download('3037.TW', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ...
            ticker_info = yf.Ticker('3037.TW').info
    ...
    ai_muted = should_mute_ai_signal('3037.TW', threshold=52)
    ...
    try:
        ...
        profit_positive = evaluate_fundamentals_for_sell(yf, '3037.TW')['good']
    ...
    weight_calc = DynamicWeightCalculator('3037.TW')
    ...
    sentiment_result = calculate_sentiment_score('3037.TW', verbose=True)
    ...
    if action_value > 0.1:
        ...
        print(f"  第一目标区:   NT${target_zone_low:.2f} - NT${target_zone_high:.2f} (分析师中值)")
        ...
        if current_price >= target_zone_high:
            ...
            buy_warnings.append(f"⚠️ 价格已达第一目标区上限 NT${target_zone_high}, 建议获利了结")
            ...
            buy_warnings.append(f"价格已进入目标区 NT${target_zone_low}-{target_zone_high}, 注意压力")
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            ...
            fundamentals = evaluate_fundamentals_for_sell(yf, '3037.TW')
        ...
        _growth = calculate_growth_score_adjustment(yf, '3037.TW')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"   {get_model_accuracy_display('3037.TW')}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 3037 欣興 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('3037', '欣興', max_results=5)