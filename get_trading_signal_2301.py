...
def get_trading_signal():
   ...
   # print(f("生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    # 1. 加载模型
    model_path = r"C:\Users\Silvi\Projects\trading-bot\ppo_2301_tw_improved"
    print(f"\n📦 加载 AI 模型: {model_path}")
    ...
    try:
        ...
        df = yf.download('2301.TW', period='90d', progress=False, auto_adjust=True)
    ...
    # ── Tavily 即時新聞 ─────────────────────────────────────────────────────
    print("\n" + "=" * 80)
    print("🌐 光寶科技 (2301.TW) 即時新聞  (Tavily REST API)")
    print("=" * 80)
    print_tavily_news('2301.TW', '光寶科技', max_results=5)
