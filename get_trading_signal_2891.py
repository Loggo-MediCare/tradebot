...
def get_trading_signal():
    """生成今日交易信号"""
    print("=" * 80)
    print("🤖 台股 2891 AI 交易信号生成器")
    print("=" * 80)
    print(f"生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    # 顯示AI模型準確度
    accuracy_display = get_model_accuracy_display('2891.TW')
    if accuracy_display:
        print(f"模型準確度: {accuracy_display}")
    print("=" * 80)

    # 1. 加载模型
    model_path = r"C:\Users\Silvi\Projects\trading-bot\ppo_2891_tw_improved"
    print(f"\n📦 加载 AI 模型: {model_path}")
    ...
    try:
        ...
        df = yf.download('2891.TW', period='90d', progress=False, auto_adjust=True)
    ...
    sentiment_result = calculate_sentiment_score('2891.TW', verbose=False)
    ...
    # ── Tavily 即時新聞 ─────────────────────────────────────────────────────
    print("\n" + "=" * 80)
    print("🌐 中信金 (2891.TW) 即時新聞  (Tavily REST API)")
    print("=" * 80)
    print_tavily_news('2891.TW', '中信金', max_results=5)
...
if __name__ == "__main__":
    ...
    if result:
        ...
        try:
            import yfinance as yf
            chart_df = yf.Ticker("2891.TW").history(period="6mo")
            chart_df.columns = [c.lower() for c in chart_df.columns]
            chart_path = "2891_chart.png"
            plot_candlestick(chart_df, "2891.TW", save_path=chart_path)
        except Exception as e:
            print(f"   圖表生成失敗: {e}")
        ...
        # 顯示AI模型準確度摘要
        print(f"   {get_model_accuracy_display('2891.TW')}")
    else:
        print("\n❌ 信号生成失败")
