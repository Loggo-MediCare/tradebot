...
TICKER = '3221.TWO'
print(f"訓練 3221.TWO (台嘉碩) XGBoost 模型")
print("=" * 80)
...
try:
    ...
    joblib.dump(model, 'xgb_3221_two_model.pkl')
    print(f"\n模型已保存: xgb_3221_two_model.pkl")

    accuracy_data = {
        'symbol': '3221.TWO', 'model_type': 'XGBoost',
        'training_accuracy': float(train_acc * 100),
        'validation_accuracy': float(test_acc * 100),
        'backtest_accuracy': float(test_acc * 100),
        'last_updated': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    }
    with open('model_accuracy_3221_TWO.json', 'w', encoding='utf-8') as f:
    ...
    fi_data = {
        'ticker': '3221.TWO', 'model_type': 'XGBoost',
    ...
    with open('3221_TWO_feature_importance.json', 'w', encoding='utf-8') as f:
