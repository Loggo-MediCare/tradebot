...
def get_trading_signal():
    ...
    print(f"🤖 OUST (OUST) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print("=" * 80)

    # 1. 加载模型
    import os
    model_filename = "ppo_oust_improved"
    ...
    try:
        ...
        df = yf.download('OUST', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ticker_info = yf.Ticker('OUST').info
    ...
    ai_muted = should_mute_ai_signal('OUST', threshold=52)
    if ai_muted:
    ...
    try:
        profit_positive = evaluate_fundamentals_for_sell(yf, 'OUST')['good']
    except Exception:
    ...
    # 7. 初始化动态权重计算器
    weight_calc = DynamicWeightCalculator('OUST')
    ...
    sentiment_result = calculate_sentiment_score('OUST', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            # 获取公司基本面（超跌时才检查）
            fundamentals = evaluate_fundamentals_for_sell(yf, 'OUST')
        ...
        _growth = calculate_growth_score_adjustment(yf, 'OUST')
