...
def get_trading_signal():
    ...
    try:
        ...
        df = yf.download('2330.TW', period='90d', progress=False, auto_adjust=True)
    ...
    rsi_status = '[超買]' if rsi > 70 else '[超賣]' if rsi < 15 else '[中性]'
    ...
    print()
    try:
        profit_positive = evaluate_fundamentals_for_sell(yf, '2330.TW')['good']
    except Exception:
        profit_positive = None

    fmt.print_section("MA50 趨勢分析")
    ma50_slope_info = calculate_ma50_slope(
        df['close'],
        window=50,
        slope_period=5,
        current_rsi=rsi,
    ...
    fmt.print_metric("RSI (10)", f"{rsi:.2f}", rsi_status)
    ...
    sentiment_result = calculate_sentiment_score('2330.TW', verbose=False)
    ...
    ai_muted = should_mute_ai_signal('2330.TW', threshold=52)
    if ai_muted:
    ...
    if action_value > 0.1:
        ...
        buy_score, signal_override, buy_reasons, buy_warnings, buy_metadata, sentiment_result = \
            calculate_enhanced_buy_score_with_sentiment(
                ...
                ai_action=action_value, buy_weights=buy_weights, symbol='2330.TW'
            )
    ...
    # ========== 處理賣出信號 ==========
        ...
        _growth = calculate_growth_score_adjustment(yf, '2330.TW')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        try:
            import yfinance as yf
            chart_df = yf.Ticker("2330.TW").history(period="6mo")
            chart_df.columns = [c.lower() for c in chart_df.columns]
            chart_path = "2330_chart.png"
            plot_candlestick(chart_df, "2330.TW", save_path=chart_path)
        except Exception as e:
            print(f"   圖表生成失敗: {e}")
