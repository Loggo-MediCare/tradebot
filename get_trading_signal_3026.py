...
def get_trading_signal():
    ...
    model_path = r"C:\Users\Silvi\Projects\trading-bot\ppo_3026_tw_improved"
    print(f"\n📦 加载 AI 模型: {model_path}")
    try:
    ...
    try:
        ...
        df = yf.download('3026.TW', period='2y', progress=False, auto_adjust=True)
        if df.empty:
            print("❌ 无法获取数据")
            return None
    ...
    sentiment_result = calculate_sentiment_score('3026.TW', verbose=False)
    ...
    # ── Tavily 即時新聞 ─────────────────────────────────────────────────────
    print("\n" + "=" * 80)
    print("🌐 禾伸堂 (3026.TW) 即時新聞  (Tavily REST API)")
    print("=" * 80)
    print_tavily_news('3026.TW', '禾伸堂', max_results=5)
    ...
    if action_value > 0.1:
        ...
        buy_score, signal_override, buy_reasons, buy_warnings, buy_metadata, sentiment_result = \
            calculate_enhanced_buy_score_with_sentiment(
                ...
                ai_action=action_value, buy_weights=buy_weights, symbol='3026.TW'
            )
...
if __name__ == "__main__":
    ...
    if result:
        ...
        try:
            import yfinance as yf
            chart_df = yf.Ticker("3026.TW").history(period="6mo")
            chart_df.columns = [c.lower() for c in chart_df.columns]
            chart_path = "3026_chart.png"
            plot_candlestick(chart_df, "3026.TW", save_path=chart_path)
        except Exception as e:
            print(f"   圖表生成失敗: {e}")
