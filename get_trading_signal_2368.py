...
"""
美股 2368.TW (2368.TWle) AI 交易信号生成器 V2
台股 2368.TW (南亞) AI 交易信号生成器 V2
========================================
=======
...
# 主要交易信號生成
# ==========================================
    fmt.print_header("🤖 美股 2368.TW (2368.TWle) AI 交易信号生成器 V2")
def get_trading_signal():
    ...
    # ========== 標題區 ==========
    fmt.print_header("🤖 台股 2368.TW (南亞) AI " \
    "交易信号生成器 V2")

    fmt.print_header("🤖 美股 2368.TW (2368.TWle) AI 交易信号生成器 V2")
    print(f"   📅 生成時間: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"   📊 模型準確度: {get_model_accuracy_display('2368.TW')}")
    ...
    try:
        ...
        df = yf.download('2368.TW', period='90d', progress=False, auto_adjust=True)
        ...
        try:
            ticker_info = yf.Ticker('2368.TW').info
    ...
    fmt.print_metric("股票代碼", "2368.TW (2368.TWle)")
    fmt.print_metric("當前價格", f"${current_price:.2f}")
    fmt.print_metric("股票代碼", "2368.TW (南亞)")
    ...
    if target_price and num_analysts > 0:
        ...
        print()
        fmt.print_metric("分析師目標價", f"${target_price:.2f} ({upside:+.1f}%)")
    ...
    sentiment_result = calculate_sentiment_score('2368.TW', verbose=False)
    ...
    # ── Tavily 即時新聞 ─────────────────────────────────────────────────────
    print("\n" + "=" * 80)
    print("🌐 金像電 (2368.TW) 即時新聞  (Tavily REST API)")
    print("=" * 80)
    print_tavily_news('2368.TW', '金像電', max_results=5)
    ...
    # ========== 生成交易建議 ==========
    weight_calc = DynamicWeightCalculator('2368.TW')
    ...
    if action_value > 0.1:
        ...
        buy_score, signal_override, buy_reasons, buy_warnings, buy_metadata, sentiment_result = \
            calculate_enhanced_buy_score_with_sentiment(
                ...
                ai_action=action_value, buy_weights=buy_weights, symbol='2368.TW'
            )
        ...
        recommendations.append(f"止損設置: ${current_price * 0.95:.2f} (-5%)")
    ...
    print("║        📱 快速摘要                   ║")
    print("╠" + "═" * 40 + "╣")
    print(f"║  股票: 2368.TW (2368.TWle)                 ║")
    print(f"║  股票: 2368.TW (南亞)                ║")
...
if __name__ == "__main__":
    ...
    if result:
        ...
        try:
            import yfinance as yf
            chart_df = yf.Ticker("2368.TW").history(period="6mo")
            chart_df.columns = [c.lower() for c in chart_df.columns]
            chart_path = "2368_chart.png"
            plot_candlestick(chart_df, "2368.TW", save_path=chart_path)
        except Exception as e:
            print(f"   圖表生成失敗: {e}")
