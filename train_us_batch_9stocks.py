"""
批量訓練 9 支美股 XGBoost 模型
MRNA, OMC, GRMN, TPL, COIN, GPN, MOH, BAX, CIEN
"""
import os
...
def train_stock(ticker, name):
    ...
    try:
        ...
        # 保存模型
        model_file = f'xgb_{ticker.lower()}_model.pkl'
...
if __name__ == "__main__":
    ...
    for stock in STOCKS:
        result = train_stock(stock['ticker'], stock['name'])
        if result:
    ...
    for result in results:
        ...
        print(f"{status_emoji} {result['ticker']:6s} {result['name']:25s} {result['accuracy']*100:6.2f}% ({result['threshold']*100:.1f}% 閾值)")
