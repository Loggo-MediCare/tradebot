...
class AdvancedTradingEnv(gym.Env):
    ...
    def _get_features(self, row):
        ...
        return np.array([
            ...
            float(row.get('K', 50)), float(row.get('D', 50)),  # KD
            float(row.get('OBV', 0)), float(row.get('ATR', 0)),
            float(row.get('volatility', 0)),
            float(row.get('price_change_5d', 0)), float(row.get('price_change_20d', 0)),
...
def add_technical_indicators(df):
    # Ensure 'close' is treated as a Series, even if it's a single-column DataFrame
    if isinstance(df['close'], pd.DataFrame) and len(df['close'].columns) == 1:
        close_price_series = df['close'].iloc[:, 0]
    else:
        close_price_series = df['close']

    # Ensure 'high' and 'low' are also treated as Series
    if isinstance(df['high'], pd.DataFrame) and len(df['high'].columns) == 1:
        high_price_series = df['high'].iloc[:, 0]
    else:
        high_price_series = df['high']

    if isinstance(df['low'], pd.DataFrame) and len(df['low'].columns) == 1:
        low_price_series = df['low'].iloc[:, 0]
    else:
        low_price_series = df['low']

    # Ensure 'volume' is also treated as a Series
    if isinstance(df['volume'], pd.DataFrame) and len(df['volume'].columns) == 1:
        volume_series = df['volume'].iloc[:, 0]
    else:
        volume_series = df['volume']

    # （你的原函數，略微補強）
    df['sma_10'] = close_price_series.rolling(10).mean()
    df['sma_30'] = close_price_series.rolling(30).mean()
    df['sma_50'] = close_price_series.rolling(50).mean()
    df['ema_12'] = close_price_series.ewm(span=12).mean()
    df['ema_26'] = close_price_series.ewm(span=26).mean()

    delta = close_price_series.diff()
    ...
    df['bb_middle'] = close_price_series.rolling(20).mean()
    std = close_price_series.rolling(20).std()
    df['bb_upper'] = df['bb_middle'] + 2 * std
    df['bb_lower'] = df['bb_middle'] - 2 * std

    low_14 = low_price_series.rolling(14).min()
    high_14 = high_price_series.rolling(14).max()
    df['K'] = 100 * (close_price_series - low_14) / (high_14 - low_14 + 1e-10)
    df['D'] = df['K'].rolling(3).mean()

    df['OBV'] = (np.sign(close_price_series.diff()) * volume_series).fillna(0).cumsum()

    tr = pd.concat([
        high_price_series - low_price_series,
        (high_price_series - close_price_series.shift()).abs(),
        (low_price_series - close_price_series.shift()).abs()
    ], axis=1).max(axis=1)
    df['ATR'] = tr.rolling(14).mean()

    df['volatility'] = close_price_series.rolling(20).std() / close_price_series.rolling(20).mean()
    df['price_change_5d'] = close_price_series.pct_change(5)
    df['price_change_20d'] = close_price_series.pct_change(20)
...
def train_advanced_model(ticker='2331.TW', total_timesteps=500000):
    ...
    # LSTM Policy
    policy_kwargs = dict(
        # Removed features_extractor_class=BaseFeaturesExtractor,
        net_arch=[256, 256],
        activation_fn=nn.Tanh,
    )

    model = PPO(
        "MlpPolicy", # Changed from "MlpLstmPolicy" to "MlpPolicy"
        env,
        verbose=1,
        learning_rate=1e-4,
        n_steps=2048,
