...
美股 3110.T (Nitto Denko) AI 交易信号生成器
...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('3110T')


    print(f"🤖 3110.T (Nitto Denko) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    model_filename = "ppo_3110t_improved"
    ...
    try:
        ...
        df = yf.download('3110.T', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ...
            ticker_info = yf.Ticker('3110.TW').info
    ...
    ai_muted = should_mute_ai_signal('3110T', threshold=52)
    ...
    try:
        ...
        profit_positive = evaluate_fundamentals_for_sell(yf, '3110T')['good']
    ...
    weight_calc = DynamicWeightCalculator('3110.T')
    ...
    sentiment_result = calculate_sentiment_score('3110.T', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            ...
            fundamentals = evaluate_fundamentals_for_sell(yf, '3110.T')
        ...
        _growth = calculate_growth_score_adjustment(yf, '3110T')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"   股票: {result['symbol']} (Nitto Denko)")
        ...
        print(f"   {get_model_accuracy_display('3110T')}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 3110t 3110t 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('3110t', '3110t', max_results=5)