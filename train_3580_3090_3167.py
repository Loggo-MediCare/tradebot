"""Train XGBoost + PPO for 3580 台亞半導體, 3090 日電貿, 3167 樺漢"""
...
STOCKS = [
    ('3580', '台亞半導體', 'TWO'),  # new — full train
    ('3090', '日電貿',     'TW'),   # has XGBoost only — add PPO
    ('3167', '樺漢',       'TW'),   # has XGBoost only — add PPO
]
...
def train(code, name, sfx, idx):
    ticker  = f"{code}.{sfx}"
    sfx_low = sfx.lower()
    need_xgb = not any(f.startswith(f'xgb_{code}_') and f.endswith('.pkl') for f in os.listdir('.'))
    need_ppo = not any(f.startswith(f'ppo_{code}_') and f.endswith('.zip') for f in os.listdir('.'))

    print(f"\n{'='*60}")
    print(f"  [{idx}/{len(STOCKS)}] {code} {name} ({ticker})")
    print(f"  Need: {'XGBoost ' if need_xgb else ''}{'PPO' if need_ppo else 'both already exist — retraining'}")
    print(f"{'='*60}")
...
if __name__ == '__main__':
    print("=" * 60)
    print(f"  Training 3580 / 3090 / 3167")
    print(f"  Already running: 2355 3535 8044 (bpn53rbjk) | 1582 (b4zwlvx3l)")
    print(f"  Already done:    8455 8291")
    ...
    print("  RESULTS")
    print(f"{'='*58}")
    print(f"  {'Code':<6} {'Name':<12} {'Ticker':<12} {'XGBoost':>9} {'PPO':>7} {'Winner'}")
    print("  " + "-"*54)
    for r in sorted(results, key=lambda x: max(x['xgb_acc'], x['ppo_acc']), reverse=True):
        ...
        print(f"  {r['code']:<6} {r['name']:<12} {r['ticker']:<12} "
