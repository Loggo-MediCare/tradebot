"""Retrain AMD + INTC, full train QCOM"""
...
STOCKS = [
    ...
    ('INTC', 'Intel',                  '2015-01-01'),
    ('QCOM', 'Qualcomm',               '2015-01-01'),
]
...
for ticker,name,start in STOCKS:
    ...
    bh=(te['close'].iloc[-1]-te['close'].iloc[0])/te['close'].iloc[0]*100
    ...
    print(f"  [PPO]     {ppo_acc*100:.2f}%  |  B&Hold: {bh:+.1f}%")
    results.append({'t':ticker,'xgb':round(xgb_acc*100,2),'ppo':round(ppo_acc*100,2),'bh':round(bh,1)})

print(f"\n{'='*55}\n  RESULTS\n{'='*55}")
print(f"  {'Ticker':<8} {'XGBoost':>9} {'PPO':>7} {'B&Hold':>8}")
for r in results:
    print(f"  {r['t']:<8} {r['xgb']:>8.2f}% {r['ppo']:>6.2f}% {r['bh']:>7.1f}%")
