"""Batch 7 — targeted training (skip already-done stocks)"""
...
# (code, name, sfx, need_xgb, need_ppo)
STOCKS = [
    ('2376', '技嘉',   'TW',  True,  False),  # add XGBoost
    ('3491', '昇達科', 'TWO', True,  False),  # add XGBoost (fix: TWO not TW)
    ('4991', '環宇-KY','TWO', False, True),   # add PPO
    ('2428', '興勤',   'TW',  True,  True),   # full
    ('1597', '直得',   'TW',  True,  True),   # full
    ('2455', '全新',   'TW',  True,  True),   # full
    ('3026', '禾伸堂', 'TW',  True,  True),   # full
    ('2851', '中再保', 'TW',  True,  True),   # full
]
# Skipping (already done): 3090, 6223, 6285, 2327, 6138, 6531
...
def train(code, name, sfx, do_xgb, do_ppo, idx):
    ...
    if do_xgb:
        ...
        try:
            d = json.load(open(f'model_accuracy_{code}.json', encoding='utf-8'))
    ...
    if do_ppo:
        ...
        pm.save(f'ppo_{code}_{sl}_improved')
        ...
        json.dump({'symbol': code, 'ticker': ticker, 'model_type': 'PPO',
                   ...
                   'last_updated': datetime.now().strftime('%Y-%m-%d %H:%M:%S')},
                  open(f'model_accuracy_{code}_{sfx}_ppo.json','w',encoding='utf-8'), indent=2)
...
if __name__ == '__main__':
    print("=" * 60)
    print(f"  BATCH 7 — {len(STOCKS)} stocks (targeted)")
    print(f"  Skipped (already done): 3090 6223 6285 2327 6138 6531")
    ...
    print("\n" + "=" * 66)
    print("  ACCURACY COMPARISON TABLE — Batch 7")
    print("=" * 66)
