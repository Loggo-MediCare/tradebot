"""
批量訓練 10 支台股 XGBoost + PPO
3034 聯詠 / 2454 聯發科 / 2383 台光電 / 8299 群聯 / 6274 台燿
2368 金像電 / 3045 台灣大 / 6223 旺矽 / 3653 健策 / 2330 台積電
"""
import os
...
STOCKS = [
    ('3034.TW',  '3034_TW',  '3034', '聯詠'),
    ('2454.TW',  '2454_TW',  '2454', '聯發科'),
    ('2383.TWO', '2383_TWO', '2383', '台光電'),
    ('8299.TWO', '8299_TWO', '8299', '群聯'),
    ('6274.TWO', '6274_TWO', '6274', '台燿'),
    ('2368.TW',  '2368_TW',  '2368', '金像電'),
    ('3045.TW',  '3045_TW',  '3045', '台灣大'),
    ('6223.TWO', '6223_TWO', '6223', '旺矽'),
    ('3653.TW',  '3653_TW',  '3653', '健策'),
    ('2330.TW',  '2330_TW',  '2330', '台積電'),
]
...
def train_stock(ticker, sym_xgb, sym_ppo, name):
    ...
    xgb_acc = 0.0
    try:
        df_xgb = add_features_xgb(raw_df.copy())
        df_clean = df_xgb.dropna(subset=FEATURE_COLUMNS + ['target'])
        ...
        print(f"  訓練: {train_acc*100:.2f}%  |  測試: {xgb_acc*100:.2f}%")
        joblib.dump(model_xgb, f'xgb_{sym_xgb.lower()}_model.pkl')
        with open(f'model_accuracy_{sym_xgb}.json', 'w', encoding='utf-8') as f:
            json.dump({'symbol': ticker, 'model_type': 'XGBoost',
                       ...
                       'validation_accuracy': float(xgb_acc*100),
        ...
        with open(f'{sym_xgb}_feature_importance.json', 'w', encoding='utf-8') as f:
            json.dump({'ticker': ticker, 'model_type': 'XGBoost', 'model_accuracy': float(xgb_acc),
    ...
    # ── PPO ────────────────────────────────────────────────────────────────────
    print(f"\n  [PPO]")
    ppo_acc = 0.0; ppo_ret = 0.0; ppo_trades = 0
    try:
        df_ppo = add_features_ppo(raw_df.copy())
        ...
        ppo_model.save(f'ppo_{sym_ppo}_improved')
        print(f"  模型已保存: ppo_{sym_ppo}_improved.zip")
        ...
        ppo_trades = test_env.total_trades
        print(f"  ✅ PPO 準確度: {ppo_acc:.2f}% | 回報率: {ppo_ret:.2f}% | 交易: {ppo_trades}")
    ...
    return (ticker, name, xgb_acc, ppo_acc, ppo_ret, ppo_trades)


if __name__ == '__main__':
    print("=" * 70)
    print("批量訓練 10 支台股 XGBoost + PPO")
    ...
    print("-" * 58)
    for ticker, name, xacc, pacc, pret, ptrades in results:
        xtag = '🌟' if xacc >= 0.65 else ('✅' if xacc >= 0.50 else '⚠️')
        print(f"{ticker:<14} {name:<8} {xacc*100:>8.2f}% {xtag}  {pacc:>7.2f}%  {pret:>+8.2f}%")
