"""
美股 LITE (Lumentum Holdings) AI 交易信号生成器
...
def get_trading_signal():
    """生成今日交易信号"""
    # 壓縮標題區塊 - 從6行減少到2行
    accuracy_display = get_model_accuracy_display('LITE')
    print(f"🤖 LITE (Lumentum Holdings) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print("=" * 80)
    ...
    try:
        ...
        df = yf.download('LITE', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ticker_info = yf.Ticker('LITE').info
    ...
    ai_muted = should_mute_ai_signal('LITE', threshold=52)
    if ai_muted:
    ...
    try:
        profit_positive = evaluate_fundamentals_for_sell(yf, 'LITE')['good']
    except Exception:
    ...
    # 7. 初始化动态权重计算器
    weight_calc = DynamicWeightCalculator('LITE')
    ...
    sentiment_result = calculate_sentiment_score('LITE', verbose=True)
    ...
    if action_value > 0.1:
        ...
        if bb_position < 35 and sell_score > 0:
            # 获取公司基本面（超跌时才检查）
            fundamentals = evaluate_fundamentals_for_sell(yf, 'LITE')
        ...
        _growth = calculate_growth_score_adjustment(yf, 'LITE')
...
if __name__ == "__main__":
    ...
    if result:
        print(f"\n📱 快速摘要:")
        print(f"   股票: {result['symbol']} (Lumentum Holdings)")
        ...
        # 顯示AI模型準確度摘要
        print(f"   {get_model_accuracy_display('LITE')}")
    else:
        print("\n❌ 信号生成失败")
