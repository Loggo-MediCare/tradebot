...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('3449.T')
    ...
    model_filename = "ppo_3449_t_improved"
    ...
    try:
        ...
        df = yf.download('3449.T', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ...
            ticker_info = yf.Ticker('3449.T').info
    ...
    ai_muted = should_mute_ai_signal('3449.T', threshold=52)
    ...
    try:
        ...
        profit_positive = evaluate_fundamentals_for_sell(yf, '3449.T')['good']
    ...
    weight_calc = DynamicWeightCalculator('3449.T')
    ...
    sentiment_result = calculate_sentiment_score('3449.T', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            ...
            fundamentals = evaluate_fundamentals_for_sell(yf, '3449.T')
        ...
        _growth = calculate_growth_score_adjustment(yf, '3449.T')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"   {get_model_accuracy_display('3449.T')}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 3449 3449 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('3449', '3449', max_results=5)