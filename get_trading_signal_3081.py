...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('3081')


    print(f"🤖 3081 (聯亞科技) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    model_path = r"ppo_3081_two_improved"
    ...
    try:
        ...
        df = yf.download('3081.TWO', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
    ...
    ai_muted = should_mute_ai_signal('3081.TWO', threshold=52)
    ...
    try:
        ...
        profit_positive = evaluate_fundamentals_for_sell(yf, '3081.TWO')['good']
    ...
    sentiment_result = calculate_sentiment_score('3081.TWO', verbose=True)
    ...
    if action_value > 0.1:
        ...
        if bb_position < 35 and sell_score > 0:
            ...
            fundamentals = evaluate_fundamentals_for_sell(yf, '3081.TWO')
        ...
        _growth = calculate_growth_score_adjustment(yf, '3081.TWO')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"   {get_model_accuracy_display('3081')}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 3081 聯亞科技 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('3081', '聯亞科技', max_results=5)