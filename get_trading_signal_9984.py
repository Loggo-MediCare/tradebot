...
日股 9984.T (SoftBank Group) AI 交易信号生成器
...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('9984.T')


    print(f"🤖 9984.T (SoftBank Group) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    model_path = r"ppo_9984_improved"
    ...
    try:
        ...
        df = yf.download('9984.T', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ...
            ticker_info = yf.Ticker('9984.T').info
    ...
    ai_muted = should_mute_ai_signal('9984.T', threshold=52)
    ...
    try:
        ...
        profit_positive = evaluate_fundamentals_for_sell(yf, '9984.T')['good']
    ...
    weight_calc = DynamicWeightCalculator('9984.T')
    ...
    sentiment_result = calculate_sentiment_score('9984.T', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            ...
            fundamentals = evaluate_fundamentals_for_sell(yf, '9984.T')
        ...
        _growth = calculate_growth_score_adjustment(yf, '9984.T')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"   股票: {result['symbol']} (SoftBank Group)")
        ...
        print(f"   {get_model_accuracy_display('9984.T')}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 9984 9984 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('9984', '9984', max_results=5)