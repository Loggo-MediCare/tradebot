...
def get_trading_signal():
    """生成今日交易信号"""
    print("=" * 80)
    print("🤖 美股 AEVA (Aeva Technologies) AI 交易信号生成器")
    print("=" * 80)
   # print(f("生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    # 1. 加载模型
    model_path = r"C:\Users\Silvi\Projects\trading-bot\ppo_aeva_improved"
    print(f"\n📦 加载 AI 模型: {model_path}")
    ...
    try:
        ...
        df = yf.download('AEVA', period='90d', progress=False, auto_adjust=True)
    ...
    sentiment_result = calculate_sentiment_score('AEVA', verbose=False)
    ...
    # ── Tavily 即時新聞 ─────────────────────────────────────────────────────
    print("\n" + "=" * 80)
    print("🌐 Aeva (AEVA) 即時新聞  (Tavily REST API)")
    print("=" * 80)
    print_tavily_news('AEVA', 'Aeva', max_results=5)
...
if __name__ == "__main__":
    ...
    if result:
        ...
        try:
            import yfinance as yf
            chart_df = yf.Ticker("AEVA").history(period="6mo")
            chart_df.columns = [c.lower() for c in chart_df.columns]
            chart_path = "AEVA_chart.png"
            plot_candlestick(chart_df, "AEVA", save_path=chart_path)
        except Exception as e:
            print(f"   圖表生成失敗: {e}")
