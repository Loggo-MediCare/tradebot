"""
美股 AEVA (Aeva Technologies) AI 交易信号生成器
======================================
 信号: 卖出 (SELL) 20251222
   强度: 1.00 / 1.00
   建议卖出比例: 100%
...
def get_trading_signal():
    """生成今日交易信号"""
    # 壓縮標題區塊 - 從6行減少到2行
    accuracy_display = get_model_accuracy_display('AEVA')
    print(f"🤖 AEVA (Aeva Technologies) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print("=" * 80)

    # 1. 加载模型
    import os
    model_filename = "ppo_aeva_improved"
    ...
    try:
        ...
        df = yf.download('AEVA', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ticker_info = yf.Ticker('AEVA').info
    ...
    ai_muted = should_mute_ai_signal('AEVA', threshold=52)
    if ai_muted:
    ...
    try:
        profit_positive = evaluate_fundamentals_for_sell(yf, 'AEVA')['good']
    except Exception:
    ...
    # 7. 初始化动态权重计算器
    weight_calc = DynamicWeightCalculator('AEVA')
    ...
    sentiment_result = calculate_sentiment_score('AEVA', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            # 获取公司基本面（超跌时才检查）
            fundamentals = evaluate_fundamentals_for_sell(yf, 'AEVA')
        ...
        _growth = calculate_growth_score_adjustment(yf, 'AEVA')
...
if __name__ == "__main__":
    ...
    if result:
        print(f"\n📱 快速摘要:")
        print(f"   股票: {result['symbol']} (Aeva Technologies)")
        ...
        # 顯示AI模型準確度摘要
        print(f"   {get_model_accuracy_display('AEVA')}")
    else:
        print("\n❌ 信号生成失败")
