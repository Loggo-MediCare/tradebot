"""
LITE (Lumentum Holdings) XGBoost 訓練 V2 - 調整目標閾值
"""
import os
...
TICKER = 'LITE'
print(f"訓練 {TICKER} (Lumentum Holdings) XGBoost 模型 V2")
print("=" * 80)
...
try:
    ...
    # 目標：預測未來5天漲幅 > 2.5% (稍微提高閾值)
    df['future_return'] = df['close'].shift(-5) / df['close'] - 1
    df['target'] = (df['future_return'] > 0.025).astype(int)
    ...
    print(f"正類比例: {y_train.sum() / len(y_train) * 100:.1f}%")
    ...
    if test_acc >= 0.50:
        ...
        print(f"\n⚠️ 準確度仍低於50% ({test_acc*100:.2f}%)")
        print("LITE 股票可能較難預測，建議:")
        print("1. 保持當前模型但降低AI信號權重")
        print("2. 或禁用AI信號，僅使用技術指標")
