...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('6196.TW')


    print(f"🤖 6196.TW (帆宣) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    model_path = "ppo_6196_tw_improved"
    ...
    try:
        ...
        df = yf.download('6196.TW', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ...
            ticker_info = yf.Ticker('6196.TW').info
    ...
    ai_muted = should_mute_ai_signal('6196.TW', threshold=52)
    ...
    try:
        ...
        profit_positive = evaluate_fundamentals_for_sell(yf, '6196.TW')['good']
    ...
    weight_calc = DynamicWeightCalculator('6196.TW')
    ...
    sentiment_result = calculate_sentiment_score('6196.TW', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            ...
            fundamentals = evaluate_fundamentals_for_sell(yf, '6196.TW')
        ...
        _growth = calculate_growth_score_adjustment(yf, '6196.TW')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"   {get_model_accuracy_display('6196.TW')}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 6196 帆宣 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('6196', '帆宣', max_results=5)