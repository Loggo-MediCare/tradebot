...
class ImprovedTradingEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.current_step=0; self.balance=self.initial_balance; self.shares_held=0
        self.total_profit=0; self.total_trades=0; self.last_action=0
...
def download_and_prepare_data(ticker, start_date='2015-01-01', end_date='2025-07-31'):
    ...
    print(f"✅ {ticker}: {len(df)} 天  ${float(df['close'].min()):.2f}-${float(df['close'].max()):.2f}")
    return df
