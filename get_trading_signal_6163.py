...
def get_trading_signal():
    ...
    # 顯示AI模型準確度
    accuracy_display = get_model_accuracy_display('6163.TW')
    if accuracy_display:
        print(f"模型準確度: {accuracy_display}")
    print("=" * 80)

    # 1. 加载模型
    model_path = r"C:\Users\Silvi\Projects\trading-bot\ppo_6163_tw_improved"
    print(f"\n📦 加载 AI 模型: {model_path}")
    ...
    try:
        ...
        df = yf.download('6163.TW', period='90d', progress=False, auto_adjust=True)
        ...
        try:
            ticker_info = yf.Ticker('6163.TW').info
    ...
    # 7. 初始化动态权重计算器
    weight_calc = DynamicWeightCalculator('6163.TW')
    ...
    sentiment_result = calculate_sentiment_score('6163.TW', verbose=False)
    ...
    # ── Tavily 即時新聞 ─────────────────────────────────────────────────────
    print("\n" + "=" * 80)
    print("🌐 華電網 (6163.TW) 即時新聞  (Tavily REST API)")
    print("=" * 80)
    print_tavily_news('6163.TW', '華電網', max_results=5)
...
if __name__ == "__main__":
    ...
    if result:
        ...
        try:
            import yfinance as yf
            chart_df = yf.Ticker("6163.TW").history(period="6mo")
            chart_df.columns = [c.lower() for c in chart_df.columns]
            chart_path = "6163_chart.png"
            plot_candlestick(chart_df, "6163.TW", save_path=chart_path)
        except Exception as e:
            print(f"   圖表生成失敗: {e}")
        ...
        # 顯示AI模型準確度摘要
        print(f"   {get_model_accuracy_display('6163.TW')}")
    else:
        print("\n❌ 信号生成失败")
