...
# 2. 下载和处理数据
# ==========================================
def download_and_prepare_data(ticker='ALAB', start_date='2020-01-01', end_date='2025-01-01'):
    """
    🔥 改进 1: 下载更多年份的数据
    NASDAQ代码格式: ALAB (Astera Labs Inc)
    注意: ALAB IPO于2024年,所以从2020年开始尝试下载
    """
    print("=" * 70)
    print(f"下载 {ticker} 股票数据 (NASDAQ - Astera Labs Inc)...")
...
def analyze_feature_importance(df, ticker):
    ...
    plt.title(f"{ticker} Feature Importance (Random Forest Model)", fontweight='bold')
...
if __name__ == "__main__":
    print("改进版 ALAB (Astera Labs Inc - NASDAQ) 交易 AI 训练系统")
    print("=" * 70)
    # 配置
    TICKER = 'ALAB'  # NASDAQ - Astera Labs Inc
    START_DATE = '2020-01-01'  # 从2020年开始 (ALAB IPO于2024年,实际可用数据可能较少)
    END_DATE = '2025-07-31'
    TRAIN_TEST_SPLIT = 0.8
    TOTAL_TIMESTEPS = 100000  # 训练100,000步
    print(f"目标股票: {TICKER} (Astera Labs Inc - NASDAQ)")
