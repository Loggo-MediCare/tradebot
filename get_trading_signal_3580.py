...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('3580')


    print(f"🤖 3580 (台亞半導體) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    model_filename = "ppo_3580_two_improved"
    ...
    try:
        ...
        df = yf.download('3580.TWO', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ...
            ticker_info = yf.Ticker('3580.TWO').info
    ...
    ai_muted = should_mute_ai_signal('3580.TWO', threshold=52)
    ...
    try:
        ...
        profit_positive = evaluate_fundamentals_for_sell(yf, '3580.TWO')['good']
    ...
    weight_calc = DynamicWeightCalculator('3580.TWO')
    ...
    sentiment_result = calculate_sentiment_score('3580.TWO', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            ...
            fundamentals = evaluate_fundamentals_for_sell(yf, '3580.TWO')
        ...
        _growth = calculate_growth_score_adjustment(yf, '3580.TWO')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"   股票: {result['symbol']} (台亞半導體)")
        ...
        print(f"   {get_model_accuracy_display('3580')}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 3580 台亞半導體 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('3580', '台亞半導體', max_results=5)