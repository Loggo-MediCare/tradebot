"""Train only the 3 missing stocks: 2486.TW, 6138.TWO, 3535.TW (PPO only)"""
...
MISSING = [
    ('2486','2486.TW','一詮','tw', True, True),    # needs XGB + PPO
    ('6138','6138.TWO','茂達','two', True, True),   # needs XGB + PPO
    ('3535','3535.TW','晶彩科','tw', False, True),  # PPO only (XGB exists)
]

results = []
for code, ticker, name, suffix, need_xgb, need_ppo in MISSING:
    print(f"\n{'='*60}")
    print(f"  {ticker} ({name})")
    print(f"{'='*60}")
    df = prep(ticker)
    dc = df.dropna(subset=FEAT+['target'])
    print(f"  Data: {len(dc)} clean rows")
    ...
    ppo_acc = None
    if need_ppo:
        env = DummyVecEnv([lambda tr=tr: Env(tr)])
        ...
        env2=Env(te.reset_index(drop=True)); obs,_=env2.reset()
        preds,labels=[],[]
...
for ticker,name,xa,pa in results:
    xs = f'{xa*100:.2f}%' if xa else 'skipped'
    ps = f'{pa*100:.2f}%' if pa else 'skipped'
    w = 'XGBoost' if (xa and pa and xa>=pa) else 'PPO' if pa else 'XGBoost'
    print(f"  {ticker} ({name}): XGB={xs}  PPO={ps}  Winner={w}")
print(f"{'='*60}")
