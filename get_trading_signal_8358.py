...
def get_trading_signal():
    """生成今日交易信号"""
    print("=" * 80)
    print("🤖 台股 8358 (聯亞) AI 交易信号生成器")
    print("=" * 80)
    print(f"生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    # 顯示AI模型準確度
    accuracy_display = get_model_accuracy_display('8358.TWO')
    if accuracy_display:
        print(f"模型準確度: {accuracy_display}")
    print("=" * 80)

    # 1. 加载模型
    model_path = r"C:\Users\Silvi\Projects\trading-bot\ppo_8358_two_improved"
    print(f"\n📦 加载 AI 模型: {model_path}")
    ...
    try:
        ...
        df = yf.download('8358.TWOO', period='90d', progress=False, auto_adjust=True)
        ...
        try:
            ticker_info = yf.Ticker('8358.TWOO').info
            # ===== 基本面資料（PEG / 市值）=====
    ...
    # 7. 初始化动态权重计算器
    weight_calc = DynamicWeightCalculator('8358.TWOO')
    ...
    sentiment_result = calculate_sentiment_score('8358.TWOO', verbose=False)
    ...
    # ── Tavily 即時新聞 ─────────────────────────────────────────────────────
    print("\n" + "=" * 80)
    print("🌐 聯亞 (8358.TWOO) 即時新聞  (Tavily REST API)")
    print("=" * 80)
    print_tavily_news('8358.TWOO', '聯亞', max_results=5)
    ...
    if action_value > 0.1:
        ...
        if bb_position < 35 and sell_score > 0:
            ...
            try:
                ticker_obj = yf.Ticker('8358.TWOO')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        try:
            import yfinance as yf
            chart_df = yf.Ticker("8358.TWO").history(period="6mo")
            chart_df.columns = [c.lower() for c in chart_df.columns]
            chart_path = "8358_chart.png"
            plot_candlestick(chart_df, "8358.TWO", save_path=chart_path)
        except Exception as e:
            print(f"   圖表生成失敗: {e}")

        print(f"\n📱 快速摘要:")
        print(f"   股票: {result['symbol']} (聯亞)")
        ...
        # 顯示AI模型準確度摘要
        print(f"   {get_model_accuracy_display('8358.TWO')}")
    else:
        print("\n❌ 信号生成失败")
