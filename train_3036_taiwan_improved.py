"""
改进版 台股 3036 (文曄 WT Microelectronics) 交易 AI 训练
"""
import os
...
def analyze_feature_importance(df, ticker):
    ...
    import json
    json_data = {'ticker': ticker, 'analysis_date': datetime.now().strftime('%Y-%m-%d'), 'model_accuracy': float(accuracy),
...
if __name__ == "__main__":
    TICKER = '3036.TW'
    print(f"训练 {TICKER} (文曄 WT Microelectronics)")
    ...
    model = train_improved_model(train_df, TICKER, total_timesteps=100000)
    analyze_feature_importance(df, TICKER)
    print(f"\n训练完成! 模型: ppo_{TICKER.lower().replace('.', '_')}_improved.zip")
