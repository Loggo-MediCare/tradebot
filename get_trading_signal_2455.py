"""
台股 2455 (VISUAL PHOTONICS EPITAXY CO LTD) AI 交易信号生成器 (PPO / DQN 自動選最佳)
"""
import os
...
TICKER      = '2455.TW'
TICKER_NAME = '全新光電'
DQN_PATH    = r'C:\Users\Silvi\Projects\trading-bot\2455.TW_improved_anti_overfit_model.keras'
PPO_PATH    = r'C:\Users\Silvi\Projects\trading-bot\ppo_2455_tw_improved'
WINDOW_SIZE  = 5
...
def get_trading_signal():
    print("=" * 80)
    print(f"AI 2455.TW (VISUAL PHOTONICS EPITAXY CO LTD) 交易信号生成器")
    ...
    if model_type == 'PPO':
        ...
        ppo_obs = np.nan_to_num(ppo_obs, nan=0.0, posinf=0.0, neginf=0.0)
    ...
    sentiment_result = calculate_sentiment_score('2455.TW', verbose=False)
    ...
    # ── Tavily 即時新聞 ─────────────────────────────────────────────────────
    print("\n" + "=" * 80)
    print(f"🌐 {TICKER_NAME} ({TICKER}) 即時新聞  (Tavily REST API)")
    print("=" * 80)
    print_tavily_news(TICKER, TICKER_NAME, max_results=5)
    print("\n" + "=" * 80)
...
if __name__ == "__main__":
    ...
    if result:
        ...
        try:
            ...
            plot_candlestick(chart_df, TICKER, save_path='2455_chart.png')
        except Exception as e:
            print(f"图表生成失败: {e}")
        print(f"\n股票: {result['symbol']} (VISUAL PHOTONICS EPITAXY CO LTD)")
