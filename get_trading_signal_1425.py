...
def get_trading_signal():
    ...
    # 1. 加载模型
    model_path = r"C:\Users\Silvi\Projects\trading-bot\ppo_1425_tw_improved"
    print(f"\n📦 加载 AI 模型: {model_path}")
    ...
    try:
        ...
        df = yf.download('1425.TW', period='90d', progress=False, auto_adjust=True)
    ...
    sentiment_result = calculate_sentiment_score('1425.TW', verbose=False)
    ...
    # ── Tavily 即時新聞 ─────────────────────────────────────────────────────
    print("\n" + "=" * 80)
    print("🌐 中工 (1425.TW) 即時新聞  (Tavily REST API)")
    print("=" * 80)
    print_tavily_news('1425.TW', '中工', max_results=5)
...
if __name__ == "__main__":
    ...
    if result:
        ...
        try:
            import yfinance as yf
            chart_df = yf.Ticker("1425.TW").history(period="6mo")
            chart_df.columns = [c.lower() for c in chart_df.columns]
            chart_path = "1425_chart.png"
            plot_candlestick(chart_df, "1425.TW", save_path=chart_path)
        except Exception as e:
            print(f"   圖表生成失敗: {e}")
