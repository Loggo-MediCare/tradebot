...
def get_trading_signal():
    ...
    print(f"🤖 {TICKER} (NVIDIA) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print("=" * 80)
    ...
    if prediction == 1 and buy_prob >= 60:
        ...
        if buy_prob < 50:
            print("\n   ⚠️ 注意: AI 準確度僅 47.89%，建議以技術指標為主")
            print("   技術面分析:")
            if float(latest['rsi']) > 70:
                print("      • RSI 超買，短期可能回調")
            if float(latest['macd']) < float(latest['macd_signal']):
                print("      • MACD 死叉，趨勢轉弱")
            if float(latest['bb_position']) > 80:
                print("      • 價格接近布林帶上軌，注意風險")
