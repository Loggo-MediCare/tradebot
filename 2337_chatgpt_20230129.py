...
def get_trading_signal():
    ...
    # 顯示AI模型準確度
    accuracy_display = get_model_accuracy_display('2337.TW')
    print(f"模型準確度: {accuracy_display}")
    print("=" * 80)

    # 1. 加载模型
    model_path = r"ppo_2337_tw_improved"
    print(f"\n📦 加载 AI 模型: {model_path}")
    ...
    try:
        ...
        df = yf.download('2337.TW', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ticker_info = yf.Ticker('2337.TW').info
    ...
    # 7. 初始化动态权重计算器
    weight_calc = DynamicWeightCalculator('2337.TW')
    ...
    sentiment_result = calculate_sentiment_score('2337.TW', verbose=True)
...
if __name__ == "__main__":
    ...
    if result:
        ...
        # 顯示AI模型準確度摘要
        print(f"   {get_model_accuracy_display('2337.TW')}")
    else:
        print("\n❌ 信号生成失败")