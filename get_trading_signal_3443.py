...
class ImprovedTradingEnv(gym.Env):
    ...
    def step(self, action):
        ...
        next_index = min(self.current_step, len(self.df) - 1)
        next_price = float(self.df.iloc[next_index]['close'])

        new_total_value = self.balance + self.shares_held * next_price
...
def get_trading_signal():
    """生成今日交易信号"""
    print("=" * 80)
    print("🤖 台股 3443 AI 交易信号生成器")
    print("=" * 80)
    print(f"生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    # 顯示AI模型準確度
    accuracy_display = get_model_accuracy_display('3443.TW')
    if accuracy_display:
        print(f"模型準確度: {accuracy_display}")
    print("=" * 80)

    # 1. 加载模型
    model_path = r"C:\Users\Silvi\Projects\trading-bot\ppo_3443_tw_improved"
    print(f"\n📦 加载 AI 模型: {model_path}")
    ...
    try:
        ...
        df = yf.download('3443.TW', period='90d', progress=False, auto_adjust=True)
        ...
        try:
            ticker_info = yf.Ticker('3443.TW').info
    ...
    bb_position_pct = ((current_price - bb_lower) / bb_denom * 100) if bb_denom > 0 else 50.0
    print(f"当前价格位置:    {bb_position_pct:.1f}% (布林带内)")
    ...
    # 7. 初始化动态权重计算器
    weight_calc = DynamicWeightCalculator('3443.TW')
    ...
    sentiment_result = calculate_sentiment_score('3443.TW', verbose=False)
    ...
    # ── Tavily 即時新聞 ─────────────────────────────────────────────────────
    print("\n" + "=" * 80)
    print("🌐 創意電子 (3443.TW) 即時新聞  (Tavily REST API)")
    print("=" * 80)
    print_tavily_news('3443.TW', '創意電子', max_results=5)
...
if __name__ == "__main__":
    ...
    if result:
        ...
        try:
            import yfinance as yf
            chart_df = yf.Ticker("3443.TW").history(period="6mo")
            chart_df.columns = [c.lower() for c in chart_df.columns]
            chart_path = "3443_chart.png"
            plot_candlestick(chart_df, "3443.TW", save_path=chart_path)
        except Exception as e:
            print(f"   圖表生成失敗: {e}")
        ...
        # 顯示AI模型準確度摘要
        print(f"   {get_model_accuracy_display('3443.TW')}")
    else:
        print("\n❌ 信号生成失败")
