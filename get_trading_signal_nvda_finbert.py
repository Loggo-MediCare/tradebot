...
def get_trading_signal():
   ...
   # print(f("生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    ai_muted = should_mute_ai_signal('NVDA_FINBERT', threshold=52)
    if ai_muted:
    ...
    try:
        profit_positive = evaluate_fundamentals_for_sell(yf, 'NVDA_FINBERT')['good']
    except Exception:
    ...
    sentiment_result = calculate_sentiment_score('NVDA_FINBERT', verbose=True)
...
# 加入爆发行情评分调整
        ...
        _growth = calculate_growth_score_adjustment(yf, 'NVDA_FINBERT')
