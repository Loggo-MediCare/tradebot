...
TICKER = 'SNDK'
print(f"優化訓練 {TICKER} (SanDisk) XGBoost 模型")
print("=" * 80)
...
try:
    ...
    # 添加技術指標 (移除 SMA200 以保留更多數據)
    df['sma_10'] = df['close'].rolling(10).mean()
    df['sma_30'] = df['close'].rolling(30).mean()
    df['sma_50'] = df['close'].rolling(50).mean()
    # df['sma_200'] = df['close'].rolling(200).mean()  # 移除以保留數據
    ...
    if test_acc > 0.30:  # Better than original 30%
        joblib.dump(model, 'xgb_sndk_model.pkl')
        print(f"\n✅ 模型已保存: xgb_sndk_model.pkl (準確度提升!)")
        ...
        with open('model_accuracy_SNDK.json', 'w', encoding='utf-8') as f:
        ...
        print(f"\n⚠️ 準確度未提升 ({test_acc*100:.2f}%)，保留原模型")
