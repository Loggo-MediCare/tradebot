...
def get_trading_signal():
    ...
    sentiment_result = calculate_sentiment_score('MU', verbose=False)
...
if __name__ == "__main__":
    ...
    if result:
        ...
        try:
            import yfinance as yf
            chart_df = yf.Ticker("MU").history(period="6mo")
            chart_df.columns = [c.lower() for c in chart_df.columns]
            chart_path = "MU_chart.png"
            plot_candlestick(chart_df, "MU", save_path=chart_path)
        except Exception as e:
            print(f"   圖表生成失敗: {e}")
