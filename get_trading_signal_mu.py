...
def get_trading_signal():
    ...
    if models_conflict:
        if ppo_action > 0.1 and all_up:
            ...
            # 衝突但趨勢不明確 → 趨勢為主
            if all_up and ppo_action > 0:
                action_value = ppo_action * 0.7  # 偏向 PPO
                print(f"  → 趨勢結構支持多頭，偏向 PPO (action={action_value:+.3f})")
    ...
    # 現價跌破 Gamma Flip：造市商處於負Gamma（追漲追跌），不論 MACD/PPO 多看多，
    # 一律強制沒收 BUY 訊號。
    gamma_veto = True  # 保守原則：預設沒收，須即時數據確認安全才放行
    gamma_flip_price = None
    gamma_gate_msg = ""
    try:
        from gamma_flip_gate import check_gamma_flip_gate
        _gflip = check_gamma_flip_gate('MU', current_price)
        gamma_flip_price = _gflip.get('gamma_flip')
        gamma_gate_msg = _gflip.get('message', '')
        gamma_veto = bool(_gflip.get('vetoed'))
    except Exception as _gf_err:
        gamma_gate_msg = f"Gamma Flip 開關檢查失敗，無法驗證籌碼結構（保守原則）→ 強制沒收買進訊號: {_gf_err}"
    # ─────────────────────────────────────────────────────────────────────────
    ...
    if gamma_flip_price is not None:
        print(f"   Gamma Flip 價位: ${gamma_flip_price:.2f}")
    print(f"   {gamma_gate_msg}")
    ...
    if action_value > 0.1 and not gamma_veto:
        signal = "买入 (BUY)"
        signal_emoji = "🟢"
    ...
    elif action_value > 0.1 and gamma_veto:
        signal = "观望 (WAIT - 大戶籌碼否決)"
        signal_emoji = "🔒"
        strength = 0.0
        ...
        print(f"\n{signal_emoji} 信号: {signal}")
        print(f"   原始 AI 動作值: {action_value:+.4f}（PPO/XGBoost 建議買入）")
        print(f"   ⚠️  大戶籌碼結構否決: {gamma_gate_msg}")
        print(f"\n   💡 操作建议:")
        if gamma_flip_price is not None:
            print(f"      • 即使技術面/AI看多，現價仍處於負Gamma區，造市商可能加速追跌")
            print(f"      • 建議等待站回 Gamma Flip ${gamma_flip_price:.2f} 之上，籌碼結構轉為正Gamma後再考慮買入")
        else:
            print(f"      • 即時選擇權籌碼數據不足，無法確認 Gamma 結構是否安全")
            print(f"      • 保守原則：在能確認正Gamma前，暫不追價買入")
        print(f"      • 量化交易不能單看歷史技術指標，須結合即時籌碼結構 (Gamma) 判斷")
        if trailing_stop is not None:
            ts_status = "🔴 已跌破" if current_price < trailing_stop else "🟢 持倉安全"
            print(f"      • 動態止損 (Trail): ${trailing_stop:.2f}  ({trailing_stop_dist_pct:+.1f}%)  {ts_status}")
