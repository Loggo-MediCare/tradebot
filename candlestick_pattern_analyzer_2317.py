...
# 2. 設定與下載
# ================================================
symbol = "2317.TW"  # <--- 阿姨請在這裡改成妳要檢查的股票
# 十字線命名模式:
# "original" = 保留「墓碑十字 (Gravestone Doji)」
# "rename"   = 改名為「長上影十字」
# "hide"     = 不顯示此子型態，視為一般十字線
...
try:
    ...
    for date, row in recent_days.iterrows():
        ...
        # BUG FIX: 必須同時檢查上影線是否夠短 (is_small_upper)
        if is_long_lower and is_small_body and is_small_upper:
            ...
            if trend == '下跌':
                ...
                signals.append(signal_text)
                hanging_man_count += 1
                bearish_signals.append((date_str, '上升趨勢中的槌子', close_price))
        ...
        print("-" * 80)
    print(f"   📊 數據檢查：上影={row['Upper_Shadow']:.2f}, 實體={row['Body_Size']:.2f}, 下影={row['Lower_Shadow']:.2f}")
    # ================================================
    # 5. 統計摘要與投資建議
    # ================================================
    print("\n" + "=" * 80)
    ...
    print(f"   😵 上升趨勢中的槌子：{hanging_man_count} 次 (看跌訊號)")
