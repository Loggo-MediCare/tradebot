"""
Auto-generate missing get_trading_signal_*.py files
"""
import os
import sys
import io
...
model_files = glob.glob("ppo_*_improved.zip")
...
for sf in signal_files:
    # 提取股票代码
    match = re.search(r'get_trading_signal_(.+)\.py', sf)
    if match:
        existing_signals.add(match.group(1).lower())

print(f"Existing {len(existing_signals)} signal files")

# Find models without signal files
missing = []
for mf in model_files:
    # 从 ppo_2317_tw_improved.zip 提取 2317_tw 或 2317.tw
    match = re.search(r'ppo_(.+)_improved\.zip', mf)
    if match:
        ticker_raw = match.group(1)  # 例如 2317_tw, tsla, mu
        # 转换为信号文件格式
        if '_tw' in ticker_raw:
            ticker_signal = ticker_raw.replace('_tw', '')  # 2317
            ticker_yf = ticker_raw.replace('_', '.').upper()  # 2317.TW
        elif '_t' in ticker_raw:  # 日股 9984_t
            ticker_signal = ticker_raw.replace('_t', '')
            ticker_yf = ticker_raw.replace('_', '.').upper()
        elif '_hk' in ticker_raw:
            ticker_signal = ticker_raw.replace('_hk', '')
            ticker_yf = ticker_raw.replace('_', '.').upper()
        elif '_sz' in ticker_raw:
            ticker_signal = ticker_raw.replace('_sz', '')
            ticker_yf = ticker_raw.replace('_', '.').upper()
        else:  # 美股
            ticker_signal = ticker_raw
            ticker_yf = ticker_raw.upper()

        if ticker_signal not in existing_signals:
            missing.append({
                'model_file': mf,
                'ticker_signal': ticker_signal,
                'ticker_yf': ticker_yf,
                'model_path': mf.replace('.zip', '')
            })

print(f"\nNeed to create {len(missing)} signal files:")
for m in missing:
    print(f"  {m['ticker_signal']} -> {m['ticker_yf']}")

# Signal file template (simplified, no FinBERT)
TEMPLATE = '''"""
{ticker_yf} AI 交易信号生成器
...
TICKER = '{ticker_yf}'
MODEL_PATH = r"C:\\Users\\Silvi\\Projects\\trading-bot\\{model_path}"
...
def get_trading_signal():
    ...
    return {{'symbol': TICKER, 'price': price, 'signal': signal, 'action': action_val}}
...
created = 0
for m in missing:
    filename = f"get_trading_signal_{m['ticker_signal']}.py"
    content = TEMPLATE.format(
        ticker_yf=m['ticker_yf'],
...
print(f"\\nDone! Created {created} signal files")
