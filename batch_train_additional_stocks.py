"""
Batch Training Script for Additional Stocks
Taiwan, Hong Kong, and US stocks
"""
import subprocess
import sys
import os

# Stocks with existing scripts
EXISTING_TW_STOCKS = ['3037.TW', '6239.TW']

# New stocks to create scripts for
NEW_STOCKS = {
    '2357.TW': 'Taiwan',
    '6446.TW': 'Taiwan',
    'APLD': 'US',
    'NEM': 'US',
    'GOLD': 'US',  # Barrick Gold
    '02202.HK': 'Hong Kong',
    '01810.HK': 'Hong Kong'
}

def create_training_script(ticker, market='US'):
    """Create a training script for a specific ticker"""
...
def download_and_prepare_data(ticker='{ticker_format}', start_date='2015-01-01', end_date='2025-01-31'):
    print("=" * 70)
    ...
    try:
        ...
        df = df.reset_index()
        print(f"[OK] 成功下载 {{len(df)}} 天的数据")
        print(f"   价格范围: {currency}{{float(df['close'].min()):.2f}} - {currency}{{float(df['close'].max()):.2f}}")
        return df
...
def train_improved_model(df, ticker, total_timesteps=100000):
    ...
    print(f"\\n[OK] 模型已保存: {{model_path}}.zip")
    return model

if __name__ == "__main__":
    print("🚀 改进版 {ticker} 交易 AI 训练系统 ({market})")
    print("=" * 70)
...
'''
    ...
    filename = f'train_{ticker_safe.lower()}_improved.py'
    with open(filename, 'w', encoding='utf-8') as f:
        f.write(script_content)
    return filename

def fix_existing_script(ticker):
    """Fix pandas compatibility in existing scripts"""
    filename = f'train_{ticker.replace(".", "_").lower()}_taiwan_improved.py'
    try:
...
def train_stock(ticker, market='US', is_existing=False):
    """Train a single stock"""
    print(f"\n{'='*70}")
    print(f"Processing {ticker} ({market})...")
    print(f"{'='*70}")

    if is_existing:
        # Fix existing script
        if not fix_existing_script(ticker):
            return False
        script_file = f'train_{ticker.replace(".", "_").lower()}_taiwan_improved.py'
    else:
        # Create new training script
        script_file = create_training_script(ticker, market)
    ...
    try:
        ...
        if result.returncode == 0:
            print(f"[OK] {ticker} training completed successfully")
            return True
        else:
            print(f"[ERROR] {ticker} training failed:")
            print(result.stderr[-500:])  # Last 500 chars of error
            return False
    except subprocess.TimeoutExpired:
        print(f"[ERROR] {ticker} training timed out")
        return False
    except Exception as e:
        print(f"[ERROR] Error training {ticker}: {e}")
        return False

if __name__ == "__main__":
    print("="*70)
    print("Batch Training for Additional Stocks")
    print("="*70)

    results = {}

    # Train existing Taiwan stocks
    print("\n=== Training Existing Taiwan Stocks ===")
    for ticker in EXISTING_TW_STOCKS:
        success = train_stock(ticker, 'Taiwan', is_existing=True)
        results[ticker] = success

    # Train new stocks
    print("\n=== Training New Stocks ===")
    for ticker, market in NEW_STOCKS.items():
        success = train_stock(ticker, market, is_existing=False)
        results[ticker] = success
