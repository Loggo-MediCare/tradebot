"""Train XGBoost + PPO for 7 stocks: 8103,1569,1595,6108,4951,1727,3105"""
...
class TradingEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        super().reset(seed=seed); self.i=0; self.bal=10000.0; self.sh=0; self.profit=0.0
        return self._o(), {}
    def _o(self):
        ...
        return np.array([float(self.sh),float(self.bal),p,
                         ...
                         float(self.profit),(self.sh*p)/tv if tv>0 else 0,
    ...
    def step(self, action):
        ...
        if a < -0.1:
            ...
            s = int((self.bal // p) * a)
            if s > 0: self.bal -= s*p; self.sh += s
        self.profit = (self.bal + self.sh*p) - 10000.0
...
for code, ticker, name, suffix in STOCKS:
    ...
    m.fit(Xt, yt)
    xgb_acc = accuracy_score(ye, m.predict(Xe))
    joblib.dump(m, f'xgb_{code}_{suffix}_model.pkl')
    sup = suffix.upper()
    with open(f'model_accuracy_{code}_{sup}.json', 'w', encoding='utf-8') as f:
    ...
    # PPO — skip if already exists
    ppo_file = f'ppo_{code}_{suffix}_improved.zip'
    ppo_acc = None
    if os.path.exists(ppo_file):
        print(f"  PPO:     skipped (model exists)")
    else:
        env = DummyVecEnv([lambda tr=tr: TradingEnv(tr)])
        ppo = PPO('MlpPolicy', env, verbose=0, learning_rate=0.0003, n_steps=2048,
                  batch_size=64, n_epochs=10, gamma=0.99, ent_coef=0.01)
        ppo.learn(total_timesteps=100000)
        ppo.save(f'ppo_{code}_{suffix}_improved')
        ...
        with open(f'model_accuracy_{code}_{sup}_ppo.json', 'w', encoding='utf-8') as f:
...
print(f"\n{'='*55}\n  COMPARISON SUMMARY\n{'='*55}")
print(f"  {'Ticker':<12} {'Name':<8} {'XGBoost':>9} {'PPO':>12} {'Winner':>9}")
print(f"  {'-'*52}")
for ticker, name, xa, pa in results:
    w = 'XGBoost' if (pa is None or xa >= pa) else 'PPO'
    ps = f'{pa*100:.2f}%' if pa is not None else 'existed'
    print(f"  {ticker:<12} {name:<8} {xa*100:>8.2f}% {ps:>12} {w:>9}")
print(f"{'='*55}")
