"""Batch train XGBoost + PPO — TW stocks batch 5"""
...
# (code, name, sfx)  — 2568 and 3746 skipped (no Yahoo Finance data)
STOCKS = [
    ('1582', '信錦',   'TW'),
    ('2481', '良風',   'TW'),
    ('3023', '信昌電', 'TW'),
...
def train(code, name, sfx, idx):
    ticker  = f"{code}.{sfx}"
    sfx_low = sfx.lower()
    need_xgb = not os.path.exists(f'xgb_{code}_{sfx_low}_model.pkl')
    ...
    if not need_xgb and not need_ppo:
        print(f"  Retraining both models (fresh data)")
...
if __name__ == '__main__':
    print("=" * 60)
    print(f"  BATCH 5 — {len(STOCKS)} TW Stocks")
    print(f"  Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"  Skipped: 2568, 3746 (no Yahoo Finance data)")
    print("=" * 60)
    ...
    print("\n" + "=" * 66)
    print("  ACCURACY COMPARISON TABLE — Batch 5")
    print("=" * 66)
