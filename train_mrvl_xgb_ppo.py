"""Train XGBoost + PPO for MRVL (Marvell Technology) with comparison"""
...
print(f"Training {TICKER} (Marvell Technology) — XGBoost + PPO")
print("=" * 60)
...
joblib.dump(m, 'xgb_mrvl_model.pkl')
with open('model_accuracy_MRVL.json', 'w', encoding='utf-8') as f:
...
class TradingEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.i = 0; self.bal = 10000.0; self.sh = 0; self.profit = 0.0
        return self._obs(), {}
    def _obs(self):
        ...
        return np.array([float(self.sh), float(self.bal), p,
                         ...
                         float(r.get('bb_u', 0)), float(r.get('bb_l', 0)),
...
with open('model_accuracy_MRVL_ppo.json', 'w', encoding='utf-8') as f:
    json.dump({'symbol': TICKER, 'model_type': 'PPO',
               ...
               'last_updated': datetime.now().strftime('%Y-%m-%d %H:%M:%S')}, f, indent=2)
print(f"[PPO] Test accuracy: {ppo_acc*100:.2f}% (buy signals: {sum(preds)}/{len(preds)})")

winner = 'XGBoost' if xgb_acc >= ppo_acc else 'PPO'
print(f"\n{'='*60}")
print(f"  MRVL COMPARISON")
print(f"{'='*60}")
print(f"  XGBoost: {xgb_acc*100:.2f}%")
print(f"  PPO:     {ppo_acc*100:.2f}%")
print(f"  Winner:  {winner} (diff: {abs(xgb_acc-ppo_acc)*100:.2f}%)")
print(f"{'='*60}")
print(f"\n  Signal script: get_trading_signal_mrvl.py (PPO-based, already exists)")
if winner == 'XGBoost':
    print(f"  Note: XGBoost wins — consider switching signal to XGBoost model")
