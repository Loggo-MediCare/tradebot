...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('4541')


    print(f"🤖 4541 (全球傳動) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    model_path = r"ppo_4541_tw_improved"
    ...
    try:
        ...
        df = yf.download('4541.TWO', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ...
            ticker_info = yf.Ticker('4541.TWO').info
    ...
    ai_muted = should_mute_ai_signal('4541.TWO', threshold=52)
    ...
    try:
        ...
        profit_positive = evaluate_fundamentals_for_sell(yf, '4541.TWO')['good']
    ...
    weight_calc = DynamicWeightCalculator('4541.TWO')
    ...
    sentiment_result = calculate_sentiment_score('4541.TWO', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            ...
            fundamentals = evaluate_fundamentals_for_sell(yf, '4541.TWO')
        ...
        _growth = calculate_growth_score_adjustment(yf, '4541.TWO')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"   {get_model_accuracy_display('4541')}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 4541 全球傳動 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('4541', '全球傳動', max_results=5)