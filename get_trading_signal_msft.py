...
from shared_market_checks import evaluate_fundamentals_for_sell, calculate_obv as shared_calculate_obv, money_flow_strength as shared_money_flow_strength, is_us_intraday, calculate_growth_score_adjustment
...
def get_trading_signal():
    """生成今日交易信号"""
    print("=" * 80)
    print("🤖 美股 MSFT AI AI HYBRID PPO + XGBoost 交易信号生成器")
    print("=" * 80)
    print(f"生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    # 顯示AI模型準確度
    accuracy_display = get_model_accuracy_display('MSFT')
    ...
    try:
        model = joblib.load("xgb_msft_model.pkl")
    ...
    ppo_model = None
    try:
        ppo_model = PPO.load("ppo_msft_improved")
        print("✅ PPO 模型加载成功!")
    except Exception as e:
    ...
    try:
        ...
        df = yf.download('MSFT', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ticker_info = yf.Ticker('MSFT').info
    ...
    if ppo_model is not None:
        ...
        try:
            ...
            print(format_ppo_roi_line('MSFT', 'MSFT', 'ppo_msft_improved', df, ppo_action))
        except Exception as e:
    ...
    ai_muted = should_mute_ai_signal('MSFT', threshold=52)
    if ai_muted:
    ...
    is_intraday = is_us_intraday(latest_data['Date'])
    volume_status = (
        "(盤中量比未定)"
        if is_intraday
        else '[放量]' if volume_ratio > 1.5 else '[缩量]' if volume_ratio < 0.7 else '[正常]'
    )
    ...
    print(f"20日平均量:      {int(avg_volume_20):,}  {volume_status}")
    print(f"量比:            {volume_ratio:.2f}x  {volume_status}")

    # 基本面獲利檢查 (供 MA50 斜率模組使用)
    try:
        profit_positive = evaluate_fundamentals_for_sell(yf, 'MSFT')['good']
    except Exception:
    ...
    # 7. 初始化动态权重计算器
    weight_calc = DynamicWeightCalculator('MSFT')
    ...
    sentiment_result = calculate_sentiment_score('MSFT', verbose=True)
    ...
    if action_value > 0.1:
        ...
        # ==========================================
        print("\n" + "=" * 80)
        print("🎯 MSFT Microsoft 专属策略分析")
        print("=" * 80)

        # 1303相关性检查已移除 - MSFT是软件公司，与台湾半导体厂商无直接关联
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            # 获取公司基本面（超跌时才检查）
            fundamentals = evaluate_fundamentals_for_sell(yf, 'MSFT')
        ...
        _growth = calculate_growth_score_adjustment(yf, 'MSFT')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        # 顯示AI模型準確度摘要
        print(f"   {get_model_accuracy_display('MSFT')}")
    else:
        print("\n❌ 信号生成失败")
