...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('6531')


    print(f"🤖 6531 (愛普) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    model_path = r"xgb_6531_tw_model.pkl"


    print(f"\n📦 加载 AI 模型: {model_path} (XGBoost)")
    ...
    try:
        ...
        df = yf.download('6531.TW', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ...
            ticker_info = yf.Ticker('6531.TW').info
    ...
    df_feat['rsi_xgb'] = 100 - (100 / (1 + _g / (_l + 1e-10)))


    df_feat['macd_xgb'] = df_feat['ema_12'] - df_feat['ema_26']


    df_feat['macd_signal_xgb'] = df_feat['macd_xgb'].ewm(span=9).mean()


    df_feat['macd_hist'] = df_feat['macd_xgb'] - df_feat['macd_signal_xgb']


    df_feat['bb_middle_xgb'] = df_feat['close'].rolling(20).mean()


    df_feat['bb_std_xgb'] = df_feat['close'].rolling(20).std()


    df_feat['bb_upper_xgb'] = df_feat['bb_middle_xgb'] + df_feat['bb_std_xgb'] * 2


    df_feat['bb_lower_xgb'] = df_feat['bb_middle_xgb'] - df_feat['bb_std_xgb'] * 2


    df_feat['bb_pct'] = (df_feat['close'] - df_feat['bb_lower_xgb']) / (df_feat['bb_upper_xgb'] - df_feat['bb_lower_xgb'] + 1e-10)
    ...
    df_feat['volume_ma'] = df_feat['volume'].rolling(10).mean()


    df_feat['volume_ratio_xgb'] = df_feat['volume'] / (df_feat['volume_ma'] + 1e-10)
    ...
    FEATURES = ['sma_10','sma_30','sma_50','ema_12','ema_26','rsi_xgb','macd_xgb','macd_signal_xgb','macd_hist',


                'bb_middle_xgb','bb_std_xgb','bb_upper_xgb','bb_lower_xgb','bb_pct','price_change_1','price_change_5','price_change_10',


                'volume_ratio_xgb','high_low_range']
    ...
    print(f"   XGBoost 預測概率: 上漲 {p_up*100:.1f}% / 下跌 {(1-p_up)*100:.1f}%")


    if p_up >= 0.6:


        action_value = p_up          # > 0.1 → BUY path


    elif p_up <= 0.4:


        action_value = -(1 - p_up)   # < -0.1 → SELL path


    else:


        action_value = 0.0           # HOLD path


    ai_muted = should_mute_ai_signal('6531.TW', threshold=52)
    ...
    try:
        ...
        profit_positive = evaluate_fundamentals_for_sell(yf, '6531.TW')['good']
    ...
    weight_calc = DynamicWeightCalculator('6531.TW')
    ...
    sentiment_result = calculate_sentiment_score('6531.TW', verbose=True)
    ...
    print(f"模型输出动作值: {action_value:+.4f}  (XGBoost 上漲概率: {p_up*100:.1f}%)")
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            ...
            fundamentals = evaluate_fundamentals_for_sell(yf, '6531.TW')
        ...
        _growth = calculate_growth_score_adjustment(yf, '6531.TW')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"   {get_model_accuracy_display('6531')}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 6531 愛普 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('6531', '愛普', max_results=5)