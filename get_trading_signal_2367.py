...
# #     # 1. 加载模型
# #     model_path = r"C:\Users\Silvi\Projects\trading-bot\ppo_2367_tw_improved"
# #     print(f"\n📦 加载 AI 模型: {model_path}")
...
# #         df = yf.download('2367.TW', period='90d', progress=False, auto_adjust=True)
...
# #         try:
# #             ticker_info = yf.Ticker('2367.TW').info
# #             # ===== 基本面資料（PEG / 市值）=====
...
# #     # 7. 初始化动态权重计算器
# #     weight_calc = DynamicWeightCalculator('2367.TW')
...
# #     sentiment_result = calculate_sentiment_score('2367.TW', verbose=False)
...
# #         # 顯示AI模型準確度摘要
# #         print(f"   {get_model_accuracy_display('2367.TW')}")
# #     else:
# #         print("\n❌ 信号生成失败")
# """
# 台股 2367 (燿華) AI 交易信号生成器
...
#     """
#     偵測「年線級別」的大型技術型態 (針對 2367.TW 這類長線股)
#     window=250 代表查看過去一年的交易日
#     """
#     # 確保數據夠長
#     if len(df) < window:
#         # 如果數據不夠，就用全部數據
...
#     dates = recent_data.index.values
    
#     patterns = {
#         'w_bottom': False,
#         'rounding_bottom': False, # 鍋底
#         'description': [],
#         'score_bonus': 0
#     }

#     # ==========================================
#     # 1. 偵測 鍋底 (Rounding Bottom) - 使用數學「二次曲線」
#     # ==========================================
#     # 我們嘗試用一個拋物線公式 (y = ax^2 + bx + c) 來擬合股價
#     # 如果 a > 0，代表開口向上 (鍋底)
    
#     x = np.arange(len(prices))
#     # 進行二次多項式擬合
#     coeffs = np.polyfit(x, prices, 2)
#     a, b, c = coeffs # a是曲率, b是斜率, c是截距
    
#     # 計算擬合度 (R-squared) 來確定它真的像個鍋子，而不是亂七八糟的線
#     p = np.poly1d(coeffs)
#     yhat = p(x)
...
#     # 1. a > 0 (開口向上)
#     # 2. R^2 > 0.7 (形狀要夠像圓弧)
#     # 3. 最低點發生在中間區域 (不是單純的下跌或上漲)
#     vertex_x = -b / (2 * a) # 拋物線頂點 X 座標
    
#     is_concave_up = a > 0
#     is_good_fit = r_squared > 0.6 # 放寬一點，0.6 即可
#     vertex_in_range = 0.2 * len(prices) < vertex_x < 0.8 * len(prices) # 底部要在中間
    
#     if is_concave_up and is_good_fit and vertex_in_range:
#         # 再次確認現在價格是否在右邊且高於底部 (右側上漲中)
#         current_price = prices[-1]
#         bottom_price = np.min(yhat)
        
#         if current_price > bottom_price * 1.05: # 比底部漲了至少 5%
#             patterns['rounding_bottom'] = True
#             patterns['description'].append(f"大鍋底 (Rounding Bottom) 成形: 年線支撐強勁 (R²={r_squared:.2f})")
#             patterns['score_bonus'] += 25 # 這種長線大底非常穩，加分重一點

#     # ==========================================
#     # 2. 偵測 大型 W 底 (Macro Double Bottom)
#     # ==========================================
#     # 尋找顯著的低點 (Prominence 參數過濾雜訊)
#     # distance=20 代表兩個低點至少要隔 20 天 (一個月)
#     troughs, _ = find_peaks(-prices, distance=20) 
    
#     if len(troughs) >= 2:
#         # 取最低的兩個點 (不一定是最後兩個，而是全域最低的兩個)
#         sorted_troughs = sorted(troughs, key=lambda x: prices[x])
#         low1_idx = sorted_troughs[0] # 絕對低點
        
#         # 找第二低點 (必須在時間上跟第一低點分開至少 40 天)
#         low2_candidates = [t for t in troughs if abs(t - low1_idx) > 40]
        
#         if low2_candidates:
#             # 找到第二個低點
#             low2_candidates.sort(key=lambda x: prices[x])
#             low2_idx = low2_candidates[0]
            
#             # 確保順序: 左腳(Left Leg) -> 右腳(Right Leg)
#             left_idx, right_idx = sorted((low1_idx, low2_idx))
#             left_price = prices[left_idx]
#             right_price = prices[right_idx]
            
#             # W底條件：
#             # A. 兩個低點高度差不多 (誤差 5% 內)
#             price_diff_pct = abs(left_price - right_price) / left_price
            
#             # B. 兩個低點中間有反彈過 (頸線)
#             neck_slice = prices[left_idx:right_idx]
#             neck_high = np.max(neck_slice)
#             neck_depth = (neck_high - min(left_price, right_price)) / min(left_price, right_price)
            
#             # C. 現在價格必須高於右腳
#             current_uptrend = prices[-1] > right_price
            
#             if price_diff_pct < 0.08 and neck_depth > 0.05 and current_uptrend:
#                 # 這是很棒的 W 底
#                 patterns['w_bottom'] = True
#                 patterns['description'].append(f"大型 W 底: 雙腳支撐 ${left_price:.1f} / ${right_price:.1f}")
#                 patterns['score_bonus'] += 30 # W底是大漲訊號
...
#     if patterns['rounding_bottom']:
#         output.append("✅ 偵測到鍋底型態 (Rounding Bottom):")
#         output.append("   • 長線底部逐漸形成")
#         output.append("   • 年線級別支撐強勁")
    
#     if not patterns['w_bottom'] and not patterns['rounding_bottom']:
...
#         df = yf.download('2367.TW', period='2y', progress=False, auto_adjust=True)
...
def detect_macro_structure(df, window=250):
    ...
    # ==========================================
    # 1. 偵測 J-Curve / Hockey Stick (針對 2367 這種噴發股)
    # 邏輯：長期低檔盤整 + 近期垂直拉升
    # ==========================================
    n = len(prices)
    # 切分數據：前 70% (盤整區) vs 後 15% (噴發區)
    if n > 100:  # 確保有足夠數據
    ...
    if not patterns['j_curve']: # 如果已經是 J-Curve 就不需要重複偵測
        ...
        try:
            ...
            # 修正：當股價暴漲時，R^2 會下降。
            # 如果 a > 0 (開口向上) 且 目前價格強勢，我們允許較低的 R^2
            is_concave_up = a > 0
            
            # 動態調整 R^2 門檻：如果現在漲很兇，R^2 只要 0.4 就好；如果漲得慢，R^2 要 0.6
    ...
    # ==========================================
    troughs, _ = find_peaks(-prices, distance=15) # 稍微縮短距離限制
    
    if len(troughs) >= 2:
        # 為了抓到 2367 這種大 W，我們找「過去一年內最低」的幾個點
        sorted_indices = sorted(troughs, key=lambda i: prices[i])
        lowest_indices = sorted_indices[:min(3, len(sorted_indices))] # 取前三個低點來配對
