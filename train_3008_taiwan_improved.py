...
def download_and_prepare_data(ticker, start_date='2015-01-01', end_date='2025-07-31'):
    ...
    if df.empty: print(f"❌ 无法下载 {ticker}"); return None
    df=df.rename(columns={'Close':'close','Volume':'volume','Open':'open','High':'high','Low':'low'}).reset_index()
    print(f"✅ {ticker}: {len(df)} 天  NT${float(df['close'].min()):.0f} - NT${float(df['close'].max()):.0f}")
    return df
...
def analyze_feature_importance(df, ticker):
    ...
    acc=accuracy_score(yv,rf.predict(Xv)); print(f"模型預測準確率: {acc:.4f}")
    ...
    with open(f'{ticker}_feature_importance.json','w',encoding='utf-8') as f:
        json.dump({'ticker':ticker,'analysis_date':datetime.now().strftime('%Y-%m-%d'),'model_accuracy':float(acc),
                   'feature_importance':{r['Feature']:float(r['Importance']) for _,r in fi.iterrows()}},f,indent=2)
    print(f"✅ {ticker}_feature_importance.json")

def train_model(df, ticker, total_timesteps=100000):
    print(f"\n开始训练 {ticker} (大立光) 交易模型")
    ...
    print(f"✅ 模型已保存: {model_path}.zip"); return model
