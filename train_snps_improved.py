...
class ImprovedTradingEnv(gym.Env):
    ...
    def _get_observation(self):
        ...
        cp = float(row['close']); tv = self.balance + self.shares_held * cp
    ...
    def step(self, action):
        ...
        otv = self.balance + self.shares_held * cp
        if action < -0.1:
            ...
            if s > 0: self.balance -= s * cp; self.shares_held += s; self.total_trades += 1
        ntv = self.balance + self.shares_held * cp
        self.total_profit = ntv - self.initial_balance
...
def download_and_prepare_data(ticker, start_date='2015-01-01', end_date='2025-07-31'):
    ...
    print(f"✅ {len(df)} 天  ${float(df['close'].min()):.2f}-${float(df['close'].max()):.2f}")
    return df
