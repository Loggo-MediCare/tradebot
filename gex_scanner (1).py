...
def fetch_options_chain(symbol, expiry=None):
    ...
    if expiry is None:
        # 自動跳過已過期的到期日（今天收盤後 T=0 數據不準）
        today_str = datetime.now().strftime('%Y-%m-%d')
        future = [e for e in all_expiries if e >= today_str]
        # 若今天是到期日且已接近收盤（16:00 EST），優先選下一個
        if future and future[0] == today_str:
            now_hour = datetime.now().hour
            target_expiry = future[1] if len(future) > 1 and now_hour >= 15 else future[0]
        else:
