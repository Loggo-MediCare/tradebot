...
def get_trading_signal():
    ...
    print(f"🤖 NVDA (NVIDIA) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print("=" * 80)

    # 1. 加载模型
    import os
    model_filename = "ppo_nvda_improved"
    ...
    try:
        ...
        df = yf.download('NVDA', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ticker_info = yf.Ticker('NVDA').info
    ...
    ai_muted = should_mute_ai_signal('NVDA', threshold=52)
    if ai_muted:
    ...
    if is_intraday:
        ...
        volume_direction = (
            '價漲量增' if candle_direction == 'up' and volume_ratio >= 1.2 else
            '價跌量增' if candle_direction == 'down' and volume_ratio >= 1.2 else '中性'
        )
    ...
    if candle_direction == 'down' and volume_ratio >= 1.2 and not is_intraday:
        print("⚠️  風險提示: 放量下跌，短線賣壓偏強")

    # 基本面獲利檢查 (供 MA50 斜率模組使用)
    try:
        profit_positive = evaluate_fundamentals_for_sell(yf, 'NVDA')['good']
    except Exception:
    ...
    # 7. 初始化动态权重计算器
    weight_calc = DynamicWeightCalculator('NVDA')
    ...
    sentiment_result = calculate_sentiment_score('NVDA', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            # 获取公司基本面（超跌时才检查）
            fundamentals = evaluate_fundamentals_for_sell(yf, 'NVDA')
        ...
        _growth = calculate_growth_score_adjustment(yf, 'NVDA')
...
if __name__ == "__main__":
    ...
    if result:
        print(f"\n📱 快速摘要:")
        print(f"   股票: {result['symbol']} (NVIDIA)")
