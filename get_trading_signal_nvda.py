...
def get_trading_signal():
    """生成今日交易信号"""
    print("=" * 80)
    print("🤖 美股 NVDA (NVIDIA) AI 交易信号生成器")
    print("=" * 80)
   # print(f("生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    sentiment_result = calculate_sentiment_score('NVDA', verbose=False)
...
if __name__ == "__main__":
    ...
    if result:
        ...
        try:
            import yfinance as yf
            chart_df = yf.Ticker("NVDA").history(period="6mo")
            chart_df.columns = [c.lower() for c in chart_df.columns]
            chart_path = "NVDA_chart.png"
            plot_candlestick(chart_df, "NVDA", save_path=chart_path)
        except Exception as e:
            print(f"   圖表生成失敗: {e}")
