"""
批量訓練天然氣/石化股 XGBoost 模型
1314.TW 中石化, 8908.TWO 欣雄, 9931.TW 欣高,
8917.TWO 欣泰, 6505.TW 台塑化, 9918.TW 欣天然
"""
import os
...
# 股票列表
STOCKS = [
    {'ticker': '1314.TW', 'name': '中石化', 'code': '1314'},
    {'ticker': '8908.TWO', 'name': '欣雄', 'code': '8908'},
    {'ticker': '9931.TW', 'name': '欣高', 'code': '9931'},
    {'ticker': '8917.TWO', 'name': '欣泰', 'code': '8917'},
    {'ticker': '6505.TW', 'name': '台塑化', 'code': '6505'},
    {'ticker': '9918.TW', 'name': '欣天然', 'code': '9918'},
]
...
def train_stock(ticker, name, code):
    """訓練單支股票"""
    print("\n" + "=" * 80)
    ...
    try:
        ...
        # 保存特徵重要性
        suffix = 'TW' if '.TW' in ticker else 'TWO'
        importance_dict = {
            'ticker': ticker,
            'company': name,
            'model_type': 'XGBoost',
        ...
        with open(f'{code}_{suffix}_feature_importance.json', 'w', encoding='utf-8') as f:
...
if __name__ == "__main__":
    ...
    for stock in STOCKS:
        result = train_stock(stock['ticker'], stock['name'], stock['code'])
        if result:
    ...
    for result in results:
        ...
        print(f"{status_emoji} {result['code']:6s} {result['name']:15s} {result['accuracy']*100:6.2f}% ({result['threshold']*100:.1f}% 閾值)")
