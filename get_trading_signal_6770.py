...
"""
美股 6770.TW (6770.TWle) AI 交易信号生成器 V2
台股 6770.TW (南亞) AI 交易信号生成器 V2
========================================
=======
...
# 主要交易信號生成
# ==========================================
    fmt.print_header("🤖 美股 6770.TW (6770.TWle) AI 交易信号生成器 V2")
def get_trading_signal():
    ...
    # ========== 標題區 ==========
    fmt.print_header("🤖 台股 6770.TW (南亞) AI " \
    "交易信号生成器 V2")

    fmt.print_header("🤖 美股 6770.TW (6770.TWle) AI 交易信号生成器 V2")
    print(f"   📅 生成時間: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"   📊 模型準確度: {get_model_accuracy_display('6770.TW')}")
    ...
    try:
        ...
        df = yf.download('6770.TW', period='90d', progress=False, auto_adjust=True)
        ...
        try:
            ticker_info = yf.Ticker('6770.TW').info
    ...
    fmt.print_metric("股票代碼", "6770.TW (6770.TWle)")
    fmt.print_metric("當前價格", f"${current_price:.2f}")
    fmt.print_metric("股票代碼", "6770.TW (南亞)")
    ...
    sentiment_result = calculate_sentiment_score('6770.TW', verbose=False)
    ...
    # ── Tavily 即時新聞 ─────────────────────────────────────────────────────
    print("\n" + "=" * 80)
    print("🌐 力積電 (6770.TW) 即時新聞  (Tavily REST API)")
    print("=" * 80)
    print_tavily_news('6770.TW', '力積電', max_results=5)
    ...
    # ========== 生成交易建議 ==========
    weight_calc = DynamicWeightCalculator('6770.TW')
    ...
    if action_value > 0.1:
        ...
        buy_score, signal_override, buy_reasons, buy_warnings, buy_metadata, sentiment_result = \
            calculate_enhanced_buy_score_with_sentiment(
                ...
                ai_action=action_value, buy_weights=buy_weights, symbol='6770.TW'
            )
    ...
    print("║        📱 快速摘要                   ║")
    print("╠" + "═" * 40 + "╣")
    print(f"║  股票: 6770.TW (6770.TWle)                 ║")
    print(f"║  股票: 6770.TW (南亞)                ║")
