"""Train XGBoost + PPO for 7751.TWO (竑騰)"""
...
if len(dc) < 50:
    print("ERROR: insufficient data"); import sys; sys.exit(1)
...
m=xgb.XGBClassifier(max_depth=4,learning_rate=0.05,n_estimators=150,
...
joblib.dump(m,'xgb_7751_two_model.pkl')
with open('model_accuracy_7751_TWO.json','w',encoding='utf-8') as f:
    json.dump({'symbol':TICKER,'model_type':'XGBoost','training_accuracy':float(accuracy_score(yt,m.predict(Xt))*100),
...
ppo.save('ppo_7751_two_improved')
...
with open('model_accuracy_7751_TWO_ppo.json','w',encoding='utf-8') as f:
...
print(f"\n{'='*60}")
print(f"  7751.TWO (竑騰) COMPARISON")
print(f"{'='*60}")
print(f"  XGBoost: {xgb_acc*100:.2f}%")
