...
TICKER = '9136.TW'
print(f"訓練 9136.TW (巨騰-DR) XGBoost 模型")
print("=" * 80)
...
try:
    ...
    joblib.dump(model, 'xgb_9136_tw_model.pkl')
    print(f"\n模型已保存: xgb_9136_tw_model.pkl")

    accuracy_data = {
        'symbol': '9136.TW', 'model_type': 'XGBoost',
        'training_accuracy': float(train_acc * 100),
        'validation_accuracy': float(test_acc * 100),
        'backtest_accuracy': float(test_acc * 100),
        'last_updated': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    }
    with open('model_accuracy_9136_TW.json', 'w', encoding='utf-8') as f:
    ...
    fi_data = {
        'ticker': '9136.TW', 'model_type': 'XGBoost',
    ...
    with open('9136_TW_feature_importance.json', 'w', encoding='utf-8') as f:
