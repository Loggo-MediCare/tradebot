"""Train 7610 聯友金屬 + 7788 兆川精密 — recently listed, short-history adapted"""
...
STOCKS = [
    ('7610', '聯友金屬', 'TW'),   # OTC stock but Yahoo only has 7610.TW; listed Aug 2025
    ('7788', '兆川精密', 'TW'),   # same — Yahoo uses .TW
]

END_DATE        = '2026-05-01'
TOTAL_TIMESTEPS = 50_000   # fewer steps for limited data

# Short-history feature set — excludes sma_200 (needs 200 bars)
...
def train(code, name, sfx, idx):
    ticker = f"{code}.{sfx}"
    sfx_low = sfx.lower()
    print(f"\n{'='*60}")
    print(f"  [{idx}/{len(STOCKS)}] {code} {name} ({ticker}) — short-history mode")
    print(f"{'='*60}")
    ...
    print(f"  Data: {len(df)} days  ⚠️  Limited history — treat signals as low-confidence")

    df = build_features(df)
    dc = df.dropna(subset=FEAT + ['target'])
    if len(dc) < 30:
        print(f"  [SKIP] Only {len(dc)} clean rows after feature build"); return None
    ...
    xm = xgb.XGBClassifier(max_depth=3, learning_rate=0.05, n_estimators=100,
    ...
    with open(f'model_accuracy_{code}_{sfx}.json', 'w', encoding='utf-8') as f:
        json.dump({'symbol': code, 'ticker': ticker, 'model_type': 'XGBoost',
                   ...
                   'note': 'Short-history model — limited data, treat with caution',
    ...
    pm = PPO('MlpPolicy', env, verbose=0, learning_rate=0.0003,
             n_steps=min(2048, len(tr)//2), batch_size=32, n_epochs=10,
...
if __name__ == '__main__':
    print("=" * 60)
    print("  Training 7610 + 7788 (short-history mode, no sma_200)")
    print(f"  Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 60)
    results = []
    for idx, (code, name, sfx) in enumerate(STOCKS, 1):
        try:
            r = train(code, name, sfx, idx)
    ...
    for r in results:
        ...
        print(f"  {r['code']} {r['name']} ({r['ticker']})")
        print(f"    XGBoost: {r['xgb_acc']:.2f}%  PPO: {r['ppo_acc']:.2f}%  Winner: {r['winner']} {star}")
