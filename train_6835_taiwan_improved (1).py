"""
台股 6835 (青新創 Qingxin) 交易 AI 训练 - Medical/industrial waste management (Cleanaway subsidiary)
"""
import os
...
class ImprovedTradingEnv(gym.Env):
    ...
    def step(self, action):
        ...
        old_total = self.balance + self.shares_held * price
        ...
        reward += -0.005 if self.balance > old_total * 0.9 else 0
        self.current_step += 1
...
def analyze_features(df, ticker):
    ...
    with open(f'{ticker}_feature_importance.json', 'w') as f:
        json.dump({'ticker': ticker, 'accuracy': acc, 'features': dict(zip(features, rf.feature_importances_.tolist()))}, f, indent=2)

if __name__ == "__main__":
    TICKER = '6835.TW'
    print(f"训练 {TICKER} (青新創 Qingxin - Waste Management)")
    ...
    print(f"完成! 模型: ppo_{TICKER.lower().replace('.', '_')}_improved.zip")
