...
# Gamma Regime: deadband + debounce
try:
    from regime_module import (
        get_confirmed_regime, regime_label,
        regime_is_negative, regime_is_positive
    )
    HAS_REGIME_MODULE = True
except ImportError:
    HAS_REGIME_MODULE = False
...
# Squeeze Score 計算
# ==========================================
def calc_squeeze_score(gex, spot, regime):
    score     = 0
    reasons   = []
    warnings  = []
    gex_data  = gex
    ...
    # Step A：Gamma 狀態 — 優先用 regime 模組（deadband+debounce），
    #          fallback 到簡單符號判定
    if regime is not None:
        currently_negative_gamma = regime_is_negative(regime) \
            if HAS_REGIME_MODULE else (net_gex < -200)
        is_flip_zone = regime.get('is_flip_zone', False)
    else:
        currently_negative_gamma = (net_gex < -200)
        is_flip_zone = abs(net_gex) < 200
...
# 信號解讀
# ==========================================
def interpret_signal(score, gex, spot, regime):
    """
    ...
    # 用 regime 模組的確認狀態（有 deadband + debounce）
    if regime is not None:
        is_neg_gamma = regime_is_negative(regime) if HAS_REGIME_MODULE \
                       else (net < -200)
        is_fz        = regime.get('is_flip_zone', False)
        dyn_db       = regime.get('dynamic_deadband', 200)
    else:
        is_neg_gamma = (net < -200)
        is_fz        = abs(net) < 200
        dyn_db       = 200
    if is_fz:
        gamma_label = f"FLIP ZONE（淨GEX {net:+.0f}M，門檻±{dyn_db:.0f}M）"
    else:
        gamma_label = f"負 Gamma（淨 GEX {net:.0f}M）" if is_neg_gamma \
                      else f"正 Gamma（淨 GEX {net:.0f}M）"
...
def print_gex_report(symbol, expiry_choice=None):
    ...
    # ── 3.5 Gamma Regime（Deadband + Debounce）────────────────────────────────
    net_gex_val = gex['total_net_gex']
    if HAS_REGIME_MODULE:
        regime = get_confirmed_regime(
            symbol, net_gex_val,
            deadband=200,
            confirm_needed=2,
        )
        regime_str    = regime_label(regime)
        is_neg_regime = regime_is_negative(regime)
        is_fz         = regime['is_flip_zone']
        pending_str   = (
            f" [待確認→{regime['pending_regime']} {regime['pending_count']}/{regime['confirm_needed']}次]"
            if regime['pending_regime'] else ''
        )
        print(f"\n   🎯 Gamma Regime: {regime_str}{pending_str}")
        if is_fz:
            print(f"      ⚡ FLIP ZONE：淨GEX {net_gex_val:+.0f}M 在門檻±{regime['dynamic_deadband']:.0f}M 內，訊號不確定")
    else:
        # fallback: 簡單符號判定
        is_neg_regime = (net_gex_val < -200)
        is_fz         = abs(net_gex_val) < 200
        regime_str    = '正 Gamma（穩定）' if not is_neg_regime else '負 Gamma（追漲追跌）'
        if is_fz: regime_str = f'FLIP ZONE（{net_gex_val:+.0f}M）'
        regime = {'confirmed_regime': 'NEGATIVE' if is_neg_regime else 'POSITIVE',
                  'is_flip_zone': is_fz, 'dynamic_deadband': 200,
                  'pending_regime': None, 'pending_count': 0, 'confirm_needed': 2}

    # ── 4. Squeeze Score
    score, reasons, warnings = calc_squeeze_score(gex, spot, regime)
    signal_label, verdict     = interpret_signal(score, gex, spot, regime)
    ...
    # regime_str already computed above from regime module

    print(f"  {'水平':<22} {'價格':>10}  {'說明'}")
    print(f"  {'─'*22} {'─'*10}  {'─'*40}")
    print(f"  {'🔴 Call Wall':<22} ${cw:>9.2f}  OI {cw_oi:,} 口  最大上方阻力  {above_cw}")
    print(f"  {'📍 當前股價':<22} ${spot:>9.2f}  ← 你在這裡  [{regime_str}]")
    if gf_found and gf is not None:
