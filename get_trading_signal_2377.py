...
def get_trading_signal():
    acc_display = get_model_accuracy_display(TICKER)
    print(f'\n🤖 {NAME}  | 準確度: {acc_display} | {datetime.now().strftime("%Y-%m-%d %H:%M")}')
    print(sep)
    ...
    try:
        ...
        if t_mean:
            print(f'   📊 分析師目標價: NT${t_mean:.2f} (平均) / NT${t_high:.2f} (最高)')
        print(f'   📊 分析師評級: {rec}')
    except Exception:
        pass
    ...
    print_tavily_news_tw('2377', '微星科技')
