...
def calculate_obv(df):
    """計算 On-Balance Volume (OBV) 資金流指標"""
    obv = [0]
...
def detect_memory_cycle_phase(df):
    ...
    late_cycle = (
        ...
        ma50.diff().iloc[-1] < ma50.diff().iloc[-5]
    )
...
def explosive_stock_scanner(df):
    """
    爆發性股票掃描器 - 主升段評分系統
    評分標準：
    - 資金流入 (40分)
    ...
    if strong_money:
        score += 40
        reasons.append("資金強力流入 (OBV創高 + 放量)")
    
    if accelerating:
        score += 35
        reasons.append("趨勢進入加速段")
    
    if cycle_phase == "EARLY_UPCYCLE":
        score += 25
        reasons.append("產業週期主升段")
...
# 市場批量掃描功能
# ==========================================
def scan_market(stock_data_dict):
    """
    批量掃描多支股票，找出主升段爆發標的
    
    參數:
        stock_data_dict: dict, 格式為 {symbol: DataFrame}
        
    返回:
        list of dict, 按評分由高到低排序的爆發股票列表
    """
    results = []
    for symbol, df in stock_data_dict.items():
        try:
            signal = explosive_stock_scanner(df)
            if signal["explosive"]:
                results.append({
                    "symbol": symbol,
                    "score": signal["score"],
                    "cycle": signal["cycle_phase"],
                    "volume_ratio": round(signal["volume_ratio"], 2),
                    "reasons": signal["reasons"]
                })
        except Exception as e:
            print(f"⚠️  掃描 {symbol} 時發生錯誤: {e}")
            continue
...
def download_stock_data(symbols, period='1y'):
    ...
    參數:
        symbols: list, 股票代碼列表，例如 ['2330.TW', '2317.TW', '2454.TW']
        period: str, 數據周期，預設為1年
        
    返回:
        dict, 格式為 {symbol: DataFrame}
    """
    import yfinance as yf
    stock_data = {}
    
    print(f"\n📊 開始下載 {len(symbols)} 支股票數據...")
    ...
    print(f"✅ 成功下載 {len(stock_data)}/{len(symbols)} 支股票\n")
    return stock_data
...
def print_scan_results(results):
    ...
    print("=" * 80)
    print(f"🔥 主升段爆發股票掃描結果 (共 {len(results)} 支)")
    print("=" * 80)
    
    for i, stock in enumerate(results, 1):
        print(f"\n#{i} {stock['symbol']}")
        print(f"   評分: {stock['score']}/100 {'🔥🔥🔥' if stock['score'] >= 90 else '🔥🔥' if stock['score'] >= 70 else '🔥'}")
        print(f"   週期: {stock['cycle']}")
        print(f"   量比: {stock['volume_ratio']}x")
        print(f"   理由:")
...
def get_trading_signal():
    """生成今日交易信号"""
    print("=" * 80)
    print("🤖 台股 2330 (台積電) AI 交易信号生成器")
    print("=" * 80)
    print(f"生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    # 顯示AI模型準確度
    accuracy_display = get_model_accuracy_display('2330.TW')
    ...
    try:
        ...
        df = yf.download('2330.TW', period='1y', progress=False, auto_adjust=True)  # 改為1年以計算MA200
        ...
        try:
            ticker_info = yf.Ticker('2330.TW').info
    ...
    # 7.5 🔥 主升段爆發行情掃描器（增強版評分系統）
    print("\n" + "=" * 80)
    print("🚀 主升段爆發行情掃描器 (Explosive Stock Scanner)")
    print("=" * 80)
    
    explosion = explosive_stock_scanner(df)
    
    print(f"爆發性評分: {explosion['score']} / 100")
    print(f"評分等級: ", end="")
    if explosion['score'] >= 90:
        print("🔥🔥🔥 極度強勢")
    elif explosion['score'] >= 70:
        print("🔥🔥 高度爆發")
    elif explosion['score'] >= 50:
        print("🔥 中等爆發")
    else:
        print("⚪ 非爆發階段")
    
    print(f"\n評分明細:")
    print(f"├─ 資金流入強度: {'✅ 強勁 (+40分)' if explosion['money_inflow'] else '❌ 弱 (0分)'}")
    print(f"├─ 趨勢加速狀態: {'✅ 加速中 (+35分)' if explosion['trend_accelerating'] else '❌ 減速 (0分)'}")
    print(f"└─ 週期位置: {explosion['cycle_phase']} {'(+25分)' if explosion['cycle_phase'] == 'EARLY_UPCYCLE' else '(0分)'}")
    
    print(f"\n成交量比率: {explosion['volume_ratio']:.2f}x")
    print(f"綜合判斷: {'🚀 爆發性主升段!' if explosion['explosive'] else '⚪ 非爆發階段'}")
    
    if explosion['reasons']:
        print(f"\n🔥 爆發理由:")
        for i, reason in enumerate(explosion['reasons'], 1):
            print(f"   {i}. {reason}")
    ...
    # 🔥 優先檢查：主升段爆發（評分>=70分）
    if explosion["explosive"]:
        signal = "強勢持有 (HOLD - TREND EXPLOSION)"
        signal_emoji = "🚀"
        
        print(f"\n{signal_emoji} 信号: {signal}")
        print(f"\n🚀 主升段爆發行情確認! (評分: {explosion['score']}/100)")
        print(f"\n   📊 爆發指標:")
        print(f"   ├─ 資金流入: {'✅' if explosion['money_inflow'] else '❌'}")
        print(f"   ├─ 趨勢加速: {'✅' if explosion['trend_accelerating'] else '❌'}")
        print(f"   ├─ 週期位置: {explosion['cycle_phase']}")
        ...
        print(f"\n   📌 操作策略:")
        print(f"      • ❌ 不建議賣出 - 主升段應持有")
        print(f"      • 💎 回調不破均線繼續抱")
        print(f"      • 📈 使用追蹤止損代替固定止損")
        print(f"      • 🎯 追蹤止損建議: NT${current_price * 0.90:.2f} (-10%)")
        
        print(f"\n   ⚠️  離場信號:")
        print(f"      • 評分降至 50 分以下")
        print(f"      • OBV 轉向 (低於 MA20)")
        print(f"      • 週期從 EARLY_UPCYCLE → LATE_CYCLE")
        print(f"      • 趨勢加速轉為減速")
        ...
        if pattern_adjustment > 0:
            buy_reasons.append(f"看漲型態出現 (+{pattern_adjustment}分)")
        elif pattern_adjustment < 0:
            buy_warnings.append(f"看跌型態警示 ({pattern_adjustment}分)")

        # 🔥 加入主升段評分獎勵（即使未達爆發標準）
        if explosion['score'] >= 50:
            bonus = int(explosion['score'] * 0.2)  # 評分的20%作為獎勵
            buy_score += bonus
            buy_score = max(0, min(100, buy_score))
            buy_reasons.append(f"主升段潛力 (評分{explosion['score']}/100, +{bonus}分)")
        ...
        if pattern_adjustment < 0:
            ...
            reasons.append(f"看跌型態確認 ({pattern_adjustment}分)")
        ...
        if explosion["score"] >= 50:
            ...
            # 評分越高，保護力度越強
            protection_factor = 0.1 + (explosion["score"] / 100) * 0.6  # 0.1-0.7之間
            sell_score = int(sell_score * (1 - protection_factor))
            
            reasons.insert(0, f"🚀 主升段保護 (評分{explosion['score']}/100)")
            reasons.insert(1, f"賣出評分調降: {original_sell_score} → {sell_score}")
            
            if explosion['money_inflow']:
                reasons.insert(2, f"✅ 資金持續流入")
            if explosion['trend_accelerating']:
                reasons.insert(3, f"✅ 趨勢加速中")
            
            reasons.insert(4, f"💡 建議: {'暫緩賣出' if explosion['score'] >= 70 else '降低賣出比例'}")
    ...
    return {
        ...
        'explosion_detected': explosion['explosive'],
        'explosion_score': explosion['score'],
        'cycle_phase': explosion['cycle_phase']
    }
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='台股AI交易信號生成器')
    parser.add_argument('--mode', type=str, default='single', choices=['single', 'scan'],
                        help='運行模式: single=單股分析, scan=市場掃描')
    parser.add_argument('--symbols', type=str, nargs='+', 
                        default=['2330.TW', '2317.TW', '2454.TW', '2379.TW', '3008.TW'],
                        help='掃描模式時的股票代碼列表')
    ...
    if args.mode == 'single':
        ...
        if result:
            ...
            if result.get('explosion_detected'):
                print(f"   🚀 主升段爆發: {result['explosion_score']}/100 分 ({result['cycle_phase']})")
            elif result.get('explosion_score', 0) >= 50:
                print(f"   🔥 主升段潛力: {result['explosion_score']}/100 分")

            # 顯示AI模型準確度摘要
            print(f"   {get_model_accuracy_display('2330.TW')}")
        else:
            print("\n❌ 信号生成失败")
        ...
        print("🔍 台股主升段爆發股票掃描器")
        print("=" * 80)
        print(f"掃描標的: {', '.join(args.symbols)}")
        print("=" * 80)
        
        # 下載股票數據
        stock_data = download_stock_data(args.symbols, period='1y')
        ...
        if not stock_data:
            ...
            # 執行市場掃描
            print("🚀 開始掃描主升段爆發股票...\n")
            explosive_stocks = scan_market(stock_data)
            
            # 顯示結果
            print_scan_results(explosive_stocks)
            
            # 保存結果到文件
            if explosive_stocks:
                output_file = f"explosive_stocks_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
                with open(output_file, 'w', encoding='utf-8') as f:
                    f.write("=" * 80 + "\n")
                    f.write(f"主升段爆發股票掃描結果 - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
                    f.write("=" * 80 + "\n\n")
                    
                    for i, stock in enumerate(explosive_stocks, 1):
                        f.write(f"#{i} {stock['symbol']}\n")
                        f.write(f"   評分: {stock['score']}/100\n")
                        f.write(f"   週期: {stock['cycle']}\n")
                        f.write(f"   量比: {stock['volume_ratio']}x\n")
                        f.write(f"   理由:\n")
                        for reason in stock['reasons']:
                            f.write(f"      • {reason}\n")
                        f.write("\n")
