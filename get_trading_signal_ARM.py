...
def get_trading_signal():
    """生成今日交易信号"""
    print("=" * 80)
    print("🤖 美股 ARM AI AI HYBRID PPO + XGBoost 交易信号生成器")
    print("=" * 80)
    print(f"生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    # 顯示AI模型準確度
    accuracy_display = get_model_accuracy_display('ARM')
    ...
    try:
        model = joblib.load("xgb_arm_model.pkl")
    ...
    ppo_model = None
    try:
        ppo_model = PPO.load("ppo_arm_improved")
        print("✅ PPO 模型加载成功!")
    except Exception as e:
    ...
    try:
        ...
        df = yf.download('ARM', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ticker_info = yf.Ticker('ARM').info
    ...
    if ppo_model is not None:
        ...
        try:
            ...
            print(format_ppo_roi_line('ARM', 'ARM', 'ppo_arm_improved', df, ppo_action))
        except Exception as e:
    ...
    ai_muted = should_mute_ai_signal('ARM', threshold=52)
    if ai_muted:
    ...
    try:
        profit_positive = evaluate_fundamentals_for_sell(yf, 'ARM')['good']
    except Exception:
    ...
    # 7. 初始化动态权重计算器
    weight_calc = DynamicWeightCalculator('ARM')
    ...
    sentiment_result = calculate_sentiment_score('ARM', verbose=True)
    ...
    if action_value > 0.1:
        ...
        # ==========================================
        # 🎯 ARM 专属策略增强 (基于相关性与机构资金)
        # ==========================================
        print("\n" + "=" * 80)
        print("🎯 ARM Arm Holdings 专属策略分析")
        print("=" * 80)
        ...
        if target_price and target_high:
            distance_to_mean = ((target_price - current_price) / current_price) * 100
            distance_to_high = ((target_high - current_price) / current_price) * 100
            print(f"  分析師平均目標: ${target_price:.2f} ({distance_to_mean:+.1f}%)")
            print(f"  分析師最高目標: ${target_high:.2f} ({distance_to_high:+.1f}%)")
...
# 加入爆发行情评分调整
        ...
        if target_price is not None and current_price > 0:
            ...
            if current_price > target_price and recommendation_mean and recommendation_mean <= 2.5:
                ...
                if target_upside_high and target_upside_high > 0:
                    ...
                    # 股價甚至超過最高目標價 — 分析師數據滯後，但評級仍強烈看好，不計入賣出
                    reasons.append(f"🔥 動能超強! 股價超越所有分析師目標 (評級仍: {recommendation_mean:.1f}/5 強買)")
                    reasons.append(f"   ⚠️ yfinance目標價可能滯後，建議查閱最新分析師報告")
        ...
        if bb_position < 35 and sell_score > 0:
            # 获取公司基本面（超跌时才检查）
            fundamentals = evaluate_fundamentals_for_sell(yf, 'ARM')
        ...
        _growth = calculate_growth_score_adjustment(yf, 'ARM')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        # 顯示AI模型準確度摘要
        print(f"   {get_model_accuracy_display('ARM')}")
    else:
        print("\n❌ 信号生成失败")
