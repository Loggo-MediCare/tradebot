...
TICKER = '2369.TW'
print(f"訓練 2369.TW (菱生) XGBoost 模型")
print("=" * 80)
...
try:
    ...
    joblib.dump(model, 'xgb_2369_tw_model.pkl')
    print(f"\n模型已保存: xgb_2369_tw_model.pkl")

    accuracy_data = {
        'symbol': '2369.TW', 'model_type': 'XGBoost',
        'training_accuracy': float(train_acc * 100),
        'validation_accuracy': float(test_acc * 100),
        'backtest_accuracy': float(test_acc * 100),
        'last_updated': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    }
    with open('model_accuracy_2369_TW.json', 'w', encoding='utf-8') as f:
    ...
    fi_data = {
        'ticker': '2369.TW', 'model_type': 'XGBoost',
    ...
    with open('2369_TW_feature_importance.json', 'w', encoding='utf-8') as f:
