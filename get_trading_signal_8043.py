"""
台股 8043 (HONEY HOPE HONESTY ENTERPRISE C) AI 交易信号生成器 (PPO / DQN 自動選最佳)
"""
import os
...
TICKER      = '8043.TWO'
TICKER_NAME = 'HONEY HOPE HONESTY ENTERPRISE C'
DQN_PATH    = r'C:\Users\Silvi\Projects\trading-bot\8043.TWO_improved_anti_overfit_model.keras'
PPO_PATH    = r'C:\Users\Silvi\Projects\trading-bot\ppo_8043_two_improved'
WINDOW_SIZE  = 5
...
def get_trading_signal():
    print("=" * 80)
    print(f"AI 8043.TWO (HONEY HOPE HONESTY ENTERPRISE C) 交易信号生成器")
    ...
    # 準備回測用 df（小寫欄位 + 技術指標）
    df_bt = df.rename(columns={'Close':'close','Open':'open','High':'high','Low':'low','Volume':'volume'}).copy()
    df_bt['sma_10']      = df_bt['close'].rolling(10).mean()
    df_bt['sma_30']      = df_bt['close'].rolling(30).mean()
    df_bt['sma_50']      = df_bt['close'].rolling(50).mean()
    _ema12               = df_bt['close'].ewm(span=12).mean()
    _ema26               = df_bt['close'].ewm(span=26).mean()
    df_bt['macd']        = _ema12 - _ema26
    df_bt['macd_signal'] = df_bt['macd'].ewm(span=9).mean()
    _delta               = df_bt['close'].diff()
    _gain                = _delta.where(_delta>0, 0).rolling(14).mean()
    _loss                = (-_delta.where(_delta<0, 0)).rolling(14).mean()
    df_bt['rsi']         = 100 - (100 / (1 + _gain / (_loss + 1e-10)))
    _mid                 = df_bt['close'].rolling(20).mean()
    _std                 = df_bt['close'].rolling(20).std()
    df_bt['bb_upper']    = _mid + _std * 2
    df_bt['bb_lower']    = _mid - _std * 2
    df_bt = df_bt.fillna(method='bfill').fillna(method='ffill')
    ...
    if model_type == 'PPO':
        ...
        macd_sg = float((ema12 - ema26).ewm(span=9).mean().iloc[-1])
        ppo_obs = np.array([0.0, 10000.0, current_price,
                            sma_10, sma_30, sma_50, rsi,
                            macd_v, macd_sg, float(bb_up), float(bb_lo),
                            float(df['Volume'].iloc[-1]),
                            0.0, 0.0, 1.0], dtype=np.float32)
        ppo_act, _ = model.predict(ppo_obs)
        av = float(ppo_act[0])
        action = 1 if av > 0.1 else (2 if av < -0.1 else 0)
        signal, _ = signals[action]
        confidence = abs(av)
        q_values = None
        _ppo_roi, _bh_roi = calculate_ppo_backtest_roi(model, df_bt)
    else:
    ...
    if q_values is not None:
        ...
        print(f"PPO以P&L評估: 回測資料不可用 (DQN模式)")
    else:
        print_ppo_action_line(av, _ppo_roi, _bh_roi)
    ...
    sentiment_result = calculate_sentiment_score('8043.TWO', verbose=False)
    ...
    # ── Tavily 即時新聞 ─────────────────────────────────────────────────────
    print("\n" + "=" * 80)
    print("🌐 HONEY HOPE HONESTY ENTERPRISE C (8043.TWO) 即時新聞  (Tavily REST API)")
    print("=" * 80)
    print_tavily_news('8043.TWO', 'HONEY HOPE HONESTY ENTERPRISE C', max_results=5)
    print("\n" + "=" * 80)
...
if __name__ == "__main__":
    ...
    if result:
        ...
        try:
            ...
            plot_candlestick(chart_df, TICKER, save_path='8043_chart.png')
        except Exception as e:
            print(f"图表生成失败: {e}")
        print(f"\n股票: {result['symbol']} (HONEY HOPE HONESTY ENTERPRISE C)")
