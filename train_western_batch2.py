"""Train XGBoost + PPO for: AAOI, SNDK, INTC, ARM, AMD, TXN, BKR, MCHP, NXPI, SNPS, MPWR, URI, GEV, STLD, MRVL"""
...
STOCKS = [
    ...
    ('uri',  'URI',   'United Rentals'),
    ('gev',  'GEV',   'GE Vernova'),
    ('stld', 'STLD',  'Steel Dynamics'),
...
for code,ticker,name in STOCKS:
    ...
    joblib.dump(m,f'xgb_{code}_model.pkl')
    with open(f'model_accuracy_{ticker.upper()}.json','w',encoding='utf-8') as f:
    ...
    ppo.learn(total_timesteps=100000); ppo.save(f'ppo_{code}_improved')
    ...
    with open(f'model_accuracy_{ticker.upper()}_ppo.json','w',encoding='utf-8') as f:
...
print(f"\n{'='*65}\n  FINAL COMPARISON\n{'='*65}")
print(f"  {'Ticker':<8} {'Name':<26} {'XGBoost':>9} {'PPO':>9} {'Winner':>9}")
print(f"  {'-'*64}")
for t,n,xa,pa in results:
    w='XGBoost' if xa>=pa else 'PPO'
    print(f"  {t:<8} {n:<26} {xa*100:>8.2f}% {pa*100:>8.2f}% {w:>9}")
print(f"{'='*65}")
