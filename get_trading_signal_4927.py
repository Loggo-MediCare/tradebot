...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('4927.TW')


    print(f"🤖 4927.TW (泰鼎-KY) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    model_path = "ppo_4927_tw_improved"
    ...
    try:
        ...
        df = yf.download('4927.TW', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
        ...
        try:
            ...
            ticker_info = yf.Ticker('4927.TW').info
    ...
    ai_muted = should_mute_ai_signal('4927.TW', threshold=52)
    ...
    try:
        ...
        profit_positive = evaluate_fundamentals_for_sell(yf, '4927.TW')['good']
    ...
    weight_calc = DynamicWeightCalculator('4927.TW')
    ...
    sentiment_result = calculate_sentiment_score('4927.TW', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            ...
            fundamentals = evaluate_fundamentals_for_sell(yf, '4927.TW')
        ...
        _growth = calculate_growth_score_adjustment(yf, '4927.TW')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"   股票: {result['symbol']} (泰鼎-KY)")
        ...
        print(f"   {get_model_accuracy_display('4927.TW')}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 4927 泰鼎-KY 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('4927', '泰鼎-KY', max_results=5)