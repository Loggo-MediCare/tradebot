...
def get_trading_signal():
    ...
    accuracy_display = get_model_accuracy_display('3044')


    print(f"🤖 3044 (健鼎) | 準確度: {accuracy_display} | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    model_path = r"ppo_3044_tw_improved"
    ...
    try:
        ...
        df = yf.download('3044.TW', period='300d', progress=False, auto_adjust=True)  # 改为300天以计算200日均线
    ...
    ai_muted = should_mute_ai_signal('3044.TW', threshold=52)
    ...
    try:
        ...
        profit_positive = evaluate_fundamentals_for_sell(yf, '3044.TW')['good']
    ...
    sentiment_result = calculate_sentiment_score('3044.TW', verbose=True)
...
# 加入爆发行情评分调整
        ...
        if bb_position < 35 and sell_score > 0:
            ...
            fundamentals = evaluate_fundamentals_for_sell(yf, '3044.TW')
        ...
        _growth = calculate_growth_score_adjustment(yf, '3044.TW')
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"   {get_model_accuracy_display('3044')}")
...
if __name__ == '__main__':
    print('\n' + '=' * 80)
    print('🌐 3044 健鼎 即時新聞  (Tavily REST API)')
    print('=' * 80)
    print_tavily_news_tw('3044', '健鼎', max_results=5)