"""
訓練 2380 虹光 (Avision Inc.) XGBoost 模型
"""
import os
...
try:
    ...
    print(f"✅ 下載 {len(df)} 天數據 ({df['Date'].min()} 到 {df['Date'].max()})")
    ...
    with open('model_accuracy_2380.json', 'w', encoding='utf-8') as f:
    ...
    with open('2380_TW_feature_importance.json', 'w', encoding='utf-8') as f:
    ...
    print("\n" + "=" * 80)
    print(f"2380.TW {COMPANY_NAME} 訓練完成!")
    print(f"模型: {model_file}")
    print(f"準確度: {test_acc*100:.2f}%")
    print(f"最佳閾值: {best_threshold*100:.1f}%")
    print("=" * 80)
