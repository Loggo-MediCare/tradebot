...
print(f"Creating training scripts for {len(stocks_to_train)} stocks...")
print("=" * 80)
...
for ticker, name in stocks_to_train:
    ...
    try:
        ...
        created_scripts.append((filename, ticker, name))
        print(f"[OK] Created: {filename} for {ticker} {name}")
    except Exception as e:
        print(f"[ERROR] Failed to create {filename}: {e}")

print("=" * 80)
print(f"Created {len(created_scripts)} training scripts")
print("")
print("To train all models, run:")
print("  python batch_train_all.py")
