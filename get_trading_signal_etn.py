...
def get_trading_signal():
   ...
   # print(f("生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    # 1. 加载模型
    model_path = r"C:\Users\Silvi\Projects\trading-bot\ppo_etn_improved"
    print(f"\n📦 加载 AI 模型: {model_path}")
    ...
    try:
        ...
        df = yf.download('ETN', period='90d', progress=False, auto_adjust=True)
    ...
    sentiment_result = calculate_sentiment_score('ETN', verbose=False)
    ...
    # ── Tavily 即時新聞 ─────────────────────────────────────────────────────
    print("\n" + "=" * 80)
    print("🌐 Eaton (ETN) 即時新聞  (Tavily REST API)")
    print("=" * 80)
    print_tavily_news('ETN', 'Eaton', max_results=5)
...
if __name__ == "__main__":
    ...
    if result:
        ...
        try:
            import yfinance as yf
            chart_df = yf.Ticker("ETN").history(period="6mo")
            chart_df.columns = [c.lower() for c in chart_df.columns]
            chart_path = "ETN_chart.png"
            plot_candlestick(chart_df, "ETN", save_path=chart_path)
        except Exception as e:
            print(f"   圖表生成失敗: {e}")
